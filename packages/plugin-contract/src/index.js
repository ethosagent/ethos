...
export function validatePluginPackageJson(raw) {
  ...
  if (ethos && Array.isArray(ethos.credentials)) {
    ...
    for (const [i, cred] of ethos.credentials.entries()) {
      const c = cred;
  ...
  if (ethos?.dependencies && Array.isArray(ethos.dependencies)) {
    const id = ethos.id;
    if (id && ethos.dependencies.includes(id)) {
      warnings.push(`ethos.dependencies contains the plugin's own id ("${id}")`);
    }
  }
  if (
    ethos?.priority !== undefined &&
    (!ethos?.dependencies || !Array.isArray(ethos.dependencies) || ethos.dependencies.length === 0)
  ) {
