...
function extractStorageMethods(src: string): string[] {
  ...
  if (!match || match.index === undefined) throw new Error('Storage interface not found');
  const startIdx = match.index;
  const openIdx = src.indexOf('{', startIdx);
  if (openIdx < 0) throw new Error('Storage opening brace not found');
