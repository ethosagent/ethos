 ...
 */
class MemoryPostgresProvider implements MemoryProvider {
  ...
  async search(query: string, ctx: MemoryContext, _opts?: SearchOpts): Promise<MemoryEntry[]> {
    const scopeStore = this.store.get(ctx.scopeId);
    if (!scopeStore) {
      return [];
    }
  ...
  async list(ctx: MemoryContext, _opts?: ListOpts): Promise<MemoryEntryRef[]> {
    const scopeStore = this.store.get(ctx.scopeId);
    if (!scopeStore) {
      return [];
    }
...
describe('MemoryPostgresProvider sketch', () => {
  it('implements the MemoryProvider interface without external dependencies', () => {
    ...
    const provider: MemoryProvider = new MemoryPostgresProvider();
