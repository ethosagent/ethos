...
import { compoundingErrorRule, rateLimitRule, suspiciousSequenceRule, tokenBudgetRule, } from '../rules';
...
describe('rateLimitRule', () => {
    ...
    it('fires when calls exceed the threshold within the window', () => {
        ...
        const state = makeInitialState();
        for (let i = 0; i < 3; i++)
            rule.evaluate({ type: 'tool_end', toolName: 't' }, state);
        const r = rule.evaluate({ type: 'tool_end', toolName: 't' }, state);
        expect(r?.action).toBe('pause');
        if (r && r.action !== 'allow')
            expect(r.rule).toBe('rate-limit');
    });
...
describe('compoundingErrorRule', () => {
    it('fires after threshold consecutive failures', () => {
        ...
        expect(r?.action).toBe('pause');
        if (r && r.action !== 'allow')
            expect(r.rule).toBe('compounding-error');
    });
...
describe('suspiciousSequenceRule', () => {
    it('terminates when read of ~/.ssh is followed by web_post', () => {
        ...
        const state = makeInitialState();
        rule.evaluate({ type: 'tool_start', toolName: 'read_file', args: { path: '/home/u/.ssh/id_rsa' } }, state);
        const r = rule.evaluate({ type: 'tool_start', toolName: 'web_post', args: { url: 'http://x' } }, state);
        expect(r?.action).toBe('terminate');
        if (r && r.action !== 'allow')
            expect(r.rule).toBe('suspicious-sequence');
    });
    it('terminates when /etc/passwd read is followed by email_send', () => {
        const rule = suspiciousSequenceRule();
        const state = makeInitialState();
        rule.evaluate({ type: 'tool_start', toolName: 'terminal', args: { command: 'cat /etc/passwd' } }, state);
        const r = rule.evaluate({ type: 'tool_start', toolName: 'email_send', args: { to: 'evil@x' } }, state);
        expect(r?.action).toBe('terminate');
    });
    it('does not fire on benign sequences', () => {
        const rule = suspiciousSequenceRule();
        const state = makeInitialState();
        rule.evaluate({ type: 'tool_start', toolName: 'read_file', args: { path: '/proj/notes.md' } }, state);
        expect(rule.evaluate({ type: 'tool_start', toolName: 'web_post', args: { url: 'http://x' } }, state)).toBeNull();
    });
    it('respects the window — old credential-read drops out', () => {
        const rule = suspiciousSequenceRule({ window: 2 });
        const state = makeInitialState();
        rule.evaluate({ type: 'tool_start', toolName: 'read_file', args: { path: '/u/.ssh/id_rsa' } }, state);
        rule.evaluate({ type: 'tool_start', toolName: 'read_file', args: { path: '/proj/a.md' } }, state);
        rule.evaluate({ type: 'tool_start', toolName: 'read_file', args: { path: '/proj/b.md' } }, state);
        expect(rule.evaluate({ type: 'tool_start', toolName: 'web_post', args: { url: 'http://x' } }, state)).toBeNull();
    });
});
