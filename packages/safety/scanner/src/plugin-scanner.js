...
function excerptLine(line, maxLen = 80) {
...
function checkDynamicCodeExec(lines) {
    const findings = [];
...
function checkEvasionPatterns(lines) {
    const findings = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? '';
...
function checkShellExec(lines, permissions) {
    const findings = [];
    ...
    const hasChildProcessImport = /require\(['"](?:node:)?child_process['"]\)/.test(fullContent) ||
...
function checkNetworkAccess(lines, permissions) {
...
function checkCredentialAccess(lines) {
    const findings = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? '';
...
const FS_WRITE_STRING_PATTERN = /\bfs\.(writeFile|writeFileSync|appendFile|appendFileSync)\s*\(\s*(['"`])(.*?)\2/;
...
function checkFsWriteOutsideSafePath(lines) {
    const findings = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? '';
...
const POST_PATTERNS = [/\bfetch\s*\(/, /\.post\s*\(/];
function checkExfilShape(lines) {
    const findings = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? '';
        let hasEncode = false;
        ...
        for (let j = i; j <= end; j++) {
            ...
            if (findings.length > 0 && findings[findings.length - 1]?.line === i + 1)
                break;
        }
    }
    return findings;
...
// Public API
// ---------------------------------------------------------------------------
export function scanPluginCode(content, permissions = {}) {
