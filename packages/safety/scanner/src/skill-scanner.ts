...
function checkPromptInjection(lines: string[]): ScanFinding[] {
...
const HIDDEN_UNICODE_CHARS: Array<{ code: number; name: string }> = [
...
function checkHiddenUnicode(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = [];
...
function checkBase64Blobs(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = [];
...
function checkSensitiveToolInstructions(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = [];
...
function checkRoleOverride(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = [];
...
const EXTERNAL_URL_PATTERNS: RegExp[] = [/\bcurl\b/i, /\bwget\b/i, /\bfetch\s*\(/i, /https?:\/\//i];

function checkExternalUrlInstructions(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = [];
...
export function scanSkillMd(content: string, _filePath?: string): ScanResult {
