...
describe('buildMcpEnv — child process isolation', () => {
    it('child process cannot see ANTHROPIC_API_KEY', () => {
        ...
        const result = spawnSync(process.execPath, ['-e', 'process.stdout.write(process.env.ANTHROPIC_API_KEY ?? "")'], { env, encoding: 'utf8' });
        expect(result.stdout).toBe('');
    });
    it('child process cannot see OPENAI_API_KEY', () => {
        process.env.OPENAI_API_KEY = 'sk-secret-openai';
        const env = buildMcpEnv('isolation-test');
        const result = spawnSync(process.execPath, ['-e', 'process.stdout.write(process.env.OPENAI_API_KEY ?? "")'], { env, encoding: 'utf8' });
        expect(result.stdout).toBe('');
    });
    it('child process cannot see GITHUB_TOKEN (not in passthrough)', () => {
        process.env.GITHUB_TOKEN = 'ghp-secret';
        const env = buildMcpEnv('isolation-test');
        const result = spawnSync(process.execPath, ['-e', 'process.stdout.write(process.env.GITHUB_TOKEN ?? "")'], { env, encoding: 'utf8' });
        expect(result.stdout).toBe('');
    });
    ...
    it('child process HOME points to the scratch dir, not the real home', () => {
        const env = buildMcpEnv('isolation-test');
        const result = spawnSync(process.execPath, ['-e', 'process.stdout.write(process.env.HOME ?? "")'], { env, encoding: 'utf8' });
    ...
    it('child process cannot see arbitrary non-passthrough vars', () => {
        ...
        const env = buildMcpEnv('isolation-test');
        const result = spawnSync(process.execPath, ['-e', 'process.stdout.write(process.env.MY_CUSTOM_VAR ?? "")'], { env, encoding: 'utf8' });
        expect(result.stdout).toBe('');
    });
});
