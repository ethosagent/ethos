...
// is called again.
const DEFAULT_DOWNGRADED_TOOLS = [
    'terminal',
    'run_code',
    'run_tests',
    'write_file',
...
export function resolveDowngradedTools(spec) {
    if (spec === undefined || spec === 'auto')
        return new Set(DEFAULT_DOWNGRADED_TOOLS);
    return new Set(spec);
}
export const DOWNGRADE_REJECTION_MESSAGE = 'Tool blocked: an `outputIsUntrusted` tool just read external content. Dangerous tools are paused for the next turn or two. Send a new user message to clear, or re-run after acknowledging the prior content.';
