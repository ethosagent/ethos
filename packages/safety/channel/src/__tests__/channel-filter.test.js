...
describe('checkMessage', () => {
    ...
    it('non-allowlisted group message → drop silently', () => {
        ...
        const result = checkMessage(msg({ userId: 'stranger', isDm: false, isGroupMention: true }), config, db);
        expect(result.action).toBe('drop');
    });
    it('allowlisted group message + no mention → drop (mention gating)', () => {
        ...
        const result = checkMessage(msg({ userId: 'user-42', isDm: false, isGroupMention: false }), config, db);
        expect(result.action).toBe('drop');
    });
    it('allowlisted group message + mention → allow', () => {
        ...
        const result = checkMessage(msg({ userId: 'user-42', isDm: false, isGroupMention: true }), config, db);
        expect(result.action).toBe('allow');
    });
    ...
    it('contextVisibility allowlist + reply with no replyToUserId → not stripped (unknown sender)', () => {
        ...
        const result = checkMessage(msg({ userId: 'user-42', isDm: true, replyToId: 'msg-100', text: '> quoted\nmy reply' }), config, db);
        expect(result.action).toBe('allow');
    ...
    it('owner in group without mention bypasses mention gate', () => {
        ...
        const result = checkMessage(msg({ userId: 'owner-1', isDm: false, isGroupMention: false }), config, db);
        expect(result.action).toBe('allow');
    });
    it('email glob *@domain.com matches user@domain.com', () => {
        const result = checkMessage(msg({ userId: 'alice@myteam.com', isDm: true, platform: 'email' }), { recipientAllowlist: ['*@myteam.com'], dmPolicy: 'allowlist' }, db);
        expect(result.action).toBe('allow');
    });
    it('email glob *@domain.com does not match user@other.com', () => {
        const result = checkMessage(msg({ userId: 'alice@other.com', isDm: true, platform: 'email' }), { recipientAllowlist: ['*@myteam.com'], dmPolicy: 'allowlist' }, db);
        expect(result.action).toBe('drop');
    });
    ...
    it('enabled: false → non-allowlisted sender in group → allow (filter bypassed)', () => {
        ...
        const result = checkMessage(msg({ userId: 'stranger', isDm: false, isGroupMention: false }), config, db);
        expect(result.action).toBe('allow');
    });
