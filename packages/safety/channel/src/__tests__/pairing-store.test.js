...
import { clearOwnerPause, consumeAndAllow, consumeCode, generateCode, getApprovedSenders, initPairingDb, revokeApproval, } from '../pairing-store';
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe('pairing-store', () => {
    ...
    it('generate code → 8 chars, stored as pending', () => {
        ...
        const row = db.prepare('SELECT status FROM pairing_codes WHERE code = ?').get(code);
    ...
    it('nonce is deterministically derived: same nonce always produces the same code', () => {
        ...
        if (!code1)
            throw new Error('expected code1');
        // Get the stored nonce
        const row1 = db.prepare('SELECT nonce FROM pairing_codes WHERE code = ?').get(code1);
        ...
        expect(() => {
         ...
         VALUES (?, 'user-2', 'telegram', ?, ?, 'pending')`).run(code1, Date.now(), row1.nonce);
    ...
    it('5 invalid /allow attempts triggers 24h owner pause', () => {
        ...
        expect(result.ok).toBe(false);
        if (!result.ok)
            expect(result.reason).toBe('owner_paused');
    });
    ...
    it('getApprovedSenders returns senders approved via consumeAndAllow', () => {
        ...
        if (!code1 || !code2)
            throw new Error('expected codes');
