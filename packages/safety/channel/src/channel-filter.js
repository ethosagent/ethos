...
export function isSenderAllowed(message, config) {
    if (!config)
        return true; // backward-compat: no platform config means no filter
    if (config.enabled === false)
        return true; // filter disabled for this platform
    const senderId = message.userId ?? '';
    const allowlist = [];
    if (config.ownerUserId)
        allowlist.push(config.ownerUserId);
    if (config.recipientAllowlist)
        allowlist.push(...config.recipientAllowlist);
...
export function checkMessage(message, config, pairingDb) {
    ...
    if ((visibility === 'allowlist' || visibility === 'allowlist_quote') &&
        ...
        if (!replyAllowed) {
            const stripped = '[quoted content from non-allowlisted sender removed]\n' +
