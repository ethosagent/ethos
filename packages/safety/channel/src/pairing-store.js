...
export function generateCode(db, senderId, platform) {
    ...
    db.prepare(`UPDATE pairing_codes SET status = 'expired' WHERE status = 'pending' AND issued_at < ?`).run(expiryCutoff);
...
function recordFailedAttempt(db, ownerId, now) {
    db.prepare(`INSERT INTO pairing_consume_attempts (owner_id, attempted_at, succeeded) VALUES (?, ?, 0)`).run(ownerId, now);
    ...
    if (globalCount.cnt >= GLOBAL_LOCKOUT_THRESHOLD) {
        ...
        db.prepare(`INSERT OR REPLACE INTO pairing_owner_pauses (owner_id, paused_until) VALUES (?, ?)`).run(ownerId, now + LOCKOUT_DURATION_MS);
        return;
    }
    const failCount = db
        .prepare(`SELECT COUNT(*) AS cnt FROM pairing_consume_attempts
...
export function getApprovedSenders(db, platform) {
    return db.prepare('SELECT sender_id FROM allowed_senders WHERE platform = ?').all(platform).map((r) => r.sender_id);
}
