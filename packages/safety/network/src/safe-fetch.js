...
import { checkAllowDeny } from './policy';
...
export async function safeFetch(initialUrl, opts) {
 ...
 */
export async function validateUrl(url, policy, resolveHost = defaultResolveHost) {
    ...
    if (!allowDeny.allowed)
        return { ok: false, reason: allowDeny.reason };
    if (!policy.allow_private_urls) {
        ...
        if (!dnsRebindCheck.ok)
            return dnsRebindCheck;
    }
    return { ok: true };
}
// ---------------------------------------------------------------------------
