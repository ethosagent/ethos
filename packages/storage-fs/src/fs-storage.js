...
export class FsStorage {
    ...
    async write(path, content, opts) {
        ...
        if (opts?.mode !== undefined) {
            await writeFile(path, content, isBinary ? { mode: opts.mode } : { encoding: 'utf-8', mode: opts.mode });
            return;
        }
    ...
    async writeAtomic(path, content, opts) {
        ...
        const isBinary = typeof content !== 'string';
        if (opts?.mode !== undefined) {
            await writeFile(tmp, content, isBinary ? { mode: opts.mode } : { encoding: 'utf-8', mode: opts.mode });
        }
        else {
