...
describe('ScopedStorage', () => {
    ...
    it('blocks reads outside the read allowlist with BoundaryError', async () => {
        ...
        await expect(scoped.read('/ethos/personalities/engineer/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
    });
    ...
    it('blocks writes outside the write allowlist with BoundaryError', async () => {
        ...
        await expect(scoped.write('/ethos/personalities/engineer/note.md', 'hi')).rejects.toBeInstanceOf(BoundaryError);
    });
    ...
    it('exists / mtime / list / listEntries respect read allowlist', async () => {
        ...
        await expect(scoped.exists('/ethos/personalities/engineer/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
        await expect(scoped.list('/ethos/personalities/engineer')).rejects.toBeInstanceOf(BoundaryError);
        await expect(scoped.listEntries('/ethos/personalities/engineer')).rejects.toBeInstanceOf(BoundaryError);
        await expect(scoped.mtime('/ethos/personalities/engineer/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
    });
    it('mkdir / remove / rename respect write allowlist', async () => {
        ...
        await expect(scoped.mkdir('/ethos/personalities/engineer/sub')).rejects.toBeInstanceOf(BoundaryError);
        await expect(scoped.remove('/ethos/personalities/engineer/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
        await expect(scoped.rename('/ethos/personalities/researcher/MEMORY.md', '/ethos/personalities/engineer/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
    });
    it('matches prefix on directory boundary, not raw substring', async () => {
        ...
        await expect(scoped.read('/ethos/personalities/researcher/MEMORY.md')).rejects.toBeInstanceOf(BoundaryError);
    });
    ...
    it('BoundaryError carries kind and path for surface translation', async () => {
        ...
        try {
            ...
            expect(err.path).toBe('/blocked');
            expect(err.code).toBe('storage-boundary');
        }
    });
    ...
    describe('alwaysDeny floor', () => {
        ...
        it('blocks writes matching alwaysDeny even when write grants the parent', async () => {
            ...
            await expect(scoped.write('/home/.ssh/authorized_keys', 'attacker-key')).rejects.toBeInstanceOf(BoundaryError);
        });
