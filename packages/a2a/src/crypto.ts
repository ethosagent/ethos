// A2A crypto — the SHARED, PURE primitives for Agent Cards: Ed25519 keygen /
// signing / verification, deterministic canonicalization, key fingerprint, and
// `did:key` derivation. Uses Node's built-in `crypto` (WebCrypto-grade
// Ed25519) — no hand-rolled primitives (plan §0A / §7).
//
// This is the SINGLE home for card canonicalization + key math: the signer
// (`extensions/personalities` — signs its OWN card) and the verifier
// (`fetchAndVerifyCard`, in the sibling `./client`) both import from here, so
// there is no chance of sign/verify drift. Depends ONLY on `@ethosagent/types`
// + `node:crypto` — NO hono, NO core, NO extensions — so importers of
// `@ethosagent/a2a/crypto` never drag in the HTTP layer (plan §2 layer model).
