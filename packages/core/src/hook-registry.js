...
function isAllowed(h, allowedPlugins) {
    if (!h.pluginId)
        return true; // built-in handler — always fires
    if (allowedPlugins === undefined)
        return false; // no allowlist — block plugin handlers
...
export class DefaultHookRegistry {
    ...
    async fireVoid(name, payload, allowedPlugins) {
        const handlers = (this.voidHandlers.get(name) ?? []).filter((h) => isAllowed(h, allowedPlugins));
    ...
    async fireModifying(name, payload, allowedPlugins) {
        const handlers = (this.modifyingHandlers.get(name) ?? []).filter((h) => isAllowed(h, allowedPlugins));
        const merged = {};
    ...
    async fireClaiming(name, payload, allowedPlugins) {
        const handlers = (this.claimingHandlers.get(name) ?? []).filter((h) => isAllowed(h, allowedPlugins));
        for (const h of handlers) {
            try {
                const result = (await h.handler(payload));
    ...
    unregisterPlugin(pluginId) {
        for (const map of [this.voidHandlers, this.modifyingHandlers, this.claimingHandlers]) {
            for (const [name, handlers] of map.entries()) {
                map.set(name, handlers.filter((h) => h.pluginId !== pluginId));
            }
        }
    }
    remove(map, name, entry) {
        const list = map.get(name) ?? [];
        map.set(name, list.filter((h) => h !== entry));
    }
}
