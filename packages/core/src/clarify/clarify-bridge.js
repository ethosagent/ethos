...
export class ClarifyBridge {
    ...
    listPending(sessionId) {
        const rows = [];
        for (const entry of this.pending.values()) {
            if (sessionId === undefined || entry.row.sessionId === sessionId)
                rows.push(entry.row);
        }
        return rows;
    }
    ...
    async request(input) {
        if (!this.presenter)
            throw new ClarifyNoSurfaceError();
        if (this.hasPending(input.sessionId))
            throw new ClarifyBusyError();
        ...
        return new Promise((resolve, reject) => {
            ...
            if (input.abortSignal) {
                if (input.abortSignal.aborted) {
                    ...
                    input.abortSignal.addEventListener('abort', () => void this.respond({ requestId, answer: '', source: 'cancel' }), { once: true });
                }
            }
