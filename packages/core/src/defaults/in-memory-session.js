export class InMemorySessionStore {
    ...
    async listSessions(filter) {
        ...
        if (filter?.platform)
            results = results.filter((s) => s.platform === filter.platform);
        if (filter?.personalityId)
            results = results.filter((s) => s.personalityId === filter.personalityId);
        if (filter?.workingDir)
            results = results.filter((s) => s.workingDir === filter.workingDir);
        if (filter?.since) {
        ...
        for (const [sessionId, msgs] of this.messages.entries()) {
            ...
            for (const msg of msgs) {
                if (options?.since && msg.timestamp < options.since)
                    continue;
                if (options?.until && msg.timestamp > options.until)
                    continue;
