...
function passesFilter(entry, filterOpts) {
    ...
    if (allowedMcpServers !== undefined) {
        ...
        if (server !== undefined && !allowedMcpServers.includes(server))
            return false;
    }
    ...
    if (allowedPlugins !== undefined && entry.pluginId !== undefined) {
        if (!allowedPlugins.includes(entry.pluginId))
            return false;
    }
    return true;
}
...
export class DefaultToolRegistry {
    ...
    constructor(backends, reducers, transport) {
        ...
        this.transport =
            transport ??
                new LocalToolTransport((name) => this.tools.get(name)?.tool, backends, () => this.turnLiveCtx);
    }
    ...
    toDefinitions(allowedTools, filterOpts) {
        const entries = [...this.tools.values()].filter((e) => !e.tool.isAvailable || e.tool.isAvailable());
    ...
    async applyFilters(filters, tool, args, ctx, meta, execute) {
        const matching = filters.filter((f) => (!f.toolName ||
        ...
        for (const f of matching) {
            if (f.after)
                result = await f.after(result, ctx, meta);
        }
        return result;
    }
    async executeParallel(calls, ctx, allowedTools, filterOpts, turnAttachments, filters) {
        ...
        const results = await Promise.allSettled(calls.map(async (call) => {
            ...
            try {
                ...
                const rawResult = filters && filters.length > 0
                    ? await this.applyFilters(filters, entry.tool, call.args, ctx, { toolName: call.name, toolCallId: call.toolCallId }, () => this.transport.execute(request, ctx.abortSignal))
