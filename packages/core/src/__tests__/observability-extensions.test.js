...
describe('Observability Extensions', () => {
    ...
    describe('InMemoryRequestDumpStore', () => {
        it('round-trips records correctly', async () => {
            ...
            expect(metaOnly[0].system).toBeUndefined();
            expect(metaOnly[0].tools).toBeUndefined();
            expect(metaOnly[0].messages).toBeUndefined();
            expect(metaOnly[0].responseText).toBeUndefined();
    ...
    describe('requestDumpStore wiring', () => {
        it('appends records after each LLM call when wired', async () => {
            ...
            expect(records[0].requestId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
