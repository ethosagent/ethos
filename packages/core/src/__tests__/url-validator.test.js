...
describe('validateUrl', () => {
    ...
    it('still blocks non-loopback private IPs even with allowLocalhost', () => {
        expect(() => validateUrl('http://192.168.1.1:8080/', { allowLocalhost: true })).toThrow(SsrfError);
    ...
    it('still blocks cloud metadata even with allowLocalhost', () => {
        expect(() => validateUrl('http://169.254.169.254/', { allowLocalhost: true })).toThrow(SsrfError);
        expect(() => validateUrl('http://metadata.google.internal/', { allowLocalhost: true })).toThrow(SsrfError);
    });
