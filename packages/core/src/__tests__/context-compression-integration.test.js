...
function makeCapturingLLM(captured) {
    return {
        name: 'mock',
        model: 'mock-model',
...
describe('V1 + Q2 — compaction notice and anti-thrashing cooldown', () => {
    ...
    async function seedHeavySession(session, key) {
        const s = await session.createSession({
            key,
            platform: 'cli',
            model: 'mock-model',
    ...
    function compactionNotice(events) {
        return events.find((e) => e.type === 'tool_progress' && e.toolName === '_compaction');
    }
    it('V1 — emits a single _compaction notice on the turn compaction fires', async () => {
        ...
        const notices = events.filter((e) => e.type === 'tool_progress' && e.toolName === '_compaction');
