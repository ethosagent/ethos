...
import { EagerPrefetchPolicy, LastWriteWinsPolicy, LazyOnDemandPolicy, MemoryConflictError, } from '../memory-policies';
/**
...
class InMemoryMemoryProvider {
    ...
    seed(scopeId, key, content, mtime) {
...
// Shared test context
// ---------------------------------------------------------------------------
function makeCtx(scopeId = 'test:scope') {
...
describe('LazyOnDemandPolicy', () => {
    ...
    it('delegates sync through', async () => {
        const inner = new InMemoryMemoryProvider();
        const policy = new LazyOnDemandPolicy(inner);
        await policy.sync([{ action: 'add', key: 'onboarding.md', content: '# Onboarding' }], makeCtx('team:alpha'));
...
describe('LastWriteWinsPolicy', () => {
    it('allows sync when the entry has not been read (blind add)', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'new.md', content: 'hello' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
    ...
    it('allows sync when the entry has not changed since last read', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'decisions.md', content: 'new line' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
    });
    it('rejects sync with MemoryConflictError when entry was modified after last read', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'decisions.md', content: 'stale write' }], makeCtx('team:alpha'))).rejects.toThrow(MemoryConflictError);
    });
    it('MemoryConflictError carries the correct key and scope', async () => {
        ...
        let caught = null;
        try {
            await policy.sync([{ action: 'replace', key: 'onboarding.md', content: 'replaced' }], makeCtx('team:alpha'));
        }
        catch (err) {
    ...
    it('integration: two concurrent writers — first succeeds, second gets conflict', async () => {
        ...
        await expect(agentA.sync([{ action: 'add', key: 'decisions.md', content: 'Agent A was here' }], ctx)).resolves.toBeUndefined();
        // After Agent A's sync, the inner store has a newer mtime.
        // Agent B's read-timestamp (1000) is now stale — its write is rejected.
        await expect(agentB.sync([{ action: 'add', key: 'decisions.md', content: 'Agent B was here' }], ctx)).rejects.toThrow(MemoryConflictError);
    ...
    it('does not record mtime when entry does not exist', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'missing.md', content: 'new' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
    });
    it('records mtime from search when no prior read exists, enabling conflict detection', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'decisions.md', content: 'extra line' }], makeCtx('team:alpha'))).rejects.toThrow(MemoryConflictError);
    });
    it('read → sync → sync on the same key succeeds without spurious conflict', async () => {
        ...
        await expect(policy.sync([{ action: 'add', key: 'decisions.md', content: 'line A' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
        // Second sync without an intervening read — must also succeed (no spurious conflict).
        await expect(policy.sync([{ action: 'add', key: 'decisions.md', content: 'line B' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
    });
    it('re-add after delete does not spuriously conflict', async () => {
        ...
        await expect(policy.sync([{ action: 'delete', key: 'temp.md' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
        ...
        await expect(policy.sync([{ action: 'add', key: 'temp.md', content: 'new content' }], makeCtx('team:alpha'))).resolves.toBeUndefined();
    });
