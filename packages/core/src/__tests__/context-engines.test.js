...
describe('DropOldestEngine', () => {
    ...
    it('preserves first N turns when configured', async () => {
        ...
        expect(typeof result.messages[0]?.content === 'string' && result.messages[0].content).toContain('task:');
    });
});
describe('SemanticSummaryEngine', () => {
    ...
    it('inserts a summary message in the middle when under pressure', async () => {
        ...
        const messages = [];
        for (let i = 0; i < 20; i++)
            messages.push(userMsg(`turn ${i}: ${big}`));
        const result = await engine.compact({
            messages,
            currentSystem: '',
            targetTokens: 1000,
...
describe('DefaultContextEngineRegistry', () => {
    it('ships drop_oldest, semantic_summary, reference_preserving by default', () => {
        ...
        expect(reg.names().sort()).toEqual(['drop_oldest', 'reference_preserving', 'semantic_summary'].sort());
    });
