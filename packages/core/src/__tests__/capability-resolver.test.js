...
describe('resolveCapabilities', () => {
    ...
    it('undefined capabilities returns empty object', () => {
        const result = resolveCapabilities('tool-a', undefined, { sessionId: 'scope-1' }, emptyBackends);
        expect(result).toEqual({});
    });
    it('network capability with specific hosts creates ScopedFetchImpl', () => {
        const result = resolveCapabilities('tool-a', { network: { allowedHosts: ['api.example.com'] } }, { sessionId: 'scope-1' }, emptyBackends);
    ...
    it('network * sentinel resolves to personality policy allow list', () => {
        ...
        const result = resolveCapabilities('tool-a', { network: { allowedHosts: ['*'] } }, { sessionId: 'scope-1' }, backends);
        expect(result.scopedFetch).toBeInstanceOf(ScopedFetchImpl);
    });
    it('network * sentinel without personality policy yields empty set', () => {
        const result = resolveCapabilities('tool-a', { network: { allowedHosts: ['*'] } }, { sessionId: 'scope-1' }, emptyBackends);
        expect(result.scopedFetch).toBeInstanceOf(ScopedFetchImpl);
    });
    it('secrets capability creates ScopedSecretsImpl on secretsResolver', () => {
        ...
        const result = resolveCapabilities('tool-a', { secrets: ['API_KEY', 'DB_PASS'] }, { sessionId: 'scope-1' }, backends);
    ...
    it('secrets without backend does not create secretsResolver', () => {
        const result = resolveCapabilities('tool-a', { secrets: ['API_KEY'] }, { sessionId: 'scope-1' }, emptyBackends);
    ...
    it('storage with scope tool-private creates kvStore with scopeId tool:<toolName>', () => {
        ...
        const backends = { kvStoreFactory };
        const result = resolveCapabilities('my-tool', { storage: { scope: 'tool-private', kind: 'kv' } }, { sessionId: 'scope-1' }, backends);
    ...
    it('storage with scope session creates kvStore with scopeId session:<scopeId>', () => {
        const kvStoreFactory = vi.fn().mockReturnValue({ get: vi.fn(), set: vi.fn() });
        const backends = { kvStoreFactory };
        const result = resolveCapabilities('my-tool', { storage: { scope: 'session', kind: 'kv' } }, { sessionId: 'sess-42' }, backends);
        expect(result.kvStore).toBeDefined();
    ...
    it('storage with scope personality creates kvStore with scopeId personality:<scopeId>', () => {
        const kvStoreFactory = vi.fn().mockReturnValue({ get: vi.fn(), set: vi.fn() });
        const backends = { kvStoreFactory };
        const result = resolveCapabilities('my-tool', { storage: { scope: 'personality', kind: 'kv' } }, { sessionId: 'sess-1', personalityId: 'p-scope' }, backends);
        expect(result.kvStore).toBeDefined();
    ...
    it('storage without kvStoreFactory does not create kvStore', () => {
        const result = resolveCapabilities('my-tool', { storage: { scope: 'tool-private', kind: 'kv' } }, { sessionId: 'scope-1' }, emptyBackends);
    ...
    it('fs_reach with explicit paths creates ScopedFsImpl', () => {
        ...
        const result = resolveCapabilities('tool-a', { fs_reach: { read: ['/data'], write: ['/out'] } }, { sessionId: 'scope-1' }, backends);
    ...
    it('fs_reach with from-personality uses personalityFsReach paths', () => {
        ...
        const result = resolveCapabilities('tool-a', { fs_reach: { read: 'from-personality', write: 'from-personality' } }, { sessionId: 'scope-1' }, backends);
        expect(result.scopedFs).toBeInstanceOf(ScopedFsImpl);
    });
    it('fs_reach without storage backend does not create scopedFs', () => {
        const result = resolveCapabilities('tool-a', { fs_reach: { read: ['/data'], write: ['/out'] } }, { sessionId: 'scope-1' }, emptyBackends);
        expect(result.scopedFs).toBeUndefined();
        expect(result).toEqual({});
    });
    it('process capability creates ScopedProcessImpl on scopedProcess', () => {
        const result = resolveCapabilities('tool-a', { process: { allowedBinaries: ['echo', 'ls'] } }, { sessionId: 'scope-1' }, emptyBackends);
    ...
    it('attachments with fs_reach extends ScopedFs read paths with attachment cache dirs', async () => {
        ...
        const result = resolveCapabilities('vision_analyze', {
            attachments: { kinds: ['image'] },
            fs_reach: { read: 'from-personality' },
        }, { sessionId: 'scope-1' }, backends);
        expect(result.scopedFs).toBeInstanceOf(ScopedFsImpl);
        expect(result.attachments).toBeDefined();
        // The ScopedFs should allow reading from the attachment cache directory
        await expect(result.scopedFs?.read('/tmp/ethos-cache/sess1/photo.jpg')).resolves.toBe('content');
    ...
    it('attachments without fs_reach creates read-only ScopedFs for attachment dirs', async () => {
        ...
        const result = resolveCapabilities('vision_analyze', { attachments: { kinds: ['image'] } }, { sessionId: 'scope-1' }, backends);
        expect(result.scopedFs).toBeInstanceOf(ScopedFsImpl);
        expect(result.attachments).toBeDefined();
        // Can read from the attachment cache directory
        await expect(result.scopedFs?.read('/tmp/ethos-cache/sess1/photo.jpg')).resolves.toBe('content');
        // Cannot write (read-only)
        await expect(result.scopedFs?.write('/tmp/ethos-cache/sess1/other.jpg', 'x')).rejects.toThrow('PATH_NOT_REACHABLE');
    });
    ...
    it('partial capabilities only populates declared fields', () => {
        ...
        const result = resolveCapabilities('tool-partial', {
