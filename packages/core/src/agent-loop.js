...
import { DOWNGRADE_REJECTION_MESSAGE, INJECTION_DEFENSE_PRELUDE, resolveDowngradedTools, sanitize, shortPatternCheck, wrapUntrusted, } from '@ethosagent/safety-injection';
...
export class AgentLoop {
    ...
    constructor(config) {
        ...
        if (config.injectionClassifier)
            this.injectionClassifier = config.injectionClassifier;
        if (config.watcher)
            this.watcher = config.watcher;
        if (config.clarifyBridge)
            this.clarifyBridge = config.clarifyBridge;
        if (config.requestDumpStore)
            this.requestDumpStore = config.requestDumpStore;
        if (config.mcpPolicy)
            this.mcpPolicy = config.mcpPolicy;
        if (config.onToolMetric)
            this.onToolMetric = config.onToolMetric;
        if (config.credentialCheck)
            this.credentialCheck = config.credentialCheck;
    ...
    getPersonalityBudgetCap(personalityId) {
        const p = (personalityId ? this.personalities.get(personalityId) : null) ??
    ...
    resolveModelWithTier(personality, tier) {
        ...
        if (modelConfig && typeof modelConfig === 'object' && personality.provider === this.llm.name) {
            ...
            if (tierModel)
                return { model: tierModel, source: 'personality' };
        }
    ...
    async *run(text, opts = {}) {
        ...
        const ethosSession = (await this.session.getSessionByKey(sessionKey)) ??
        ...
        const personality = (opts.personalityId ? this.personalities.get(opts.personalityId) : null) ??
            this.personalities.getDefault();
        ...
        const { model: effectiveModel, source: modelSource } = this.resolveModelWithTier(personality, activeTier);
        ...
        const allowedMcpTools = mcpServers
            ? Object.fromEntries(Object.entries(mcpServers)
        ...
        const activeMemory = personality.memory?.provider
            ? ((await this.memoryProviders.get(personality.memory.provider)?.(personality.memory.options)) ?? this.memory)
            : this.memory;
        ...
        for (const injector of this.injectors) {
            ...
            if (injPluginId !== undefined && !allowedPlugins.includes(injPluginId))
                continue;
            if (injector.shouldInject && !injector.shouldInject(promptCtx))
                continue;
        ...
        const activeSkillFiles = Array.isArray(rawSkills) && rawSkills.every((s) => typeof s === 'string')
            ? rawSkills
            : undefined;
        ...
        if (buildResult.overrideSystem) {
            ...
            if (buildResult.prependSystem)
                systemParts.unshift(buildResult.prependSystem);
            if (buildResult.appendSystem)
                systemParts.push(buildResult.appendSystem);
        }
        if (opts.dryRun) {
            systemParts.push('IMPORTANT: You are in DRY-RUN mode. Every tool call will be intercepted and return a stub ' +
        ...
        for (let iteration = 0; iteration < this.maxIterations; iteration++) {
            ...
            const overusedTool = [...toolNameCounts.entries()].find(([, count]) => count >= this.maxIdenticalToolCalls);
            if (overusedTool) {
                yield {
                    type: 'tool_progress',
            ...
            try {
                ...
                for await (const chunk of stream) {
                    ...
                    if (chunk.type === 'usage') {
                        ...
                        if (chunk.usage.requestTokens)
                            llmRequestTokens = chunk.usage.requestTokens;
                    }
                    for (const event of this.handleChunk(chunk, pendingToolCalls, (t) => {
                        ...
                        if (event.type === 'usage') {
                            this.sessionCosts.set(sessionKey, (this.sessionCosts.get(sessionKey) ?? 0) + event.estimatedCostUsd);
            ...
            const execResults = execInputs.length > 0
            ...
            if (directResult?.result.ok) {
                ...
                if (traceId)
                    this.observability?.endTrace(traceId, 'ok');
                this.observability?.flush();
            ...
            for (const p of prepped) {
                ...
                if (p.rejected !== undefined) {
                    ...
                    if (result.ok && result.cost_usd) {
                        this.sessionCosts.set(sessionKey, (this.sessionCosts.get(sessionKey) ?? 0) + result.cost_usd);
                        yield {
                            type: 'usage',
                            inputTokens: 0,
                            outputTokens: 0,
                    ...
                    if (touchedPath !== undefined) {
                        await this.hooks.fireVoid('tool_end_with_path', {
                            sessionId,
                            personalityId: personality.id,
                            toolName: p.name,
                    ...
                    if (injectionDefenseEnabled && result.ok) {
                        ...
                        if (tool?.outputIsUntrusted) {
                            const verdict = await this.handleUntrustedResult(p.name, p.args, result.value, personality, traceId);
        ...
        await this.hooks.fireVoid('agent_done', {
            sessionId,
            text: fullText,
            turnCount,
            personalityId: personality.id,
    ...
    *handleChunk(chunk, pendingToolCalls, onText) {
        switch (chunk.type) {
            ...
            case 'tool_use_delta': {
                ...
                if (tc)
                    tc.partialJson += chunk.partialJson;
                break;
            }
            case 'tool_use_end': {
                const tc = pendingToolCalls.find((t) => t.toolCallId === chunk.toolCallId);
    ...
    dedupHistory(history) {
        ...
        history.forEach((msg, idx) => {
            ...
            if (list)
                ...
                occurrences.set(fingerprint, [idx]);
        });
        ...
        for (const indices of occurrences.values()) {
            ...
            for (const idx of indices.slice(1)) {
                replacement.set(idx, `[deduped — identical to earlier result, see tool_use id ${oldestId}]`);
            }
        }
    ...
    async handleUntrustedResult(toolName, args, rawValue, personality, traceId) {
        ...
        const shouldCallLLM = this.injectionClassifier !== undefined &&
    ...
    async maybeCompact(messages, systemPrompt, personality, sessionMetadata) {
        ...
        const inCooldown = sessionMetadata.lastCompactionTurn > 0 &&
        ...
        try {
            ...
            if (changed) {
                try {
                    ...
                    await this.session.recordCompactionTurn(sessionMetadata.sessionId, sessionMetadata.turnNumber);
                }
                catch (persistErr) {
                    this.observability?.recordCompaction({
                        severity: 'warn',
    ...
    buildScopedStorage(personality) {
        ...
        const readPrefixes = fsReach?.read && fsReach.read.length > 0
            ? fsReach.read.map((p) => substitute(p, { ethosHome, self, cwd }))
            : [ownDir, `${join(ethosHome, 'skills')}/`, cwd];
        const writePrefixes = fsReach?.write && fsReach.write.length > 0
...
function extractFilePath(args) {
    ...
    if (typeof a.path === 'string' && a.path.length > 0)
        return a.path;
    if (typeof a.file_path === 'string' && a.file_path.length > 0)
        return a.file_path;
...
// ---------------------------------------------------------------------------
export function checkMcpRejectArgs(mcpPolicy, toolName, args) {
    const servers = mcpPolicy?.servers;
    if (!servers || !toolName.startsWith('mcp__'))
        return undefined;
...
// ---------------------------------------------------------------------------
export function checkMcpEnabled(mcpPolicy, toolName) {
    const servers = mcpPolicy?.servers;
    if (!servers || !toolName.startsWith('mcp__'))
        return undefined;
    const firstSep = toolName.indexOf('__');
...
function describeSource(toolName, args) {
    ...
    if (typeof a.path === 'string')
        return `${toolName === 'read_file' ? 'file:' : ''}${a.path}`;
