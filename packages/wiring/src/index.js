...
import { AgentLoop, ChainedProvider, ClarifyBridge, DefaultHookRegistry, DefaultLLMProviderRegistry, DefaultMemoryProviderRegistry, DefaultNotificationRouter, DefaultToolRegistry, DefaultToolResultReducerRegistry, EagerPrefetchPolicy, FileClarifyStore, LastWriteWinsPolicy, LazyOnDemandPolicy, SimpleCompletionImpl, validateUrl, } from '@ethosagent/core';
...
import { platformId as discordId, platformPrompt as discordPrompt, } from '@ethosagent/platform-discord/format';
import { platformId as emailId, platformPrompt as emailPrompt, } from '@ethosagent/platform-email/format';
import { platformId as slackId, platformPrompt as slackPrompt, } from '@ethosagent/platform-slack/format';
import { platformId as telegramId, platformPrompt as telegramPrompt, } from '@ethosagent/platform-telegram/format';
...
import { createKanbanRoleGateHook, createKanbanTools, registerPostmortemHandler, } from '@ethosagent/tools-kanban';
...
import { createPersonalityDesignTools, createTeamDesignTools, } from '@ethosagent/tools-personality-design';
...
function createSingleProvider(cfg) {
    ...
    if (cfg.provider === 'azure') {
        if (!cfg.baseUrl) {
            throw new Error('Azure provider requires `baseUrl` set to the resource endpoint (e.g. https://my-resource.openai.azure.com).');
        }
...
function buildCompressionSummarizer(registry, config, observability, log) {
    ...
    const getProvider = async () => {
        ...
        if (!factory) {
            throw new Error(`LLM provider "${providerName}" is not registered (compression summarizer). ` +
    ...
    return async (middle, targetTokens) => {
        ...
        let outputTokens = 0;
        try {
            const stream = provider.complete([{ role: 'user', content: renderMiddleForSummary(middle) }], [], {
            ...
            log.warn(`compression summarizer failed: ${err instanceof Error ? err.message : String(err)}`);
            throw err;
        }
...
export async function createLLM(config) {
    ...
    if (config.providers && config.providers.length >= 2) {
        const instances = config.providers.map((p) => createSingleProvider({
            provider: p.provider,
...
async function createLLMFromRegistry(registry, config, log) {
    ...
    const resolveOne = async (cfg) => {
        ...
        if (!factory) {
            throw new Error(`LLM provider "${cfg.provider}" is not registered. ` +
                `Available: ${registry.list().join(', ')}`);
        }
        ...
        if (typeof provider.supportsCaching !== 'boolean' ||
            typeof provider.supportsThinking !== 'boolean' ||
            typeof provider.maxContextTokens !== 'number') {
            throw new Error(`LLM provider "${cfg.provider}" is missing required capability declarations ` +
    ...
    if (config.providers && config.providers.length >= 2) {
        const instances = await Promise.all(config.providers.map((p) => resolveOne({
            provider: p.provider,
            model: p.model ?? config.model,
            apiKey: p.apiKey,
            ...(p.baseUrl !== undefined ? { baseUrl: p.baseUrl } : {}),
...
export async function createAgentLoop(config, opts) {
    ...
    if (config.storage?.encryption) {
        ...
        if (!key) {
            console.error('Error: storage encryption is enabled but ETHOS_STORAGE_KEY is not set.\n' +
    ...
    llmProviders.register('azure', async ({ config: cfg, secrets }) => {
        if (!cfg.baseUrl) {
            throw new Error('Azure provider requires `baseUrl` set to the resource endpoint ' +
    ...
    if (!opts.disableDocker) {
        ...
        if (!sandbox.isAvailable())
            log.warn('Docker not available — run_code tool disabled');
    }
    ...
    for (const tool of createTodoTools(todoStore))
        tools.register(tool);
    ...
    const clarifyBridge = new ClarifyBridge(new FileClarifyStore(new FsStorage(), join(dataDir, 'clarify')));
    for (const tool of createInteractiveTools(clarifyBridge))
        tools.register(tool);
    ...
    if (personalityWantsKanban(activePerson)) {
        ...
        for (const tool of createKanbanTools(kanbanOpts))
            tools.register(tool);
    }
    for (const tool of createProcessTools(dataDir))
        tools.register(tool);
    ...
    if (!opts.disableDocker) {
        for (const tool of createCodeTools(sandbox))
            tools.register(tool);
    ...
    if (opts.cronScheduler) {
        for (const tool of createCronTool(opts.cronScheduler))
            tools.register(tool);
    }
    ...
    for (const tool of createTtsTools({ provider: null }))
        tools.register(tool);
    ...
    for (const tool of createSkillsTools({
        ...
        getSkillContent: (name) => {
            for (const skill of skillPool.values()) {
                if (skill.name === name || skill.qualifiedName === name)
                    return skill.body;
            }
            return null;
        },
    ...
    if (mcpConfig.length > 0) {
        ...
        if (attached.length === 0) {
            ...
            log.info(`MCP: 0 of ${mcpConfig.length} server(s) attached to "${activePerson.id}". ` +
    ...
    if (config.modelCatalogConfig && config.modelCatalogConfig.enabled !== false) {
        try {
            const catalogUrl = config.modelCatalogConfig.url ?? 'https://ethos-agent.ai/api/model-catalog.json';
            ...
            if (config.modelCatalogConfig.providers) {
                for (const [providerId, providerCfg] of Object.entries(config.modelCatalogConfig.providers)) {
                    try {
                        ...
                        log.warn(`model catalog: per-provider override for '${providerId}' failed; using main catalog`);
                    }
                }
            }
    ...
    const { injectors, tools: skillTools, scanner: skillScanner, } = createInjectors(personalities, {
    ...
    if (kanbanStore !== null && config.teamName !== undefined && config.role !== undefined) {
        hooks.registerModifying('before_tool_call', createKanbanRoleGateHook({
            role: config.role,
    ...
    if (config.teamName) {
        if (!isSafeTeamName(config.teamName)) {
            throw new Error(`Invalid teamName "${config.teamName}": must match [a-zA-Z0-9_-]+ (no path separators or traversal)`);
        }
        ...
        const teamMemory = new LazyOnDemandPolicy(new LastWriteWinsPolicy(new MarkdownFileMemoryProvider({ dir: teamMemoryDir })));
        // Seed bootstrap topic files if the directory has no .md files yet.
        await seedTeamMemory(teamMemory, config.teamName);
        for (const tool of createTeamMemoryTools(teamMemory))
            tools.register(tool);
    ...
    if (pluginSkillSources.length > 0) {
        ...
        for (const [k, v] of pluginSkillPool)
            skillPool.set(k, v);
    }
    // -------------------------------------------------------------------------
    ...
    pluginRegistries.llmFactory = () => new SimpleCompletionImpl(llm, config.model, ({ input, output }) => {
    ...
    if (!memoryFactory) {
        throw new Error(`Memory provider "${memoryName}" is not registered. ` +
            `Available: ${memoryProviders.list().join(', ')}`);
    }
    const memory = new EagerPrefetchPolicy(await memoryFactory({
        config: {},
        dataDir,
        secrets: config.secretsResolver ?? NOOP_SECRETS,
        logger: log,
    }));
    for (const tool of createMemoryTools(memory, session))
        tools.register(tool);
    ...
    const auxVisionCollidesWithPrimary = auxVisionConfig !== undefined && auxVisionConfig.model === config.model;
    if (auxVisionConfig &&
        ...
        log.warn(`auxiliary.vision.model ("${auxVisionConfig.model}") matches the primary model; ` +
    ...
    if (auxVisionConfig && !auxVisionCollidesWithPrimary) {
        ...
        if (auxFactory) {
            ...
            log.warn(`auxiliary.vision provider "${auxProviderName}" not registered; ` +
    ...
    const { Watcher: WatcherClass, defaultRules: watcherDefaultRules } = await import('@ethosagent/safety-watcher');
    ...
    for (const name of memoryProviders.list()) {
        ...
        if (factory) {
            memoryProviderMap.set(name, (options) => factory({
                config: options ?? {},
                dataDir,
                secrets: config.secretsResolver ?? NOOP_SECRETS,
                logger: log,
    ...
    for (const tool of createDelegationTools(loop, opts.meshRegistryPath))
        tools.register(tool);
    ...
    if (validationErrors.length > 0) {
        const summary = validationErrors
            ...
            .join('\n');
        throw new Error(`Tool capability validation failed for personality "${activePerson.id}":\n${summary}\n` +
    ...
    try {
        ...
        if (refs.length === 0) {
            for (const topic of TEAM_MEMORY_BOOTSTRAP_TOPICS) {
                ...
                await teamMemory.sync([{ action: 'add', key: `${topic.key}.md`, content: topic.placeholder }], seedCtx);
            }
        }
    }
    catch {
...
export { ETHOS_EVENT_CATEGORIES, ETHOS_TRACE_KINDS, EthosObservability, } from './observability/ethos-observability';
