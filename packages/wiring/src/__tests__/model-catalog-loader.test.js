...
import { bundledToManifest, fetchManifest, loadCachedManifest, loadModelCatalog, mergeRemoteIntoBundled, writeCachedManifest, } from '../model-catalog-loader';
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
describe('fetchManifest', () => {
    ...
    it('throws on non-200 status', async () => {
        ...
        await expect(fetchManifest('https://example.com/catalog.json')).rejects.toThrow('model catalog fetch failed: HTTP 500');
    });
    it('throws on invalid manifest shape (missing version)', async () => {
        vi.stubGlobal('fetch', mockFetchOk({ updatedAt: 'x', providers: {} }));
        await expect(fetchManifest('https://example.com/catalog.json')).rejects.toThrow('model catalog: invalid manifest shape');
    });
...
describe('loadModelCatalog', () => {
    ...
    it('200 OK with malformed JSON → falls through to bundled, logs warning', async () => {
        ...
        expect(result.providers.anthropic.models.length).toBe(bundled.providers.anthropic.models.length);
    ...
    it('cache present but stale, remote fails → stale cache + warn log', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('model catalog fetch failed; using cache'));
    });
