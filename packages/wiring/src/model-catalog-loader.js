...
export async function fetchManifest(url) {
    ...
    try {
        ...
        if (!result.ok)
            throw new Error(`model catalog fetch blocked: ${result.reason}`);
        const res = result.response;
        if (!res.ok)
            throw new Error(`model catalog fetch failed: HTTP ${res.status}`);
        const json = await res.json();
        if (!isValidManifest(json))
            throw new Error('model catalog: invalid manifest shape');
        return json;
    }
    finally {
...
function isValidManifest(obj) {
    ...
    if (typeof m.updatedAt !== 'string')
        return false;
    if (typeof m.providers !== 'object' || m.providers === null)
        return false;
    for (const [, provider] of Object.entries(m.providers)) {
 ...
 */
export async function loadCachedManifest(storage, cachePath) {
    ...
    try {
        ...
        return { manifest: manifest, ageMs };
    }
    catch {
        return null;
    }
 ...
 */
export async function writeCachedManifest(storage, cachePath, manifest) {
 ...
 */
export function bundledToManifest() {
    const providers = {};
    for (const entry of MODEL_CATALOG) {
        const key = entry.providerId === 'anthropic'
            ? 'anthropic'
 ...
 */
export function mergeRemoteIntoBundled(remote, bundled) {
...
export async function loadModelCatalog(opts) {
    ...
    try {
        ...
        logger?.info(`model catalog: loaded ${Object.values(remote.providers).reduce((sum, p) => sum + p.models.length, 0)} entries from remote (manifest v${remote.version}, updated ${remote.updatedAt})`);
 ...
 */
export function manifestToEntries(manifest) {
    const entries = [];
