export class DiagnosticStore {
    ...
    pushEvent(event) {
        if (!this.events.has(event.pluginId))
            this.events.set(event.pluginId, []);
        const buf = this.events.get(event.pluginId);
        if (buf) {
            buf.push(event);
            if (buf.length > this.maxPerPlugin)
                buf.shift();
        }
    }
    pushMetric(metric) {
        if (!this.metrics.has(metric.pluginId))
            this.metrics.set(metric.pluginId, []);
    ...
    getEvents(pluginId, opts) {
        ...
        if (opts?.level)
            buf = buf.filter((e) => e.level === opts.level);
