...
export class PluginApiImpl {
    ...
    registerContextEngine(engine) {
        if (!this.registries.contextEngines) {
            throw new Error(`Plugin "${this.pluginId}" called registerContextEngine but the host wiring did not expose a ContextEngineRegistry.`);
        }
    ...
    registerLLMProvider(name, factory) {
        ...
        if (name.includes('/') && !name.startsWith(`${this.pluginId}/`)) {
            throw new Error(`Plugin "${this.pluginId}" cannot register LLM provider "${name}": ` +
    ...
    registerMemoryProvider(name, factory) {
        const qualifiedName = name.includes('/') ? name : `${this.pluginId}/${name}`;
        if (name.includes('/') && !name.startsWith(`${this.pluginId}/`)) {
            throw new Error(`Plugin "${this.pluginId}" cannot register memory provider "${name}": ` +
                `namespaced names must start with the plugin id ("${this.pluginId}/").`);
        }
    ...
    registerPlatformAdapter(name, factory) {
        if (!this.registries.platformAdapters) {
            throw new Error(`Plugin "${this.pluginId}" called registerPlatformAdapter but the host wiring did not expose an adapter registry.`);
        }
    ...
    async setSecret(key, value) {
        ...
        await this.credentialStorage.writeAtomic(`${this.credPath(key)}.meta`, JSON.stringify({ updatedAt: new Date().toISOString() }));
    ...
    onCredentialUpdate(handler) {
        ...
        return () => {
            ...
            if (idx >= 0)
                this.credentialUpdateHandlers.splice(idx, 1);
        };
    }
    ...
    registerRoute(opts) {
        ...
        if (!opts.path.startsWith('/')) {
            throw new Error(`Plugin "${this.pluginId}": route path must start with "/", got "${opts.path}"`);
        }
        const entry = {
    ...
    startMonitor(name, params) {
        const store = new Map();
        this.diagnostics.info('Monitor started with ephemeral kvStore — data will not persist across restarts', { monitorName: name });
    ...
    async notify(opts) {
        if (!this.registries.notificationRouter) {
            this.diagnostics.warn('notify() called but no NotificationRouter is configured — message dropped', { sessionKey: opts.sessionKey });
            return;
        }
    ...
    registerRenderer(spec) {
        ...
        if (existing && !this.registeredRendererTypes.includes(spec.type)) {
            throw new Error(`Plugin "${this.pluginId}" cannot register renderer type "${spec.type}": already registered by another plugin.`);
        }
    ...
    cleanup() {
        ...
        for (const inj of this.registeredInjectors) {
            ...
            if (idx >= 0)
                this.registries.injectors.splice(idx, 1);
