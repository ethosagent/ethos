...
import { BUILTIN_SKIN_NAMES, BUILTIN_SKINS, deepMerge, resolveBuiltinSkin, resolveSkin, } from '../skins';
...
describe('deepMerge', () => {
    ...
    it('ignores undefined overrides — leaves base intact', () => {
        ...
        expect(deepMerge(base, overrides)).toEqual({ a: 1, b: 2 });
    });
});
...
describe('resolveSkin (extends chain)', () => {
    ...
    it('throws on unknown skin name', () => {
        expect(() => resolveSkin(DEFAULT_TOKENS, BUILTIN_SKINS, 'nonexistent')).toThrow(/Unknown skin: "nonexistent"/);
    });
...
describe('Skin shape (type-level + runtime sanity)', () => {
    it('a skin with no overrides is a valid Skin', () => {
        const skin = { name: 'noop', description: 'no-op', tokens: {} };
