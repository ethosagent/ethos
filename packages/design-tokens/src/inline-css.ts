...
// Email digest adapter. Inline styles (rather than CSS classes) because
// many email clients strip `<style>` blocks and most don't honor :root
// custom properties. Light-mode-only per DESIGN.md (most clients render
// dark mode poorly). Email aggregates across personalities, so we ship a
// single brand accent (researcher blue) rather than fingerprinting per
// agent.

export interface InlineCssOptions {
  /** Override the brand accent. Defaults to researcher blue from tokens. */
 ...
 */
export function tokensToInlineCss(
  tokens: Tokens,
  options: InlineCssOptions = {},
): Record<'body' | 'heading' | 'link' | 'meta' | 'rule' | 'badge', string> {
