...
export class AgentBridge extends EventEmitter {
    ...
    replaceLoop(newLoop) {
        ...
        if (this.clarifyPresenter)
            cb.setPresenter(this.clarifyPresenter);
        for (const listener of this.clarifyResolvedListeners)
            cb.onResolved(listener);
    }
    // -------------------------------------------------------------------------
    // Internals
    // -------------------------------------------------------------------------
    async runTurn(input, opts) {
        ...
        const timeoutHandle = setTimeout(() => {
            if (!this.controller)
                return; // turn already finished normally
            timedOut = true;
            this.flushText();
            this.emit('error', `Agent did not complete within ${Math.round(this.turnTimeoutMs / 60_000)} minutes — turn abandoned.`, 'TIMEOUT');
            this.controller.abort();
            ...
            for await (const event of this.loop.run(input, {
                ...
                switch (event.type) {
                    ...
                    case 'tool_progress':
                        ...
                        break;
                    case 'tool_end':
                        this.emit('tool_end', event.toolCallId, event.toolName, event.ok, event.durationMs, event.result);
                        break;
                    case 'usage':
