...
export default function HomePanel({ pluginId, getCredential, credentialPreview, requestOAuth, executeTool, theme }: PluginPanelProps) {
  ...
  useEffect(() => {
    credentialPreview('__OAUTH_REF__').then(setAuthStatus).catch(() => {});
  }, [credentialPreview]);

  const fetchData = useCallback(async (toolName: string, args: Record<string, unknown> = {}) => {
    setLoading(true);
    setError(null);
    try {
  ...
  return (
    <div style={{ padding: 16, fontFamily: 'var(--font-display, system-ui)', color: 'var(--text-primary, #e8e8e6)' }}>
      {/* Header */}
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border-subtle, #2a2a2a)', marginBottom: 12 }}>
        {tabs.map((tab, i) => (
          <button
            key={tab}
            type="button"
            ...
            style={{
              ...
              background: 'none',
              border: 'none',
              borderBottom: activeTab === i ? '2px solid var(--info, #4a9eff)' : '2px solid transparent',
              cursor: 'pointer',
              marginBottom: -1,
            }}
          >
      ...
      <div style={{ minHeight: 120 }}>
        {loading ? (
          ...
          <pre style={{ margin: 0, fontSize: 12, fontFamily: 'var(--font-mono, monospace)', whiteSpace: 'pre-wrap' }}>
            {data}
          </pre>
        ) : (
