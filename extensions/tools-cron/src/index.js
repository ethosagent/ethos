...
// Factory
// ---------------------------------------------------------------------------
export function createCronTool(scheduler) {
    return [
        {
            name: 'cron',
            description: 'Manage scheduled recurring tasks. Use the `action` field to create, list, get, read_run, update, pause, resume, run, or remove cron jobs.',
            toolset: 'cron',
            capabilities: {},
            schema: {
                type: 'object',
                properties: {
                    ...
                    schedule: {
                        type: 'string',
                        description: 'Schedule: cron ("0 8 * * 1-5"), relative delay ("30m"), recurring interval ("every 2h"), or ISO timestamp ("2026-06-01T09:00:00Z"). All cron times are local (create, update).',
                    },
                    prompt: {
                        type: 'string',
                    ...
                    missed_run_policy: {
                        type: 'string',
                        enum: ['run-once', 'skip'],
                        description: '"run-once" runs the missed job on next start. "skip" waits for the next scheduled time. Default: skip (create).',
                    },
                    context_from: {
                        type: 'array',
                        items: { type: 'string' },
                        description: 'Job ids/names whose latest output will be prepended as context at run time (optional, create).',
                    },
                    repeat: {
                        type: 'object',
                        description: 'Repeat policy: { kind: "forever" | "once" | "count", maxRuns?: number }. Default: forever for cron/interval, once for delays/timestamps (create).',
                        properties: {
            ...
            async execute(args, ctx) {
                const { action, name, schedule, prompt, missed_run_policy, context_from, repeat, id, at, personalityId, } = args;
                switch (action) {
                    case 'create':
 ...
 */
function extractOrigin(ctx) {
    if (!ctx.platform || ctx.platform === 'cli' || ctx.platform === 'web')
        return undefined;
    const parts = ctx.sessionKey.split(':');
    if (parts.length >= 3) {
        const chatId = parts[2];
        if (chatId)
            return { platform: ctx.platform, chatId };
    }
    return undefined;
}
async function handleCreate(scheduler, ctx, args) {
    ...
    if (!schedule)
        return { ok: false, error: 'schedule is required', code: 'input_invalid' };
    if (!prompt)
...
async function handleList(scheduler, args) {
...
async function handleGet(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    const job = await scheduler.getJob(args.id);
    if (!job)
        return { ok: false, error: `Job not found: ${args.id}`, code: 'input_invalid' };
    let runs = [];
    try {
...
async function handleReadRun(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    if (!args.at)
        return { ok: false, error: 'at is required', code: 'input_invalid' };
    let runs = [];
    try {
        runs = await scheduler.listRuns(args.id);
    }
...
async function handleUpdate(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
...
async function handlePause(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    try {
...
async function handleResume(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    try {
...
async function handleRun(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    try {
...
async function handleRemove(scheduler, args) {
    if (!args.id)
        return { ok: false, error: 'id is required', code: 'input_invalid' };
    try {
