...
async function seedPersonality(id) {
...
describe('update validation rejects bad inputs', () => {
    describe('fs_reach validation', () => {
        it('rejects path traversal (..) in fs_reach', async () => {
            ...
            await expect(registry.update('v-dotdot', { fs_reach: { read: ['..'] } })).rejects.toThrow(/fs_reach/);
        });
        it('rejects bare root (/) in fs_reach', async () => {
            await seedPersonality('v-bare-root');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-bare-root', { fs_reach: { read: ['/'] } })).rejects.toThrow(/fs_reach/);
        });
        it('rejects relative path without leading / or token', async () => {
            await seedPersonality('v-relative');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-relative', { fs_reach: { read: ['relative/path'] } })).rejects.toThrow(/fs_reach/);
        });
        it('rejects paths with newlines (YAML injection)', async () => {
            await seedPersonality('v-newline');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-newline', { fs_reach: { read: ['/tmp\nprovider: azure'] } })).rejects.toThrow(/invalid characters/);
        });
        it('rejects paths with commas (list separator injection)', async () => {
            await seedPersonality('v-comma');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-comma', { fs_reach: { read: ['/data,/etc/passwd'] } })).rejects.toThrow(/invalid characters/);
        });
        it('rejects tilde path (~) without leading / or token', async () => {
            await seedPersonality('v-tilde');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-tilde', { fs_reach: { read: ['~/home'] } })).rejects.toThrow(/fs_reach/);
        });
    });
    describe('capabilities validation', () => {
        it('rejects tags with spaces', async () => {
            ...
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-cap-space', { capabilities: ['has space'] })).rejects.toThrow(/capabilities tag/);
        });
        it('rejects tags with commas', async () => {
            await seedPersonality('v-cap-comma');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-cap-comma', { capabilities: ['a,b'] })).rejects.toThrow(/capabilities tag/);
        });
    });
    describe('provider validation', () => {
        it('rejects unrecognized provider', async () => {
            ...
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-bad-prov', { provider: 'made-up' })).rejects.toThrow(/provider/);
        });
        it('accepts valid provider (anthropic)', async () => {
            await seedPersonality('v-good-prov');
            const registry = makeRegistry();
            await registry.loadFromDirectory(join(testDir, 'personalities'));
            await expect(registry.update('v-good-prov', { provider: 'anthropic' })).resolves.not.toThrow();
        });
