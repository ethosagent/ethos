...
**Identity & routing.** `name`, `description`, `capabilities` (comma-separated labels — teams route by these). `provider` selects the LLM backend (`anthropic`, `openai-compat`, `azure`). `platform` declares the default channel (`cli` default, or `telegram` / `discord` / `slack` / `email`). `model` names a role (`trivial` / `default` / `deep` / `dreaming`) or an alias from the operator's `modelRegistry` — never a vendor model id, which does not parse — written as one string or as dotted per-role keys (`model.default`, `model.deep`, …).

**Memory.** There is no scope setting — `memoryScope` does not exist, and I never write it. A personality's memory is always its own: scope `personality:<id>`, under `personalities/<id>/` in the memory root. When a gateway resolves who is speaking, that person's `USER.md` also comes from their `user:<userId>` scope; `ethos chat` carries no user id. Memory shared across personalities is team memory — the `team_memory_*` tools, which need the personality in a team and listed in its toolset. `memory.provider` chooses the backend — `markdown` (default; MEMORY.md + USER.md), or `vector` (semantic recall). `memory.options` passes provider-specific config.
...
**Egress / export.** `outbound_policy.approve_before_send: true` queues an agent-initiated `send_message` for human approval instead of sending it — but only where a surface wires an outbox, which today is `ethos gateway start` and `ethos boot` and nothing else, so I say which process a personality will run in before I call it gated. `mcp_export.enabled: true` is a declaration, not a running server: it takes effect only while someone runs `ethos mcp serve --personality <id>`, which publishes ONE tool, `ask`, that runs a whole turn as this personality. `expose_tools` names what that turn may use — those tools are never published to the caller, and it can only narrow the personality's existing reach, never widen it. Everything defaults closed: no tools, no memory, no conversations, stdio-only auth. The caller never picks the personality; the server pins it.
