...
import { createPersonalityDesignTools, createTeamDesignTools, } from '../index';
...
function makePersonalityRegistry(personalities) {
    const map = new Map(personalities.map((p) => [p.id, p]));
    return {
        define: () => { },
        get: (id) => map.get(id),
        list: () => personalities,
        getDefault: () => personalities[0] ?? { id: 'default', name: 'Default' },
        setDefault: () => { },
...
describe('scaffold_personality', () => {
    it('creates personality files with valid input', async () => {
        ...
        expect(result?.ok).toBe(true);
        const ethos = await storage.read(`${process.env.HOME ?? '/root'}/.ethos/personalities/my-researcher/SOUL.md`);
        expect(ethos).toContain('# My Researcher');
        const config = await storage.read(`${process.env.HOME ?? '/root'}/.ethos/personalities/my-researcher/config.yaml`);
        expect(config).toContain('name: My Researcher');
        expect(config).toContain('model: claude-sonnet-4-6');
        const toolset = await storage.read(`${process.env.HOME ?? '/root'}/.ethos/personalities/my-researcher/toolset.yaml`);
