...
export interface PersonalityDesignDeps {
...
export function createPersonalityDesignTools(deps: PersonalityDesignDeps): Tool[] {
  return [
...
export function createTeamDesignTools(deps: TeamDesignDeps): Tool[] {
  return [
...
export function createAllDesignTools(deps: AllDesignDeps): Tool[] {
...
function listAvailableToolsTool(toolRegistry: ToolRegistry): Tool {
  return {
...
function listAvailableModelsTool(modelCatalog: ModelCatalogEntry[]): Tool {
  return {
...
function listAvailableSkillsTool(skills: Skill[]): Tool {
  return {
...
function listPersonalitiesTool(personalityRegistry: PersonalityRegistry): Tool {
  return {
    ...
    async execute(_args: unknown, _ctx: ToolContext): Promise<ToolResult> {
      ...
      const lines: string[] = [`# Registered Personalities (${personalities.length})\n`];
...
const TEAM_PATTERNS: TeamPattern[] = [
  {
...
function listTeamPatternsTool(): Tool {
  return {
    ...
    schema: { type: 'object', properties: {} },
    async execute(_args: unknown, _ctx: ToolContext): Promise<ToolResult> {
      const lines: string[] = [`# Team Patterns (${TEAM_PATTERNS.length})\n`];
...
// ===========================================================================
// Tool 7: scaffold_team
// ===========================================================================

interface ScaffoldTeamArgs {
  name: string;
  description: string;
...
function scaffoldTeamTool(storage: Storage): Tool {
  return {
    ...
    async execute(raw, _ctx): Promise<ToolResult> {
      const args = raw as ScaffoldTeamArgs;
      // Validation
