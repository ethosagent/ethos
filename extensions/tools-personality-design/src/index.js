...
function listAvailableToolsTool(toolRegistry) {
    return {
        ...
        description: 'List all registered tools with their name, description, toolset, and capabilities. Use to understand what tools are available for inclusion in a personality toolset.',
...
function listAvailableModelsTool(modelCatalog) {
    return {
        ...
        description: 'List available LLM models from the model catalog with provider, model ID, label, context window, and default flag.',
        toolset: 'personality_design',
        capabilities: {},
        schema: { type: 'object', properties: {} },
        async execute(_args, _ctx) {
...
function listAvailableSkillsTool(skills) {
    return {
        ...
        description: 'List available skills with name, summary (first 200 chars), required tools, and source. Use to understand what skills a personality might leverage.',
        toolset: 'personality_design',
        capabilities: {},
        schema: { type: 'object', properties: {} },
        async execute(_args, _ctx) {
...
function scaffoldPersonalityTool(storage, toolRegistry) {
    return {
        ...
        description: 'Validate and write personality files (SOUL.md, config.yaml, toolset.yaml) atomically to ~/.ethos/personalities/<id>/. Creates a fully-formed custom personality.',
        toolset: 'personality_design',
        ...
        async execute(raw, _ctx) {
            ...
            if (args.config.model)
                configLines.push(`model: ${yamlScalar(args.config.model)}`);
            if (args.config.provider)
                configLines.push(`provider: ${yamlScalar(args.config.provider)}`);
...
function listPersonalitiesTool(personalityRegistry) {
    return {
        ...
        description: 'List all registered personalities (built-in and custom) with their id, name, description, and toolset. Use to see available personalities for team composition.',
        toolset: 'personality_design',
        capabilities: {},
        schema: { type: 'object', properties: {} },
        async execute(_args, _ctx) {
            ...
            for (const p of personalities) {
                ...
                if (p.toolset?.length)
                    lines.push(`- **Toolset:** ${p.toolset.join(', ')}`);
                lines.push('');
            }
            return { ok: true, value: lines.join('\n') };
        },
...
const TEAM_PATTERNS = [
    {
        ...
        when_to_use: 'When you want every code change reviewed before completion. The engineer coordinates and delegates review tasks.',
    },
    {
    ...
    },
    {
        ...
        description: 'Multi-agent team with a lead engineer coordinating specialists (frontend, backend, devops).',
        dispatch_mode: 'coordinator',
        roles: [
    ...
    },
    {
        ...
        when_to_use: 'Multi-article or documentation projects where research, writing, and editing are separate concerns.',
    },
    {
        name: 'operator-team',
        description: 'Self-routing team of equally-capable operators that pick up tasks based on availability.',
        dispatch_mode: 'self-routing',
        roles: ['operator (member)', 'operator (member)', 'operator (member)'],
        when_to_use: 'High-throughput task queues where any agent can handle any task. No coordinator overhead.',
    },
];
function listTeamPatternsTool() {
    return {
        name: 'list_team_patterns',
        description: 'List curated team shape patterns (e.g. engineer-reviewer-pair, engineering-team) with descriptions, dispatch modes, and usage guidance.',
        toolset: 'personality_design',
        capabilities: {},
        schema: { type: 'object', properties: {} },
        async execute(_args, _ctx) {
...
function serializeTeamYaml(args) {
    ...
    if (args.dispatch_mode)
        lines.push(`dispatch_mode: ${yamlScalar(args.dispatch_mode)}`);
    if (args.coordinator)
        lines.push(`coordinator: ${yamlScalar(args.coordinator)}`);
    if (args.members.length === 0) {
        ...
        for (const m of args.members) {
            ...
            if (m.role)
                lines.push(`    role: ${yamlScalar(m.role)}`);
...
function scaffoldTeamTool(storage) {
    return {
        name: 'scaffold_team',
        description: 'Validate and write a team manifest YAML file to ~/.ethos/teams/<name>.yaml. Creates a team definition that can be started with `ethos team start`.',
        toolset: 'personality_design',
