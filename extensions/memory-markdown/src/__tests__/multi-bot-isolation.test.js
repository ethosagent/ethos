...
let provider;
beforeEach(() => {
    testDir = join(tmpdir(), `ethos-multi-bot-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('different-personality bots — memory isolation', () => {
    it('each bot writes to its own personality subdirectory', async () => {
        ...
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Researcher insight.' }], researcherCtx);
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Coder insight.' }], coderCtx);
        const researcherMem = await readFile(join(testDir, 'personalities', 'researcher', 'MEMORY.md'), 'utf-8');
    ...
    it('shared MEMORY.md at root is not written when both bots use per-personality scope', async () => {
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Researcher insight.' }], botCtx('researcher-bot', 'researcher'));
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Coder insight.' }], botCtx('coder-bot', 'coder'));
...
describe('same-personality bots — shared memory', () => {
    it('two bots bound to the same personality accumulate into one MEMORY.md', async () => {
        ...
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Human session fact.' }], humanBot);
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'API session fact.' }], apiBot);
        const sharedMem = await readFile(join(testDir, 'personalities', 'researcher', 'MEMORY.md'), 'utf-8');
...
describe('USER.md — scoped per personality', () => {
    it('a USER.md write from one bot is NOT visible to a different-personality bot', async () => {
        const researcherCtx = botCtx('researcher-bot', 'researcher');
        const coderCtx = botCtx('coder-bot', 'coder');
        await provider.sync([{ action: 'replace', key: 'USER.md', content: 'I prefer TypeScript.' }], researcherCtx);
    ...
    it('USER.md file lives in the personality subdirectory', async () => {
        await provider.sync([{ action: 'replace', key: 'USER.md', content: 'Per-personality user fact.' }], botCtx('researcher-bot', 'researcher'));
        const personalityUser = await readFile(join(testDir, 'personalities', 'researcher', 'USER.md'), 'utf-8');
...
describe('prefetch — per-bot memory context', () => {
    it('each bot sees only its personality MEMORY.md on prefetch', async () => {
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Researcher-only fact.' }], botCtx('researcher-bot', 'researcher'));
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Coder-only fact.' }], botCtx('coder-bot', 'coder'));
    ...
    it('prefetch includes only that personality own USER.md alongside MEMORY.md', async () => {
        await provider.sync([{ action: 'replace', key: 'USER.md', content: 'I work in fintech.' }], botCtx('researcher-bot', 'researcher'));
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Researcher memory.' }], botCtx('researcher-bot', 'researcher'));
        await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Coder memory.' }], botCtx('coder-bot', 'coder'));
        const researcherResult = await provider.prefetch(botCtx('researcher-bot', 'researcher'));
        const coderResult = await provider.prefetch(botCtx('coder-bot', 'coder'));
