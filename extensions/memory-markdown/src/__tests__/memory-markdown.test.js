...
let provider;
beforeEach(async () => {
    testDir = join(tmpdir(), `ethos-memory-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('MarkdownFileMemoryProvider', () => {
    ...
    describe('sync — add', () => {
        ...
        it('writes USER.md to the scope directory', async () => {
            await provider.sync([{ action: 'add', key: 'USER.md', content: 'Prefers TypeScript.' }], globalCtx);
    ...
    describe('sync — replace', () => {
        it('replaces entire file content', async () => {
            ...
            await provider.sync([{ action: 'replace', key: 'MEMORY.md', content: 'Brand new.' }], globalCtx);
            const content = await readFile(join(scopeDir, 'MEMORY.md'), 'utf-8');
    ...
    describe('sync — remove', () => {
        it('removes lines containing substringMatch', async () => {
            await writeFile(join(scopeDir, 'MEMORY.md'), 'Keep this line.\nRemove this specific line.\nKeep this too.\n');
            await provider.sync([{ action: 'remove', key: 'MEMORY.md', substringMatch: 'specific' }], globalCtx);
            const content = await readFile(join(scopeDir, 'MEMORY.md'), 'utf-8');
    ...
    describe('scope: personality:<id>', () => {
        ...
        it('writes per-personality MEMORY.md to the personality subdirectory', async () => {
            await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Reviewer-only fact.' }], reviewerCtx);
            const personalityFile = await readFile(join(testDir, 'personalities', 'reviewer', 'MEMORY.md'), 'utf-8');
        ...
        it('two per-personality scopes do not cross-contaminate', async () => {
            await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Reviewer fact.' }], reviewerCtx);
            await provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Operator fact.' }], operatorCtx);
            const reviewerFile = await readFile(join(testDir, 'personalities', 'reviewer', 'MEMORY.md'), 'utf-8');
            const operatorFile = await readFile(join(testDir, 'personalities', 'operator', 'MEMORY.md'), 'utf-8');
        ...
        it('USER.md is scoped per-personality (lives in personality subdirectory)', async () => {
            await provider.sync([{ action: 'add', key: 'USER.md', content: 'Senior engineer.' }], reviewerCtx);
            const personalityUser = await readFile(join(testDir, 'personalities', 'reviewer', 'USER.md'), 'utf-8');
        ...
        it('rejects unsafe personality ids with an error (no path traversal)', async () => {
            ...
            await expect(provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Evil fact.' }], evilCtx)).rejects.toThrow('unrecognised memory scope');
            const escaped = await readFile(join(testDir, '..', 'etc', 'passwd', 'MEMORY.md'), 'utf-8').catch(() => null);
        ...
        it('unrecognised scope prefix throws', async () => {
            ...
            await expect(provider.sync([{ action: 'add', key: 'MEMORY.md', content: 'Bad.' }], badCtx)).rejects.toThrow('unrecognised memory scope');
        });
    });
});
