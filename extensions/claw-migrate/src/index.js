...
export class ClawMigrator {
    ...
    async applyConfigMerge(op, plan) {
        ...
        const lines = [];
        if (kv.provider)
            lines.push(`provider: ${kv.provider}`);
        if (kv.model)
            lines.push(`model: ${kv.model}`);
        if (kv.apiKey && this.preset === 'all')
            lines.push(`apiKey: ${kv.apiKey}`);
        lines.push(`personality: ${plan.personality.resolved}`);
        if (kv.baseUrl)
            lines.push(`baseUrl: ${kv.baseUrl}`);
        for (const key of PLATFORM_TOKEN_KEYS) {
            if (kv[key])
