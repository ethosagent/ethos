...
describe('ClawMigrator.plan', () => {
    it('detects all expected files and emits ops in dependency order', async () => {
        ...
        await writeFile(join(source, 'skills', 'my-skill', 'SKILL.md'), '---\nname: my-skill\n---\nBody.\n');
...
describe('ClawMigrator.execute', () => {
    it('copies a complete OpenClaw layout into ~/.ethos', async () => {
        ...
        expect(await readFile(join(target, 'MEMORY.md'), 'utf-8')).toBe('- prefers TypeScript\n- uses pnpm\n');
        expect(await exists(join(target, 'skills', 'openclaw-imports', 'my-skill', 'SKILL.md'))).toBe(true);
