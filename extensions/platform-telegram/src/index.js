...
export class TelegramAdapter {
    ...
    async start() {
        ...
        this.bot.on('message', (ctx) => {
            ...
            const rawThreadId = typeof rawMsg.message_thread_id === 'number' ? rawMsg.message_thread_id : undefined;
            // General topic (id 1) is treated as no thread for backward compat.
            const threadId = rawThreadId !== undefined && rawThreadId !== 1 ? String(rawThreadId) : undefined;
            ...
            const hasBotPosted = threadId !== undefined && this.threadState !== undefined
        ...
        this.bot.on('edited_message', (ctx) => {
            ...
            this.editDebounce.set(debounceKey, setTimeout(() => {
        ...
        if (this.config.useWebhook && !this.config.webhookSecretToken) {
            throw new Error('TelegramAdapter: useWebhook requires webhookSecretToken for request verification');
        }
        if (this.config.useWebhook && this.config.webhookUrl) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            try {
                ...
                if (message.replyToId &&
                    ...
                    console.warn(`[telegram] HTML parse fallback chunk=${i + 1}/${totalChunks} hash=${chunkHash(raw)}`);
