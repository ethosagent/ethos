...
vi.mock('grammy', () => {
  ...
  class MockInlineKeyboard {
    private buttons: Array<{ label: string; data: string }> = [];
...
describe('4.1 — markdownToTelegramHtml', () => {
  let markdownToTelegramHtml: typeof import('../format').markdownToTelegramHtml;
  let escapeHtml: typeof import('../format').escapeHtml;
...
describe('4.1 — HTML mode send()', () => {
  ...
  it('skips HTML translation when parseMode is plain', async () => {
    const adapter = new TelegramAdapter({ token: '1:fake-token', parseMode: 'plain' });
    await adapter.start();
...
describe('4.1 — HTML mode editMessage()', () => {
  ...
  it('edits without HTML when parseMode is plain', async () => {
    ...
    const callArgs = mockApi.editMessageText.mock.calls[0][3] as Record<string, unknown>;
...
describe('4.1 — personalityRichMessage HTML mode', () => {
  it('renders bold as <b> in HTML mode', async () => {
    ...
    const card = {
      ...
      provider: 'anthropic',
      toolset: ['read_file'],
      skills: [{ id: 'review', source: 'global' as const }],
    };
