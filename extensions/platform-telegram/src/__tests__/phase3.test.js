...
function fireEditedMessage(ctx) {
    const handlers = registeredHandlers.edited_message;
    if (!handlers?.length)
        throw new Error('No edited_message handler registered');
    for (const h of handlers)
        h(ctx);
}
// ---------------------------------------------------------------------------
...
describe('3.2 — Forum-mode topic isolation', () => {
    ...
    it('passes message_thread_id on send() when threadId is set', async () => {
        ...
        expect(mockApi.sendMessage).toHaveBeenCalledWith(100, 'reply in topic', expect.objectContaining({
            message_thread_id: 42,
        }));
    });
    it('does not pass message_thread_id on send() when threadId is undefined', async () => {
        ...
        const callArgs = mockApi.sendMessage.mock.calls[0][2];
