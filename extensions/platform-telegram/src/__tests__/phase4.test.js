...
describe('4.1 — markdownToTelegramHtml', () => {
    ...
    it('converts ```ts\\ncode\\n``` (with language) to <pre><code class="language-ts">code</code></pre>', () => {
        expect(markdownToTelegramHtml('```ts\ncode\n```')).toBe('<pre><code class="language-ts">code\n</code></pre>');
    });
...
describe('4.1 — HTML mode send()', () => {
    ...
    it('sends with parse_mode HTML by default', async () => {
        ...
        expect(mockApi.sendMessage).toHaveBeenCalledWith(100, '<b>hello</b>', expect.objectContaining({ parse_mode: 'HTML' }));
    });
    it('skips HTML translation when parseMode is plain', async () => {
        ...
        await adapter.start();
        await adapter.send('100', { text: '**hello**' });
        expect(mockApi.sendMessage).toHaveBeenCalledWith(100, '**hello**', expect.not.objectContaining({ parse_mode: expect.anything() }));
    });
...
describe('4.1 — HTML mode editMessage()', () => {
    ...
    it('edits with HTML translation', async () => {
        ...
        expect(mockApi.editMessageText).toHaveBeenCalledWith(100, 50, '<b>updated</b>', expect.objectContaining({ parse_mode: 'HTML' }));
    });
    it('edits without HTML when parseMode is plain', async () => {
        ...
        await adapter.send('100', { text: 'original' });
        await adapter.editMessage('100', '50', '**updated**');
        expect(mockApi.editMessageText).toHaveBeenCalledWith(100, 50, '**updated**', expect.objectContaining({}));
...
describe('4.2 — updateApprovalCard', () => {
    ...
    it('edits message to plain text with decision', async () => {
        ...
        expect(result.ok).toBe(true);
        expect(mockApi.editMessageText).toHaveBeenCalledWith(42, 100, expect.stringContaining('Approved'), expect.objectContaining({ reply_markup: { inline_keyboard: [] } }));
    });
