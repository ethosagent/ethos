...
import { TelegramClarifySurface } from '../clarify-surface';
const BOT_KEY = 'tg-key';
function makeFakeAdapter(initialMessageId = 'msg-1') {
    const sentInline = [];
    const sentForceReply = [];
    const edits = [];
    let cqHandler = null;
    ...
    return {
        ...
        async sendInlineKeyboard(chatId, text, rows) {
            ...
            return { ok: true, messageId: nextMessageId };
        },
...
function fakeCallback(overrides) {
    const answered = [];
    return {
        queryId: 'q1',
        chatId: 'chat-1',
...
describe('TelegramClarifySurface — correlateMessage (force-reply + /cancel)', () => {
    it('correlates a force-reply by replyToId + chatId', async () => {
        ...
        await store.add(row);
        const res = await surface.correlateMessage(inbound({ replyToId: 'prompt-77', text: 'free-form answer' }));
        expect(res).toEqual({
    ...
    it('handles `/cancel <requestId>` — only matches that row', async () => {
        ...
        expect(res).toBeNull();
        const res2 = await surface.correlateMessage(inbound({ chatId: 'chat-2', text: '/cancel req-b' }));
