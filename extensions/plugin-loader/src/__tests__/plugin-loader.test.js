...
import { DefaultHookRegistry, DefaultLLMProviderRegistry, DefaultMemoryProviderRegistry, DefaultPersonalityRegistry, DefaultToolRegistry, } from '@ethosagent/core';
...
describe('PluginLoader', () => {
    it('loads a plugin that registers a tool', async () => {
        ...
        await writePlugin(testDir, 'my-plugin', `
...
export async function activate(api) {
  api.registerTool({
    ...
    it('unloads a plugin and removes its tools', async () => {
        const registries = makeRegistries();
        const loader = new PluginLoader(registries);
        await writePlugin(testDir, 'unload-test', `
export async function activate(api) {
  api.registerTool({
    name: 'removable_tool',
...
export async function deactivate() {}
    ...
    it('loadFromNodeModules discovers scoped @ethos-plugins/* packages', async () => {
        ...
        await writeFile(join(flatDir, 'package.json'), JSON.stringify({ name: 'ethos-plugin-flat', main: 'index.ts', ethos: { type: 'plugin' } }));
        await writeFile(join(flatDir, 'index.ts'), `export async function activate(api) {
        api.registerTool({
          name: 'flat_tool',
          description: '',
          schema: { type: 'object', properties: {} },
        ...
        await writeFile(join(scopedDir, 'index.ts'), `export async function activate(api) {
        api.registerTool({
          name: 'scoped_tool',
          description: '',
          schema: { type: 'object', properties: {} },
    ...
    describe('credential methods', () => {
        it('setCredential throws for an unknown plugin id', async () => {
            ...
            await expect(loader.setCredential('nonexistent', 'API_KEY', 'secret')).rejects.toThrow('Plugin "nonexistent" is not loaded');
        });
        it('setCredential writes credential and meta files via PluginApiImpl', async () => {
            ...
            const metaRaw = await credStorage.read('/test-data/plugins/cred-test/credentials/API_KEY.meta');
        ...
        it('clearCredential removes credential and meta files', async () => {
            ...
            // Verify they exist
            expect(await credStorage.exists('/test-data/plugins/clear-test/credentials/SECRET')).toBe(true);
            expect(await credStorage.exists('/test-data/plugins/clear-test/credentials/SECRET.meta')).toBe(true);
            await loader.clearCredential('clear-test', 'SECRET');
            expect(await credStorage.exists('/test-data/plugins/clear-test/credentials/SECRET')).toBe(false);
            expect(await credStorage.exists('/test-data/plugins/clear-test/credentials/SECRET.meta')).toBe(false);
        });
        it('listCredentialKeys merges manifest declarations with storage state', async () => {
            ...
            await credStorage.write('/test-data/plugins/list-test/credentials/EXTRA_TOKEN', 'extra-value');
            await credStorage.write('/test-data/plugins/list-test/credentials/EXTRA_TOKEN.meta', JSON.stringify({ updatedAt: '2026-01-01T00:00:00.000Z' }));
    ...
    describe('Phase 30.6: contract major gate', () => {
        ...
        it('allows a plugin without pluginContractMajor (backward compat)', async () => {
            ...
            await mkdir(pluginDir, { recursive: true });
            await writeFile(join(pluginDir, 'package.json'), JSON.stringify({ name: 'no-major', version: '1.0.0', ethos: { type: 'plugin' } }));
            await writeFile(join(pluginDir, 'index.ts'), `export async function activate(api) {
          api.registerTool({
            name: 'compat_tool',
            description: '',
