...
import { createOpenClawApiShim, extractOpenClawRegister, isOpenClawPackageJson, } from '@ethosagent/openclaw-compat';
...
export class PluginLoader {
    ...
    async scanNodeModulesDir(nmDir, opts = {}) {
        ...
        if (opts.allowAll) {
            ...
            for (const entry of entries) {
                if (entry === 'node_modules' || entry === '.bin')
                    continue;
                if (entry.startsWith('@')) {
        ...
        for (const name of candidates) {
            ...
            try {
                ...
                const decision = canInstall(scanResult, tier);
                if (!decision.allowed) {
                    this.logger.warn(`[plugin-loader] "${name}" blocked by safety scan: ${decision.blockedBy}`, { component: 'plugin-loader', pluginId: name, blockedBy: decision.blockedBy });
                    continue;
                }
            ...
            for (const check of checks) {
                ...
                try {
                    result = await Promise.race([
                        check.run(),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Health check timed out after 5000ms')), 5000)),
                    ]);
                }
                catch (err) {
    ...
    async activatePlugin(id, mod) {
        ...
        if (Array.isArray(deps)) {
            for (const dep of deps) {
                if (typeof dep === 'string' && !this.plugins.has(dep)) {
                    this.logger.warn(`[plugin-loader] Plugin "${id}" requires "${dep}" but it is not loaded — skipping`, { component: 'plugin-loader', pluginId: id });
        ...
        if (!isPluginModule(mod)) {
            this.logger.warn(`[plugin-loader] "${id}" has no activate() or register() export — skipping`, { component: 'plugin-loader', pluginId: id });
            return;
        }
...
export function topologicalSort(manifests) {
    ...
    for (const m of manifests) {
        graph.set(m.id, (m.dependencies ?? []).filter((d) => ids.has(d)));
    }
    ...
    for (const entry of entries) {
        ...
        if (entry.isDir) {
            if (entry.name === 'node_modules')
                continue; // skip dep trees
            await collectFindings(storage, fullPath, permissions, out);
        }
        else if (/\.[jt]sx?$|\.(?:cjs|mjs)$/.test(entry.name) &&
...
export async function scanInstalledPlugins(opts) {
    ...
    if (opts.workingDir) {
        out.push(...(await scanManifestsIn(storage, join(opts.workingDir, '.ethos', 'plugins'), 'project')));
    }
...
async function scanNodeModules(storage, nmDir, source) {
    ...
    for (const entry of entries) {
        ...
        if (entry.name.startsWith('.'))
            continue; // skip .package-lock.json etc.
        if (entry.name.startsWith('@')) {
...
async function readManifest(storage, pluginDir) {
    ...
    if (!parsed.ethos && !isOpenClawPackageJson(parsed))
        return null;
    return parsed;
}
...
async function resolveEntry(storage, dir) {
    ...
    try {
        ...
        if (main) {
            ...
            if (!isContainedIn(candidate, dir))
                return null;
            if (await storage.exists(candidate))
                return candidate;
        }
