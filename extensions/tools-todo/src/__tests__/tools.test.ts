...
describe('todo tools', () => {
  ...
  it('todo_set + todo_list round-trip yields the ids that were assigned', async () => {
    ...
    const set = await call<{ count: number; ids: string[] }>(
      todo_set,
      {
        todos: [
    ...
    const list = await call<Array<{ id: string; status: string }>>(todo_list, {}, ctx);
  ...
  it('todo_list default filter excludes completed', async () => {
    ...
    const visible = await call<Array<{ id: string }>>(todo_list, {}, ctx);
    expect(visible.map((i) => i.id)).toEqual(['t1']);

    const allOfThem = await call<Array<{ id: string }>>(todo_list, { filter: 'all' }, ctx);
  ...
  it('todo_add with notes > 500 chars surfaces notes_truncated:true in the result', async () => {
    ...
    const result = await call<{ id: string; notes_truncated?: true }>(
      todo_add,
  ...
  it('todo_clear empties to count and resets the id counter', async () => {
    ...
    const cleared = await call<{ cleared: number }>(todo_clear, {}, ctx);
    expect(cleared).toEqual({ cleared: 2 });

    const added = await call<{ id: string }>(todo_add, { content: 'C', activeForm: 'C' }, ctx);
  ...
  it('two sessions stay isolated through the tool surface', async () => {
    ...
    const fromA = await call<Array<{ content: string }>>(todo_list, {}, makeCtx('a'));
    const fromB = await call<Array<{ content: string }>>(todo_list, {}, makeCtx('b'));
