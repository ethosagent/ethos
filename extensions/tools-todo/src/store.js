...
function clipNotes(notes) {
    ...
    if (notes.length <= NOTES_MAX_CHARS)
        return { value: notes, truncated: false };
    return {
...
export class InMemoryTodoStore {
    ...
    update(sessionKey, id, patch) {
        return this.runSerial(sessionKey, () => {
            ...
            if (patch.status === 'in_progress' && item.status !== 'in_progress') {
                ...
                if (existing)
                    throw new MultipleInProgressError(existing.id);
            }
            const ts = this.now();
            ...
            if (patch.activeForm !== undefined)
                item.activeForm = patch.activeForm;
            let truncated = false;
    ...
    runSerial(sessionKey, fn) {
        ...
        this.locks.set(sessionKey, next.then(() => undefined, () => undefined));
        return next;
    }
}
function resolvePosition(position, len) {
    ...
    if (Number.isFinite(position))
        return Math.max(0, Math.min(len, Math.floor(position)));
    return len;
}
