...
export function parseRewriteResponse(raw) {
    ...
    if (trimmed === 'NO_REWRITE')
        return { kind: 'skip', reason: 'NO_REWRITE' };
    const match = trimmed.match(/<skill>([\s\S]*?)<\/skill>/);
    if (!match)
        return { kind: 'skip', reason: 'malformed-output' };
    const content = (match[1] ?? '').trim();
    if (!content)
        return { kind: 'skip', reason: 'empty-skill' };
    return { kind: 'rewrite', content };
}
export function parseNewSkillResponse(raw) {
    const trimmed = raw.trim();
    if (trimmed === 'NO_PATTERN')
        return { kind: 'skip', reason: 'NO_PATTERN' };
    ...
    if (!isSafeKebabFilename(fileName))
        return { kind: 'skip', reason: 'invalid-filename' };
    if (!content)
        return { kind: 'skip', reason: 'empty-skill' };
