...
describe('parseNewSkillResponse', () => {
    it('extracts filename and content', () => {
        const r = parseNewSkillResponse('<filename>map-over-loops.md</filename>\n<skill>\nPrefer map.\n</skill>');
    ...
    it('rejects unsafe filenames', () => {
        ...
        expect(r.kind).toBe('skip');
        if (r.kind === 'skip')
            expect(r.reason).toBe('invalid-filename');
    });
    ...
    it('honors NO_PATTERN', () => {
        ...
        expect(r.kind).toBe('skip');
        if (r.kind === 'skip')
            expect(r.reason).toBe('NO_PATTERN');
    });
});
