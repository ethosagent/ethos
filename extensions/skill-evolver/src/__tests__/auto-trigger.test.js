...
beforeEach(async () => {
    ...
    dataDir = join(tmpdir(), `ethos-evolver-auto-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await mkdir(dataDir, { recursive: true });
});
afterEach(async () => {
    await rm(dataDir, { recursive: true, force: true });
});
async function listAutoCandidates(personalityId) {
...
describe('SkillEvolutionAutoTrigger (E3)', () => {
    ...
    it('candidate is personality-scoped — each personality gets its own subdir', async () => {
        ...
        const personalities = {
            define: () => { },
            get: (id) => (id === 'coach' ? coach : engineer),
            list: () => [engineer, coach],
            getDefault: () => engineer,
            setDefault: () => { },
            loadFromDirectory: async () => { },
    ...
    it('candidate frontmatter includes auto_proposed provenance', async () => {
        ...
        const body = await readFile(join(dataDir, 'skills', '.pending', 'engineer', files[0] ?? ''), 'utf-8');
