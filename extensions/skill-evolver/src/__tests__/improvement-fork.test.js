...
// Shared helpers
// ---------------------------------------------------------------------------
function createMockSessionStore(messages) {
...
describe('buildForkContext', () => {
    ...
    it('includes tool summary', async () => {
        ...
        const result = await buildForkContext({ sessionId: 'test-session', toolNames: ['read_file', 'write_file'] }, store);
        expect(result).toContain('Tools used: read_file, write_file');
    });
    it('includes skill summary', async () => {
        const store = createMockSessionStore([{ role: 'user', content: 'Hello' }]);
        const result = await buildForkContext({ sessionId: 'test-session', activeSkillFiles: ['coding.md'] }, store);
...
describe('ImprovementFork', () => {
    ...
    it('respects cooldown — second fire within window does not fork', async () => {
        ...
        mockLLM.complete = async function* (...args) {
            calls.push(now);
