...
import { analyzeEvalOutput, DEFAULT_EVOLVE_CONFIG, loadEvolveConfig, parseEvalJsonl, } from '../analyze';
let testDir;
beforeEach(async () => {
    testDir = join(tmpdir(), `ethos-evolve-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('parseEvalJsonl', () => {
    it('parses well-formed records', () => {
        const src = recs({ task_id: 't1', role: 'user', content: 'q1' }, { task_id: 't1', role: 'assistant', content: 'a1', score: 0.9 });
...
describe('analyzeEvalOutput', () => {
    it('aggregates per-skill stats from skill_files_used', async () => {
        const src = recs({ task_id: 't1', role: 'user', content: 'q1' }, {
            task_id: 't1',
            role: 'assistant',
            content: 'a1',
            score: 1,
            skill_files_used: ['a.md', 'b.md'],
        }, { task_id: 't2', role: 'user', content: 'q2' }, { task_id: 't2', role: 'assistant', content: 'a2', score: 0, skill_files_used: ['a.md'] });
    ...
    it('skips errored tasks', async () => {
        const src = recs({ task_id: 't1', role: 'user', content: 'q' }, {
            task_id: 't1',
            role: 'assistant',
            content: '',
            score: 0,
    ...
    it('flags rewrite candidate only when runs >= minRunsBeforeEvolve and avg < threshold', async () => {
        ...
        const plan = await analyzeEvalOutput(parseEvalJsonl(lines.join('\n')), testDir, DEFAULT_EVOLVE_CONFIG);
