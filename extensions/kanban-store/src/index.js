...
export { autonomyTier, tierMaxRetries, } from './autonomy-tier';
// ---------------------------------------------------------------------------
// Schema
// ---------------------------------------------------------------------------
const SCHEMA = `
...
export class KanbanStore {
    ...
    constructor(dbPath, opts = {}) {
        ...
        const versionRows = this.db.pragma('user_version');
        const currentVersion = versionRows[0]?.user_version ?? 0;
        if (currentVersion > 4) {
            throw new Error(`kanban-store: database user_version=${currentVersion} is newer than code (4); refusing to open to avoid downgrade`);
        }
    ...
    migrateV1ToV2() {
        ...
        `);
                ...
                if (violations.length > 0) {
                    throw new Error(`kanban-store: v1→v2 migration left ${violations.length} foreign-key violation(s)`);
                }
    ...
    migrateV2ToV3() {
        ...
        `);
                const violations = this.db.pragma('foreign_key_check');
                if (violations.length > 0) {
                    throw new Error(`kanban-store: v2→v3 migration left ${violations.length} foreign-key violation(s)`);
                }
                // Bump inside the transaction so version and schema move together: a
    ...
    createTask(input) {
        ...
        const workspaceMode = input.workspaceMode ?? 'scratch';
        const scheduledFor = input.scheduledFor ?? null;
        const status = scheduledFor !== null ? 'scheduled' : 'todo';
        ...
        const insertTask = this.db.prepare(`INSERT INTO tasks
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
        const tx = this.db.transaction(() => {
            ...
            try {
                insertTask.run(id, input.title, body, assignee, status, priority, workspaceMode, null, scheduledFor, idempotencyKey, null, maxRetries, 0, acceptanceCriteria, now, now);
            }
            catch (err) {
        ...
        if (!task)
            throw new Error(`createTask: inserted task ${finalId} not found`);
        return task;
    }
    getTask(id) {
    ...
    updateStatus(taskId, status, reason, actor = 'system') {
        const now = Date.now();
        const tx = this.db.transaction(() => {
            const row = this.db
                .prepare('SELECT status, assignee, current_run_id, max_retries, retry_count FROM tasks WHERE id = ?')
                .get(taskId);
            if (!row)
                throw new Error(`updateStatus: task ${taskId} not found`);
            ...
            if (status === 'running' && oldRunId === null) {
                ...
                if (budgetExhausted) {
                    ...
                    this.db
                        .prepare(`INSERT INTO task_runs (id, task_id, started_at, last_heartbeat_at)
               VALUES (?, ?, ?, ?)`)
                        .run(runId, taskId, now, now);
                    this.db
                        .prepare('UPDATE tasks SET status = ?, current_run_id = ?, retry_count = ?, updated_at = ? WHERE id = ?')
                ...
                const result = this.db
                    .prepare(`UPDATE task_runs SET ended_at = ?, outcome = ?
            ...
            if (runStarted)
                this.emit(taskId, 'run_started', actor, {});
            if (runCancelled) {
    ...
    completeRun(taskId, summary, actor = 'system') {
        return this.endRun(taskId, 'done', 'completed', summary, actor);
    }
    blockRun(taskId, reason, actor = 'system') {
    ...
    endRun(taskId, newStatus, outcome, summary, actor, opts = {}) {
        const now = Date.now();
        ...
        const tx = this.db.transaction(() => {
            const row = this.db
                ...
                .get(taskId);
            if (!row)
                throw new Error(`endRun: task ${taskId} not found`);
            if (row.current_run_id === null) {
                throw new Error(`no open run: task ${taskId} has no current run to end`);
            }
            const result = this.db
                .prepare(`UPDATE task_runs SET ended_at = ?, outcome = ?, summary = ?
           WHERE id = ? AND ended_at IS NULL`)
    ...
    heartbeatRun(taskId, note, actor = 'system') {
        const now = Date.now();
        const tx = this.db.transaction(() => {
            const row = this.db.prepare('SELECT current_run_id FROM tasks WHERE id = ?').get(taskId);
            if (!row)
                throw new Error(`heartbeatRun: task ${taskId} not found`);
            if (row.current_run_id === null) {
    ...
    assign(taskId, assignee, actor = 'system') {
        const now = Date.now();
        const tx = this.db.transaction(() => {
            const result = this.db
    ...
    link(parentId, childId, actor = 'system') {
        if (parentId === childId) {
        ...
        const tx = this.db.transaction(() => {
            const ancestor = this.db
                .prepare(`WITH RECURSIVE ancestors(id) AS (
           ...
           SELECT 1 AS hit FROM ancestors WHERE id = ? LIMIT 1`)
            ...
            try {
                ...
                if (/FOREIGN KEY/i.test(msg)) {
                    throw new Error(`not found: parent or child task does not exist (${parentId} -> ${childId})`);
                }
     ...
     */
    promoteScheduled(nowMs = Date.now(), actor = 'system') {
        const candidates = this.db
            .prepare(`SELECT id FROM tasks WHERE status = 'scheduled' AND scheduled_for IS NOT NULL
         AND scheduled_for <= ?`)
            .all(nowMs);
        for (const c of candidates) {
     ...
     * `blockRun(id, 'stalled')`.
     */
    findStalledRuns(cutoffMs, nowMs = Date.now()) {
        const threshold = nowMs - cutoffMs;
        const rows = this.db
            .prepare(`SELECT * FROM task_runs WHERE ended_at IS NULL AND last_heartbeat_at < ?
     ...
     */
    findStaleRunningTasks(thresholdMs, nowMs = Date.now()) {
        const threshold = nowMs - thresholdMs;
        const rows = this.db
            .prepare(`SELECT * FROM tasks WHERE status = 'running' AND updated_at < ?
     ...
     */
    reclaimTask(taskId, reason, actor = 'system') {
        const now = Date.now();
        const tx = this.db.transaction(() => {
            const row = this.db
                .prepare('SELECT status, assignee, current_run_id FROM tasks WHERE id = ?')
                .get(taskId);
            if (!row)
                throw new Error(`reclaimTask: task ${taskId} not found`);
            let runCancelled = false;
            if (row.current_run_id !== null) {
                const result = this.db
                    .prepare(`UPDATE task_runs SET ended_at = ?, outcome = ?
     ...
     */
    rollupCompletedGoals(actor = 'system') {
        const candidates = this.db
            .prepare(`SELECT g.id FROM tasks g
     ...
     */
    adoptOrphanTickets(coordinator, opts = {}) {
    ...
    emit(taskId, kind, actor, data) {
        this.db
            .prepare('INSERT INTO task_events (task_id, kind, actor, data_json, created_at) VALUES (?,?,?,?,?)')
     ...
     */
    bumpMemberStat(memberId, column) {
        if (this.teamId === null || memberId === null)
            return;
        this.db
            .prepare(`INSERT INTO team_member_stats (team_id, member_id, ${column}, last_updated_at)
         VALUES (?, ?, 1, ?)
    ...
    getMemberStats() {
        ...
        if (this.teamId === null)
            return out;
        const rows = this.db
            .prepare(`SELECT team_id, member_id, tickets_completed, tickets_failed, tickets_orphaned,
                last_updated_at
