...
describe('KanbanStore', () => {
  ...
  it('findStaleRunningTasks returns running tasks whose updated_at is older than the threshold', () => {
    ...
    store.db.prepare('UPDATE tasks SET updated_at = ? WHERE id = ?').run(now - 200, stale.id);
  ...
  it('findStaleRunningTasks ignores non-running tasks even if updated_at is old', () => {
    ...
    const now = Date.now();
    store.db.prepare('UPDATE tasks SET updated_at = ? WHERE id = ?').run(now - 10_000, ready.id);
  ...
  it('rolls the v1->v2 migration back atomically when it fails — no half-migrated state', () => {
    ...
    try {
      ...
      try {
        const version = after.pragma('user_version')[0]?.user_version;
        ...
        const taskCount = after.prepare('SELECT COUNT(*) AS n FROM tasks').get().n;
  ...
  it('migrates a v1 database through the stepwise v1->v2->v3 chain (columns + statuses)', () => {
    ...
    try {
      ...
      try {
        const version = raw.pragma('user_version')[0]?.user_version;
