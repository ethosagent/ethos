...
describe('KanbanStore', () => {
    ...
    it('searchFts finds tasks by FTS5 query over title or body', () => {
        ...
        expect(hits.map((t) => t.id)).not.toContain(store.listTasks().find((t) => t.title === 'rotate keys')?.id);
    });
      ...
      `);
            ...
            try {
                ...
                expect(store
                    .listTasks({ q: 'legacy' })
                    .map((t) => t.id)
                    .sort()).toEqual(['t_parent', 't_seed'].sort());
            }
            finally {
                store.close();
            }
    ...
    it('rolls the v1->v2 migration back atomically when it fails — no half-migrated state', () => {
      ...
      `);
            ...
            try {
                const version = after.pragma('user_version')[0]
                    ?.user_version;
                expect(version).toBe(1);
                const cols = after.pragma('table_info(tasks)').map((c) => c.name);
                expect(cols).not.toContain('max_retries');
                expect(cols).not.toContain('retry_count');
                const taskCount = after.prepare('SELECT COUNT(*) AS n FROM tasks').get()
                    .n;
    ...
    it('migrates a v2 database to v3: data survives, acceptance_criteria appears, needs_revision becomes insertable', () => {
        ...
        try {
            ...
            try {
                ...
                expect(store.updateStatus(t.id, 'needs_revision', 'missing tests').status).toBe('needs_revision');
                // FTS triggers were rebuilt — search over title still works.
                expect(store
                    .listTasks({ q: 'legacy' })
                    .map((t) => t.id)
    ...
    it('migrates a v1 database through the stepwise v1->v2->v3 chain (columns + statuses)', () => {
      ...
      `);
            ...
            try {
                const version = raw.pragma('user_version')[0]
                    ?.user_version;
    ...
    it('a fresh database opens at user_version 4 with the team_member_stats table', () => {
        ...
        try {
            ...
            try {
                ...
                expect(version).toBe(4);
                const tables = raw
                    .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='team_member_stats'")
                    .all().map((t) => t.name);
            ...
            const snapshot = (path) => {
                ...
                try {
                 ...
                 ORDER BY type, name`)
                    ...
                    const tasksColumns = db.pragma('table_info(tasks)').map((c) => ({
                        name: c.name,
                        type: c.type,
                        notnull: c.notnull,
                    ...
                    const userVersion = db.pragma('user_version')[0]
                        ?.user_version;
