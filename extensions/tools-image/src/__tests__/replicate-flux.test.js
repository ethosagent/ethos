...
describe('ReplicateFluxProvider', () => {
    ...
    it('throws when prediction status is failed', async () => {
        ...
        await expect(provider.generate({ prompt: 'bad', size: '1024x1024', quality: 'standard' })).rejects.toThrow('NSFW content detected');
    });
    // -------------------------------------------------------------------------
    ...
    it('throws on initial API error', async () => {
        ...
        await expect(provider.generate({ prompt: 'test', size: '1024x1024', quality: 'standard' })).rejects.toThrow('429');
    });
    // -------------------------------------------------------------------------
    // Timeout
    // -------------------------------------------------------------------------
    it('throws on timeout when poll never succeeds', async () => {
        ...
        await expect(provider.generate({ prompt: 'timeout', size: '1024x1024', quality: 'standard' })).rejects.toThrow('timed out');
