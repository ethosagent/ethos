...
describe('OpenAIDalleProvider', () => {
    ...
    describe('generate() errors', () => {
        it('throws on content policy rejection', async () => {
            ...
            await expect(provider.generate({ prompt: 'bad', size: '1024x1024', quality: 'standard' })).rejects.toThrow('content policy');
        });
        it('throws on rate limit', async () => {
            mockGenerate.mockRejectedValue(new Error('rate limit exceeded (429)'));
            await expect(provider.generate({ prompt: 'test', size: '1024x1024', quality: 'standard' })).rejects.toThrow('rate limit');
        });
        it('throws on server error', async () => {
            mockGenerate.mockRejectedValue(new Error('server error 500'));
            await expect(provider.generate({ prompt: 'test', size: '1024x1024', quality: 'standard' })).rejects.toThrow('server error');
        });
        it('throws when no image data returned', async () => {
            mockGenerate.mockResolvedValue({ data: [{}] });
            await expect(provider.generate({ prompt: 'test', size: '1024x1024', quality: 'standard' })).rejects.toThrow('no image data');
        });
    });
});
