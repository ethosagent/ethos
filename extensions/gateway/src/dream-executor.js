...
// ---------------------------------------------------------------------------
const DEFAULT_DREAM_PROMPT = 'Review recent session history. Consolidate key facts into MEMORY.md. ' +
...
export class DreamExecutor {
    ...
    async tick() {
        ...
        try {
            for (const [personalityId, lastTurn] of this.lastUserTurnAt) {
                ...
                if (!config?.dreaming?.enable)
                    continue;
                if (this.inFlight.has(personalityId))
                    continue;
                const idleMs = (config.dreaming.idleMinutes ?? 60) * 60_000;
                if (Date.now() - lastTurn < idleMs)
                    continue;
                // Check eligibility
    ...
    async checkEligibility(personalityId, maxPerDay) {
        ...
        try {
            ...
            const now = Date.now();
            if (now - windowStart > 24 * 60 * 60_000)
                return true;
    ...
    async persistState(personalityId) {
        ...
        await this.storage.writeAtomic(path, JSON.stringify({ lastDreamAt: new Date().toISOString(), runsToday, windowStart }, null, 2));
    }
}
