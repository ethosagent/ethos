...
async function seedStorage(storage, personalityId = 'test-personality') {
...
describe('DreamExecutor', () => {
    ...
    function build(config) {
        ...
        executor = new DreamExecutor(storage, () => loop, () => cfg);
        return executor;
    }
    // -----------------------------------------------------------------------
    // Eligibility logic
    ...
    describe('eligibility', () => {
        ...
        it('within window, under cap → eligible', async () => {
            ...
            await storage.writeAtomic(statePath, JSON.stringify({ lastDreamAt: windowStart, runsToday: 0, windowStart }));
            build();
            executor.recordUserTurn(personalityId);
            vi.setSystemTime(Date.now() + 61 * 60_000);
            await internals(executor).tick();
        ...
        it('within window, at cap → not eligible', async () => {
            const windowStart = new Date().toISOString();
            await storage.writeAtomic(statePath, JSON.stringify({ lastDreamAt: windowStart, runsToday: 2, windowStart }));
        ...
        it('window expired → eligible (reset)', async () => {
            ...
            await storage.writeAtomic(statePath, JSON.stringify({ lastDreamAt: oldWindow, runsToday: 5, windowStart: oldWindow }));
            build();
            executor.recordUserTurn(personalityId);
            vi.setSystemTime(Date.now() + 61 * 60_000);
            await internals(executor).tick();
    ...
    describe('idle detection', () => {
        ...
        it('no dreaming config → no dream', async () => {
            const noConfig = { id: personalityId, name: 'Test' };
            build(noConfig);
            executor.recordUserTurn(personalityId);
            vi.setSystemTime(Date.now() + 120 * 60_000);
            await internals(executor).tick();
    ...
    describe('model routing', () => {
        it('session key starts with dream:<personalityId>:', async () => {
            ...
            await internals(executor).tick();
            expect(loop.runCalls).toHaveLength(1);
            const opts = loop.runCalls[0].opts;
