...
export class Dispatcher {
    ...
    async tick() {
        ...
        const staleIds = new Set(this.board.findStaleRunningTasks(this.stalenessThresholdMs).map((t) => t.id));
        ...
        const memberStats = this.trustPolicy?.mode === 'tiered' ? this.board.getMemberStats() : undefined;
            ...
            if (task.maxRetries === null && this.trustPolicy?.mode === 'tiered' && memberStats) {
                ...
                if (stats) {
                    ...
                    if (task.retryCount > budget) {
                        try {
                            this.board.updateStatus(task.id, 'failed', 'tier_retry_budget_exhausted', 'dispatcher');
                        }
                        catch {
    ...
    orderForDispatch(ready) {
        if (!this.preferReliable || ready.length < 2)
            return ready;
        ...
        const reliabilityOf = (assignee) => {
            if (assignee === null)
                return { ratio: 1, hasRecord: false };
        ...
        return [...ready].sort((a, b) => {
            ...
            if (ra.hasRecord !== rb.hasRecord)
                return ra.hasRecord ? -1 : 1;
            return 0;
        });
    }
