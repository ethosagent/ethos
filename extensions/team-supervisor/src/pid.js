...
export function acquirePidFile(pidPath, opts = {}) {
    ...
    if (!tryCreate()) {
        ...
        if (existingPid !== null && isProcessAlive(existingPid)) {
            throw new Error(`Team already running (PID ${existingPid}). ` +
                "Use 'ethos team status <name>' for details.");
        }
        // Stale PID file from a previous crash — clean up and take the lock.
        logger.warn(`[team-supervisor] Cleaning up stale PID file from previous crash (PID ${existingPid ?? 'unknown'})`, { component: 'team-supervisor', staleEntryPid: existingPid ?? 'unknown' });
        try {
            unlinkSync(pidPath);
        }
        catch {
