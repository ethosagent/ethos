...
export function buildMemberLaunchArgs(entryPoint, port, personality, meshName, modelOverride, teamName, role) {
    const base = ['serve', '--port', String(port), '--personality', personality, '--mesh', meshName];
    if (modelOverride)
        base.push('--model', modelOverride);
 ...
 */
export async function runSupervisor(manifest, manifestPath, opts = {}) {
    const log0 = opts.logger ?? noopLogger;
    const name = manifest.name;
    if (!isSafePathSegment(name)) {
        throw new Error(`Invalid team name "${name}": must not contain path separators, "..", or start with "."`);
    }
    for (const member of manifest.members) {
        if (!isSafePathSegment(member.personality)) {
            throw new Error(`Invalid personality "${member.personality}": must not contain path separators, "..", or start with "."`);
        }
    }
    ...
    function log(personality, event, data) {
    ...
    function spawnMember(personality) {
        ...
        const childArgs = buildMemberLaunchArgs(entryPoint, m.port, personality, meshName, modelOverride, name, role);
        ...
        child.on('exit', (code, signal) => {
            ...
            if (!autoRestart) {
                m.status = 'failed';
                log0.error(`[team-supervisor] ${personality} exited (code=${code}, signal=${signal}); auto_restart disabled`, { component: 'team-supervisor', team: name, personality, code, signal });
                persist();
                return;
            }
            ...
            if (m.recentFailures.length >= MAX_FAILURES_PER_MINUTE) {
                ...
                log0.error(`[team-supervisor] ${personality}: giving up — ${MAX_FAILURES_PER_MINUTE} failures in ${FAILURE_WINDOW_MS / 1000}s`, {
                    component: 'team-supervisor',
                    team: name,
                    personality,
                    failureCount: m.failureCount,
            ...
            const backoff = Math.min(BACKOFF_BASE_MS * 2 ** (m.recentFailures.length - 1), BACKOFF_CAP_MS);
            m.status = 'restarting';
    ...
    stopProbeLoop = startHealthProbeLoop({
        getMembers: () => [...memberMap.values()].map(({ personality, port, status, pid }) => ({
            personality,
            port,
            status,
            pid,
