...
const TeamManifestSchema = z
    ...
    .superRefine((val, ctx) => {
    const mode = val.dispatch_mode ?? (val.coordinator !== undefined ? 'coordinator' : 'self-routing');
...
function firstIssueMessage(err) {
 ...
 */
export function parseTeamManifest(yamlContent, opts = {}) {
    ...
    const effectiveMode = manifest.dispatch_mode ?? (manifest.coordinator !== undefined ? 'coordinator' : 'self-routing');
    if (effectiveMode === 'self-routing' && manifest.coordinator !== undefined) {
        ...
        logger.warn(`[team-supervisor] team.yaml: \`coordinator\` field is set but dispatch_mode is "self-routing" — the coordinator field will be ignored`, { component: 'team-supervisor', team: manifest.name });
    }
    return manifest;
}
/**
 ...
 */
export function validateForStart(manifest, opts = {}) {
    const logger = opts.logger ?? noopLogger;
    ...
    if (manifest.personality_models) {
        ...
        for (const key of Object.keys(manifest.personality_models)) {
            if (!knownPersonalities.has(key)) {
                logger.warn(`[team] Warning: personality_models key "${key}" does not match any team member personality. ` +
                    `Known personalities: ${[...knownPersonalities].join(', ') || '(none)'}`, { component: 'team-supervisor', team: manifest.name, key });
            }
        }
    }
}
