...
`;
        ...
        expect(warn).toHaveBeenCalledWith(expect.stringContaining('personality_models'), expect.any(Object));
    });
...
describe('buildMemberLaunchArgs', () => {
    ...
    it('includes --model when modelOverride is provided', async () => {
        ...
        const args = buildMemberLaunchArgs('/usr/bin/ethos', 3001, 'engineer', 'alpha', 'claude-haiku-4-5');
    ...
    it('passes --team and --role when both are provided (Plan B member spawn)', async () => {
        const { buildMemberLaunchArgs } = await import('../supervisor');
        const args = buildMemberLaunchArgs('/usr/bin/ethos', 3001, 'engineer', 'alpha', undefined, 'analytics', 'member');
