...
const MEMBERS = ['engineer', 'researcher', 'writer'];
...
describe('cross-phase durability — 100 tickets, failures, crashes, stale claims', () => {
    ...
    it('drives a full cycle and holds all five plan invariants together', async () => {
        ...
        const taskIdFromPrompt = (prompt) => {
            const m = prompt.match(/Task id: `([^`]+)`/);
            if (m === null)
                throw new Error(`dispatch stub: no task id in prompt`);
            return m[1];
        };
        const makeDispatch = () => vi.fn(async ({ prompt }) => {
        ...
        for (const member of MEMBERS) {
            ...
            const expectedCompleted = memberSeeded.filter((s) => s.fate === 'ok' || s.fate === 'slow').length;
