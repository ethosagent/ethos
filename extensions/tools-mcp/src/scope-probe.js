...
export async function probeTokenScopes(server, introspectionEndpoint, declaredScopes, bearerToken, logger) {
    const base = {
        server,
        declaredScopes,
    };
    try {
        ...
        logger.warn(`[ethos] Scope probe: mismatch for '${server}' — missing: [${missing.join(', ')}], extra: [${extra.join(', ')}]`, { component: 'tools-mcp', server, missing, extra });
