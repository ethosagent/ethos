...
export class OAuthDiscoveryError extends Error {
    attemptedUrls;
    constructor(attemptedUrls, message) {
...
export class ConfidentialClientUnsupported extends Error {
    constructor(serverUrl) {
        super(`MCP server at ${serverUrl} returned a client_secret (confidential client).` +
...
export class DcrUnsupported extends Error {
    mcpUrl;
    constructor(mcpUrl) {
        super(`MCP server at ${mcpUrl} does not advertise a registration_endpoint.` +
...
export class MissingToken extends Error {
    ...
    constructor(serverName) {
        super(`No OAuth token available for MCP server '${serverName}'.` +
...
// ---------------------------------------------------------------------------
function accessTokenRef(serverName) {
    return `mcp/${serverName}/access_token`;
}
function refreshTokenRef(serverName) {
    return `mcp/${serverName}/refresh_token`;
}
function expiresAtRef(serverName) {
    return `mcp/${serverName}/expires_at`;
}
// ---------------------------------------------------------------------------
// Token storage
// ---------------------------------------------------------------------------
export async function storeTokens(serverName, tokens, secrets) {
...
export async function loadAccessToken(serverName, secrets) {
    return secrets.get(accessTokenRef(serverName));
}
export async function deleteTokens(serverName, secrets) {
...
// ---------------------------------------------------------------------------
export async function isTokenExpired(serverName, secrets, bufferMs = 60_000) {
...
export async function exchangeCode(config, code, redirectUri, codeVerifier) {
    ...
    if (!fetchResult.ok)
        throw new Error(`Token exchange blocked: ${fetchResult.reason}`);
...
// Refresh flow
// ---------------------------------------------------------------------------
export async function refreshToken(serverName, config, secrets) {
    // SSRF gate: validate token endpoint before sending credentials
    validateSsrfUrl(config.token_endpoint);
    ...
    if (data.expires_in) {
        await secrets.set(expiresAtRef(serverName), new Date(Date.now() + data.expires_in * 1000).toISOString());
    }
...
// Revocation
// ---------------------------------------------------------------------------
export async function revokeToken(serverName, config, secrets) {
...
// ---------------------------------------------------------------------------
function buildWellKnownUrl(baseUrl, suffix) {
    const parsed = new URL(baseUrl);
    const path = parsed.pathname === '/' ? '' : parsed.pathname;
    return `${parsed.origin}/.well-known/${suffix}${path}`;
}
export async function discoverOAuthMetadata(mcpUrl) {
    const attemptedUrls = [];
    ...
    try {
        ...
        const data = (await asResp.json());
        if (!data.authorization_endpoint || !data.token_endpoint) {
            throw new OAuthDiscoveryError(attemptedUrls, 'OAuth metadata missing required authorization_endpoint or token_endpoint');
        }
        if (data.code_challenge_methods_supported &&
            !data.code_challenge_methods_supported.includes('S256')) {
            throw new OAuthDiscoveryError(attemptedUrls, 'OAuth server does not support S256 code challenge method');
        }
        ...
        for (const ep of endpointsToCheck) {
            try {
                ...
                if (epUrl.protocol !== 'https:') {
                    throw new OAuthDiscoveryError(attemptedUrls, `Discovered endpoint ${ep} uses insecure protocol ${epUrl.protocol}`);
                }
            }
            catch (err) {
...
// ---------------------------------------------------------------------------
export async function registerOAuthClient(registrationEndpoint, request) {
...
export async function runDcrAuthorization(mcpUrl, clientName, onAuthUrl) {
...
// ---------------------------------------------------------------------------
export async function runPkceLogin(serverName, config, secrets) {
    const verifier = generateCodeVerifier();
    const challenge = generateCodeChallenge(verifier);
    const state = randomBytes(16).toString('base64url');
    const callback = await startCallbackServer();
...
// ---------------------------------------------------------------------------
export async function ensureValidToken(serverName, config, secrets, context = 'cli') {
    const token = await secrets.get(accessTokenRef(serverName));
    if (!token) {
        if (context === 'ui') {
