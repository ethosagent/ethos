...
describe('probeTokenScopes', () => {
    ...
    it('returns match when actual scopes equal declared scopes', async () => {
        ...
        const result = await probeTokenScopes('test-server', 'http://auth/introspect', ['read', 'write'], 'tok_123', mockLogger());
    ...
    it('returns mismatch when scopes differ', async () => {
        ...
        const result = await probeTokenScopes('test-server', 'http://auth/introspect', ['read', 'write'], 'tok_123', logger);
    ...
    it('returns inactive when token is not active', async () => {
        ...
        const result = await probeTokenScopes('test-server', 'http://auth/introspect', ['read'], 'tok_123', mockLogger());
