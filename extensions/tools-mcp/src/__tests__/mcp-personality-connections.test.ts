...
async function spawnFreshEchoServer(toolName: string) {
...
class PersonalityTestManager extends McpManager {
  ...
  // to this subclass.
  private _transports?: Map<
    string,
    () => Promise<{ clientTransport: InstanceType<typeof InMemoryTransport> }>
  >;
  private _secretsLog?: Array<{ configName: string; secrets: SecretsResolver | undefined }>;
  private _clientLog?: Map<string, McpClient>;
  private _connectCount = 0;

  private get transports(): Map<
    string,
    () => Promise<{ clientTransport: InstanceType<typeof InMemoryTransport> }>
  > {
  ...
  get secretsLog(): Array<{ configName: string; secrets: SecretsResolver | undefined }> {
    if (!this._secretsLog) this._secretsLog = [];
    return this._secretsLog;
  }

  get clientLog(): Map<string, McpClient> {
  ...
  setTransportFactory(
    name: string,
    factory: () => Promise<{ clientTransport: InstanceType<typeof InMemoryTransport> }>,
  ): void {
  ...
  protected override _buildClientWithSecrets(
  ...
  it('existing connect() + getTools() still works for single-personality path', async () => {
    ...
    class BootManager extends McpManager {
      protected override _buildClient(config: McpServerConfig): McpClient {
        const real = super._buildClient(config);
        // biome-ignore lint/suspicious/noExplicitAny: test seam
        (real as any)._createTransport = async () => clientTransport;
        return real;
      }
    }
