...
describe('streamable-http transport', () => {
    ...
    it('throws when streamable-http config lacks url', async () => {
        ...
        await expect(client.createTransportPublic()).rejects.toThrow("streamable-http transport requires 'url'");
    });
});
// ---------------------------------------------------------------------------
...
describe('SSE deprecation warning', () => {
    it('emits deprecation warning when SSE transport is used', async () => {
        ...
        const transport = await client.createTransportPublic();
        expect(transport).toBeDefined();
        expect(warnFn).toHaveBeenCalledWith(expect.stringContaining('SSE transport is deprecated'), expect.objectContaining({ component: 'tools-mcp', server: 'legacy-sse' }));
    });
