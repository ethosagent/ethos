...
// Helpers
// ---------------------------------------------------------------------------
async function createTestServer(tools = [
...
class TestMcpClient extends McpClient {
    _inMemoryTransport;
    constructor(transport, name = 'test') {
...
function makeTestManager(configs, factory, opts) {
...
describe('McpManager mutation methods', () => {
    ...
    it('addServer fires onToolsChanged callback with added tools', async () => {
        ...
        const manager = makeTestManager([], (config) => new TestMcpClient(clientTransport, config.name), { onToolsChanged });
        await manager.connect();
    ...
    it('removeServer removes server and tools disappear from getTools()', async () => {
        ...
        const manager = makeTestManager([{ name: 'initial', transport: 'stdio', command: 'unused' }], (config) => new TestMcpClient(clientTransport, config.name));
        await manager.connect();
    ...
    it('removeServer fires onToolsChanged callback with removed tool names', async () => {
        ...
        const onToolsChanged = vi.fn();
        const manager = makeTestManager([{ name: 'srv', transport: 'stdio', command: 'unused' }], (config) => new TestMcpClient(clientTransport, config.name), { onToolsChanged });
        await manager.connect();
    ...
    it('addServer with duplicate name throws', async () => {
        const { clientTransport } = await createTestServer();
        const manager = makeTestManager([{ name: 'dup', transport: 'stdio', command: 'unused' }], (config) => new TestMcpClient(clientTransport, config.name));
        await manager.connect();
        await expect(manager.addServer({ name: 'dup', transport: 'stdio', command: 'unused' })).rejects.toThrow("MCP server 'dup' is already registered");
        await manager.disconnect();
    });
    // -------------------------------------------------------------------------
    // Unknown name
    ...
    it('removeServer with unknown name throws', async () => {
        ...
        await manager.connect();
        await expect(manager.removeServer('nonexistent')).rejects.toThrow("MCP server 'nonexistent' is not registered");
        await manager.disconnect();
    });
    // -------------------------------------------------------------------------
    // Connect failure
    ...
    it('addServer connect failure does NOT add client or invoke callback', async () => {
        ...
        await manager.connect();
        await expect(manager.addServer({ name: 'broken', transport: 'stdio', command: 'unused' })).rejects.toThrow('connect failed');
        expect(manager.getTools()).toHaveLength(0);
    ...
    it('concurrent addServer/removeServer serialize correctly', async () => {
        ...
        const manager = makeTestManager([], (config) => {
            ...
            if (!transport)
                throw new Error(`No transport for ${config.name}`);
    ...
    it('listServers returns a snapshot of connected servers', async () => {
        const { clientTransport } = await createTestServer();
        const manager = makeTestManager([{ name: 'srv1', transport: 'stdio', command: 'node' }], (config) => new TestMcpClient(clientTransport, config.name));
        await manager.connect();
