...
import { ConfidentialClientUnsupported, ensureValidToken, MissingToken, registerOAuthClient, } from '../oauth';
describe('registerOAuthClient', () => {
    ...
    it('throws ConfidentialClientUnsupported when server returns client_secret', async () => {
        ...
        await expect(registerOAuthClient('https://auth.example.com/register', baseDcrRequest)).rejects.toThrow(ConfidentialClientUnsupported);
    });
    ...
    it('throws when response is missing client_id', async () => {
        mockFetch.mockResolvedValue(jsonResponse({}));
        await expect(registerOAuthClient('https://auth.example.com/register', baseDcrRequest)).rejects.toThrow('Dynamic client registration response missing required client_id');
    });
    it('preserves all optional fields in the response', async () => {
        ...
        expect(result.registration_client_uri).toBe('https://auth.example.com/register/full-client-xyz');
    });
});
describe('ensureValidToken with UI context', () => {
    it('throws MissingToken when no token exists and context is ui', async () => {
        ...
        await expect(ensureValidToken('test-server', config, secrets, 'ui')).rejects.toThrow(MissingToken);
        try {
