...
describe('cross-surface parity', () => {
    ...
    it('refresh_token and expires_at are also personality-scoped', async () => {
        ...
        await storeTokens(serverName, {
            access_token: 'at-1',
            refresh_token: 'rt-1',
            expires_at: '2026-06-01T00:00:00.000Z',
        }, scoped);
        // Scoped paths
        expect(await baseResolver.get(`personalities/${personalityId}/mcp/${serverName}/access_token`)).toBe('at-1');
        expect(await baseResolver.get(`personalities/${personalityId}/mcp/${serverName}/refresh_token`)).toBe('rt-1');
        expect(await baseResolver.get(`personalities/${personalityId}/mcp/${serverName}/expires_at`)).toBe('2026-06-01T00:00:00.000Z');
