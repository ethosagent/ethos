...
import { buildAuthorizationUrl, consumeOAuthState, deleteTokens, exchangeCode, generateCodeChallenge, generateCodeVerifier, isTokenExpired, loadAccessToken, refreshToken, registerOAuthState, startCallbackServer, storeTokens, } from '../oauth';
// ---------------------------------------------------------------------------
...
describe('refreshToken', () => {
    ...
    it('calls token endpoint with refresh_token grant', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('https://auth.example.com/token', expect.objectContaining({ method: 'POST' }));
    ...
    it('throws when no refresh token is available', async () => {
        const secrets = createMockSecrets();
        await expect(refreshToken('srv', config, secrets)).rejects.toThrow('No refresh token available');
    });
...
describe('startCallbackServer', () => {
    ...
    it('rejects requests with invalid state', async () => {
        const { port, close } = await startCallbackServer();
        const resp = await fetch(`http://127.0.0.1:${port}/oauth/callback?code=test-code&state=bad-state`);
