...
import { McpInstallFlow, McpJsonStore } from '../index';
...
function scriptHappyDiscoveryAndDcr(script) {
    ...
    script.responders.set('https://mcp.linear.app/.well-known/oauth-protected-resource/sse', async () => new Response('not found', { status: 404 }));
    script.responders.set('https://mcp.linear.app/.well-known/oauth-authorization-server', async () => jsonResponse(DISCOVERY_OK));
    script.responders.set(DISCOVERY_OK.registration_endpoint, async () => jsonResponse(DCR_OK, 201));
}
function scriptHappyTokenExchange(script) {
...
class StubPersonalityRegistry {
    ...
    getDefault() {
        ...
        if (!first)
            throw new Error('no personalities');
        return first;
    }
    setDefault() {
        /* no-op */
    ...
    async update(id, patch) {
        ...
        if (patch.mcp_servers !== undefined)
            next.mcp_servers = patch.mcp_servers;
...
describe('McpInstallFlow.start', () => {
    ...
    it('OAuthDiscoveryError propagates; no placeholder written, no pending session', async () => {
        ...
        await expect(flow.start({ mcpUrl: MCP_URL, personalityId: PERSONALITY_ID })).rejects.toBeInstanceOf(OAuthDiscoveryError);
    ...
    it('DcrUnsupported when registration_endpoint is missing; no placeholder', async () => {
        ...
        script.responders.set('https://mcp.linear.app/.well-known/oauth-protected-resource/sse', async () => new Response('nope', { status: 404 }));
        ...
        await expect(flow.start({ mcpUrl: MCP_URL, personalityId: PERSONALITY_ID })).rejects.toBeInstanceOf(DcrUnsupported);
        expect(await jsonStore.list()).toEqual([]);
    });
    it('ConfidentialClientUnsupported when DCR returns a client_secret; no placeholder', async () => {
        ...
        script.responders.set('https://mcp.linear.app/.well-known/oauth-protected-resource/sse', async () => new Response('nope', { status: 404 }));
        script.responders.set('https://mcp.linear.app/.well-known/oauth-authorization-server', async () => jsonResponse(DISCOVERY_OK));
        script.responders.set(DISCOVERY_OK.registration_endpoint, async () => jsonResponse({ ...DCR_OK, client_secret: 'shh' }, 201));
        await expect(flow.start({ mcpUrl: MCP_URL, personalityId: PERSONALITY_ID })).rejects.toBeInstanceOf(ConfidentialClientUnsupported);
        expect(await jsonStore.list()).toEqual([]);
    });
    it('rejects when the derived name collides with an existing mcp.json entry', async () => {
        ...
        scriptHappyDiscoveryAndDcr(script);
        await expect(flow.start({ mcpUrl: MCP_URL, personalityId: PERSONALITY_ID })).rejects.toMatchObject({
            name: 'EthosError',
            code: 'INVALID_INPUT',
        });
    ...
    it('per-flow redirectUri overrides the constructor default in DCR + authorize URL', async () => {
        ...
        script.responders.set(DISCOVERY_OK.registration_endpoint, async ({ init }) => {
            ...
            capturedDcrBody = raw ? JSON.parse(raw) : undefined;
...
describe('McpInstallFlow.complete', () => {
    ...
    it('rolls back placeholder + drops session when token exchange fails', async () => {
        ...
        script.responders.set(DISCOVERY_OK.token_endpoint, async () => new Response('bad code', { status: 400 }));
    ...
    it('rolls back placeholder + tokens when secrets.set throws', async () => {
        ...
        failingSecrets.set = vi.fn(async (ref, value) => {
            if (firstSet) {
                firstSet = false;
        ...
        scriptHappyTokenExchange(script);
        await expect(flow.complete({ code: 'auth-code', state })).rejects.toThrow(/secrets backend down/);
        expect(manager.added).toHaveLength(0);
        expect(await jsonStore.get(serverName)).toBeNull();
        const prefix = `personalities/${PERSONALITY_ID}`;
    ...
    it('stores tokens at the personality-scoped path, not the global path', async () => {
        ...
        expect(await secrets.get(`personalities/${pid}/mcp/${serverName}/access_token`)).toBe(TOKEN_OK.access_token);
    ...
    it('different personalityIds produce isolated token namespaces', async () => {
        ...
        expect(await secrets.get(`personalities/alpha/mcp/${rAlpha.serverName}/access_token`)).toBe(TOKEN_OK.access_token);
        expect(await secrets.get(`personalities/beta/mcp/${rBeta.serverName}/access_token`)).toBe(TOKEN_OK.access_token);
...
describe('McpInstallFlow.attachToPersonalities', () => {
    function seedPersonalities(reg) {
