...
describe('McpManager', () => {
    ...
    it('Tool adapter execute calls through to McpClient', async () => {
        ...
        if (!echoTool)
            throw new Error('Expected echo tool to exist');
        const ctx = {
            sessionId: 'test',
            sessionKey: 'cli:test',
            platform: 'cli',
...
describe('McpServerConfig', () => {
    it('accepts introspection_endpoint in auth block', () => {
        ...
        expect(config.auth?.type === 'oauth2' ? config.auth.introspection_endpoint : undefined).toBe('http://auth/introspect');
    });
});
// ---------------------------------------------------------------------------
// Bearer auth tests
...
describe('bearer auth', () => {
    it('constructor rejects Authorization header', () => {
        ...
        expect(() => new McpClient(config)).toThrow('McpServerConfig.headers must not contain Bearer tokens');
    });
