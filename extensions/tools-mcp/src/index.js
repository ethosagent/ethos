...
export { buildAuthorizationUrl, ConfidentialClientUnsupported, DcrUnsupported, deleteTokens, discoverOAuthMetadata, ensureValidToken, exchangeCode, generateCodeChallenge, generateCodeVerifier, isTokenExpired, loadAccessToken, MissingToken, OAuthDiscoveryError, refreshToken, registerOAuthClient, revokeToken, runDcrAuthorization, runPkceLogin, startCallbackServer, storeTokens, } from './oauth';
...
export class McpClient {
        ...
        if (config.headers) {
            for (const [key, value] of Object.entries(config.headers)) {
                if (key.toLowerCase() === 'authorization' && /^\s*bearer\s/i.test(value)) {
                    throw new Error(`McpServerConfig.headers must not contain Bearer tokens.\n` +
    ...
    async connect() {
        ...
        this._sdk.setNotificationHandler(ToolListChangedNotificationSchema, async () => {
            try {
                ...
                this._logger.warn(`[ethos] MCP server '${this._config.name}' tools/list_changed re-fetch failed`, {
                    component: 'tools-mcp',
    ...
    _startKeepalive() {
        ...
        this._keepaliveInterval = setInterval(async () => {
            try {
                ...
                this._logger.warn(`[ethos] MCP server '${this._config.name}' keepalive failed, reconnecting`, { component: 'tools-mcp', server: this._config.name });
                this._clearKeepalive();
                this._connected = false;
                try {
    ...
    async _createTransport() {
        if (this._config.transport === 'stdio') {
            ...
            if (this._config.env) {
                ...
                for (const [key, value] of Object.entries(this._config.env)) {
                    if (PINNED_MCP_KEYS.has(key))
                        continue;
                    if (!(key in safeEnv) && !declared.has(key))
                        continue;
                    mergedEnv[key] = value;
                }
            }
        ...
        if (this._config.transport === 'streamable-http') {
            ...
            if (!url)
                throw new Error(`MCP server '${this._config.name}': streamable-http transport requires 'url'`);
            ...
            if (this._config.auth?.type === 'bearer') {
                if (!this._secrets) {
                    throw new Error(`MCP server '${this._config.name}' requires bearer auth but no secrets resolver is configured.`);
                }
                const token = await this._secrets.get(mcpTokenSecretRef(this._config.name));
                if (!token) {
                    throw new Error(`MCP server '${this._config.name}' requires a bearer token. Set it via: echo "<token>" | ethos personality mcp <id> --token-stdin ${this._config.name}`);
                }
                headers.Authorization = `Bearer ${token}`;
            }
        ...
        if (this._config.transport === 'sse') {
            const { url } = this._config;
            if (!url)
                throw new Error(`MCP server '${this._config.name}': sse transport requires 'url'`);
            this._logger.warn(`[ethos] MCP '${this._config.name}': SSE transport is deprecated, use 'streamable-http'`, { component: 'tools-mcp', server: this._config.name });
            const headers = { ...this._config.headers };
            if (this._config.auth?.type === 'oauth2' && this._secrets) {
                const token = await ensureValidToken(this._config.name, this._config.auth, this._secrets, 'ui');
                headers.Authorization = `Bearer ${token}`;
        ...
        throw new Error(`MCP server '${this._config.name}': unknown transport '${this._config.transport}'`);
    }
    _scheduleReconnect(attempt) {
        if (this._destroyed || attempt >= 5)
            return;
    ...
    async _callToolInner(name, args, allowRetry) {
        ...
        try {
            const raw = await Promise.race([
                this._sdk.callTool({ name, arguments: args }),
                guard,
            ]);
            ...
            if (allowRetry &&
                ...
                this._config.auth?.type === 'oauth2' &&
                this._secrets) {
                this._logger.warn(`[ethos] MCP server '${this._config.name}' returned 401, attempting token refresh`, { component: 'tools-mcp', server: this._config.name });
                try {
...
export class McpManager {
    ...
    async getToolsForPersonality(personalityId) {
        const tools = [];
        await Promise.allSettled(this._configs.map(async (config) => {
            let client;
                    ...
                    if (inflight) {
                        ...
                        const connectPromise = (async () => {
                            ...
                            try {
                                ...
                                this.logger.warn(`[ethos] MCP server '${config.name}' failed to connect for personality '${personalityId}'`, {
                                    component: 'tools-mcp',
                                    server: config.name,
    ...
    async connect() {
        ...
        const pendingTools = [];
        await Promise.allSettled(this._clients.map(async (client) => {
            try {
    ...
    async addServer(config) {
        await this._serialize(async () => {
            ...
            try {
                ...
                connected = true;
                const mcpTools = await client.listTools();
                const adapted = mcpTools.map((t) => adaptMcpTool(t, client.name, client, config.mcpResultLimitChars));
    ...
    async removeServer(name) {
        await this._serialize(async () => {
            ...
            for (const k of oauthToRemove)
                this._oauthClients.delete(k);
            ...
            if (this._onToolsChanged)
                this._onToolsChanged([], removedNames);
        });
    }
    /**
    ...
    async renameServer(oldName, newName) {
        await this._serialize(async () => {
            ...
            try {
                ...
                const adapted = mcpTools.map((t) => adaptMcpTool(t, newName, newClient, newConfig.mcpResultLimitChars));
                ...
                if (this._onToolsChanged)
                    this._onToolsChanged(adapted, removedNames);
            }
            catch (err) {
                if (connected) {
                    try {
    ...
    async updateToken(serverName, token) {
        await this._serialize(async () => {
            ...
            try {
                ...
                const adapted = mcpTools.map((t) => adaptMcpTool(t, serverName, newClient, config.mcpResultLimitChars));
    ...
    listServers() {
        return this._clients.map((c) => {
            ...
            if (cfg.url !== undefined)
                info.url = cfg.url;
            if (cfg.created_via !== undefined)
                info.created_via = cfg.created_via;
            return info;
        });
    }
}
