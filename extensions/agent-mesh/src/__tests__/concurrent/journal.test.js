...
beforeEach(() => {
    seq++;
    workDir = join(tmpdir(), `ethos-cc4-${process.pid}-${seq}-${Math.random().toString(36).slice(2)}`);
...
describe('CC-4: mesh.jsonl atomic line writes', () => {
    it('100 concurrent writers all land in the journal', async () => {
        const WRITERS = 100;
        await Promise.all(Array.from({ length: WRITERS }, (_, i) => Promise.resolve().then(() => appendLine(logFile, { ts: new Date().toISOString(), event: 'test', writer: i }))));
    ...
    it('1000 concurrent writes produce 1000 valid rows', async () => {
        const ROWS = 1000;
        await Promise.all(Array.from({ length: ROWS }, (_, i) => Promise.resolve().then(() => appendLine(logFile, { ts: new Date().toISOString(), event: 'stress', seq: i }))));
        const lines = await readLines(logFile);
