...
import { dispatch, parseSubcommand } from '../commands';
...
describe('dispatch — personality', () => {
    ...
    it('describes a team binding', async () => {
        const r = await dispatch({ ...basePayload, text: 'personality' }, ctxFor({ binding: { type: 'team', name: 'eng' } }));
        expect(r.text).toContain('team coordinator');
        expect(r.text).toContain('eng');
    });
    const cardReader = (card) => ({ read: async () => card });
    const sampleCard = {
        id: 'researcher',
        name: 'Researcher',
        ...
        skills: [{ id: 'cite-sources', source: 'personality' }],
    };
    it('renders the rich card for `personality rich` when a reader is wired', async () => {
        const r = await dispatch({ ...basePayload, text: 'personality rich' }, ctxFor({ personalityCard: cardReader(sampleCard) }));
        expect(r.responseType).toBe('ephemeral');
    ...
    it('falls back to the compact view for a team binding', async () => {
        const r = await dispatch({ ...basePayload, text: 'personality rich' }, ctxFor({ binding: { type: 'team', name: 'eng' }, personalityCard: cardReader(sampleCard) }));
        expect(r.text).toContain('team coordinator');
        expect(r.text).not.toContain('What it can do');
    });
    it('falls back to the compact view when the reader returns null', async () => {
        const r = await dispatch({ ...basePayload, text: 'personality rich' }, ctxFor({ personalityCard: cardReader(null) }));
        expect(r.text).toContain('bound to');
        expect(r.text).not.toContain('What it can do');
    });
});
describe('dispatch — channel-mode', () => {
    ...
    it('show reflects per-channel override after set', async () => {
        ...
        await dispatch({ ...basePayload, text: 'channel-mode all' }, ctxFor({ channelOverrides: overrides }));
        const show = await dispatch({ ...basePayload, text: 'channel-mode show' }, ctxFor({ channelOverrides: overrides }));
        expect(show.text).toContain('all');
        expect(show.text).toContain('per-channel override');
    });
    it('rejects invalid mode argument', async () => {
        const overrides = new ChannelOverrideStore(memStorage(), '/slack', 'bot-a');
        const r = await dispatch({ ...basePayload, text: 'channel-mode unicorn' }, ctxFor({ channelOverrides: overrides }));
...
describe('dispatch — ask', () => {
    ...
    it('invokes submitter and posts in_channel acknowledgement', async () => {
        ...
        const r = await dispatch({ ...basePayload, text: 'ask hello world' }, ctxFor({ submitAgentTurn: submit }));
...
describe('dispatch — memory', () => {
    ...
    it('show renders entries from MemoryReader', async () => {
        const memory = {
            read: async () => ['- one entry', '- two entries', '- three entries', '- four entries'].join('\n'),
    ...
    it('add appends to memory and confirms', async () => {
        ...
        const memory = { read: async () => null, append };
        const r = await dispatch({ ...basePayload, text: 'memory add this is a thing' }, ctxFor({ memory }));
...
describe('dispatch — kanban', () => {
    ...
    it('lists tickets for team bots', async () => {
        ...
        const r = await dispatch({ ...basePayload, text: 'kanban list' }, ctxFor({ binding: { type: 'team', name: 'eng' }, kanban }));
        expect(r.text).toContain('fix bug');
        expect(r.text).toContain('alice');
    });
    it('says unavailable when no kanban wired for team bot', async () => {
        const r = await dispatch({ ...basePayload, text: 'kanban list' }, ctxFor({ binding: { type: 'team', name: 'eng' } }));
