...
import { CLARIFY_ANSWER_ACTION_ID, CLARIFY_CANCEL_ACTION_ID, CLARIFY_CHOICE_ACTION_ID, CLARIFY_MODAL_CALLBACK_ID, CLARIFY_MODAL_INPUT_ACTION_ID, CLARIFY_MODAL_INPUT_BLOCK_ID, } from '../blocks/clarify';
...
describe('handleClarifyAction', () => {
    ...
    it('routes a cancel click', async () => {
        ...
        expect(onAction).toHaveBeenCalledWith(expect.objectContaining({ kind: 'cancel', requestId: 'r1' }));
    });
    it('routes an open-modal click and forwards the trigger id', async () => {
        const onAction = vi.fn();
        await handleClarifyAction(payload({ actionId: CLARIFY_ANSWER_ACTION_ID, value: 'r1', triggerId: 'TRG' }), { onAction });
        expect(onAction).toHaveBeenCalledWith(expect.objectContaining({ kind: 'open-modal', requestId: 'r1', triggerId: 'TRG' }));
    });
    ...
    it('drops open-modal click with no trigger id (Slack would 4xx anyway)', async () => {
        const onAction = vi.fn();
        await handleClarifyAction(payload({ actionId: CLARIFY_ANSWER_ACTION_ID, value: 'r1', triggerId: '' }), { onAction });
        expect(onAction).not.toHaveBeenCalled();
    });
