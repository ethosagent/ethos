...
describe('blocks/unfurl', () => {
    ...
    it('kanbanUnfurlBlocks renders title, status, assignee, parent goal', () => {
        const text = plaintextFallback(kanbanUnfurlBlocks({
            id: 't1',
            title: 'Fix the bug',
            status: 'in_progress',
            assignee: 'alice',
    ...
    it('personalityUnfurlBlocks renders name, description, memory scope', () => {
        const text = plaintextFallback(personalityUnfurlBlocks({
            id: 'researcher',
            name: 'Researcher',
    ...
    it('escapes mrkdwn metacharacters in every model-influenced field', () => {
        const sessionJson = JSON.stringify(sessionUnfurlBlocks({
            id: 's1',
        ...
        const kanbanJson = JSON.stringify(kanbanUnfurlBlocks({
            id: 't1',
            title: '<!here> sneaky',
            status: '<status>',
            assignee: '<@U1>',
        ...
        const personalityJson = JSON.stringify(personalityUnfurlBlocks({
            id: 'p1',
            name: '<!everyone>',
            description: '<script>',
        }));
    ...
    it('truncates oversized fields well under the Slack ~3000-char section limit', () => {
        const longest = (blocks) => {
            ...
            for (const block of blocks) {
                if (block.type === 'section' || block.type === 'header') {
                    const t = block.text;
                    if (t?.text)
                        max = Math.max(max, t.text.length);
                }
                else if (block.type === 'context') {
                    const els = block.elements ?? [];
                    for (const el of els)
                        if (el.text)
                            max = Math.max(max, el.text.length);
                }
            }
            return max;
        };
        expect(longest(kanbanUnfurlBlocks({
            id: 't1',
        ...
        expect(longest(personalityUnfurlBlocks({
            id: 'p1',
...
describe('events/links — registerLinkEvents', () => {
    ...
    it('calls chat.unfurl with a block map keyed by the matched URL', async () => {
        ...
        const arg = unfurl.mock.calls[0]?.[0];
    ...
    it('unfurls each of the three URL types', async () => {
        ...
        const arg = unfurl.mock.calls[0]?.[0];
        expect(Object.keys(arg.unfurls).sort()).toEqual([`${base}/kanban/t1`, `${base}/personalities/p1`, `${base}/sessions/s1`].sort());
    });
    ...
    it('skips a matched URL when its reader is absent (no hollow card)', async () => {
        const app = fakeApp();
        registerLinkEvents(app, { webUiBaseUrl: base });
        const unfurl = vi.fn().mockResolvedValue(undefined);
        const handler = app.handlers.get('event:link_shared');
        if (!handler)
            throw new Error('handler not registered');
    ...
    it('caps reader fan-out at MAX_UNFURLS_PER_EVENT for a link-spamming message', async () => {
        const app = fakeApp();
        const lookupSession = vi.fn(async (id) => ({
            id,
            personalityName: 'Researcher',
            lastActivity: new Date(0),
        }));
        registerLinkEvents(app, { webUiBaseUrl: base, session: { lookupSession } });
        const unfurl = vi.fn().mockResolvedValue(undefined);
        const handler = app.handlers.get('event:link_shared');
        if (!handler)
            throw new Error('handler not registered');
