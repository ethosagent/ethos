...
describe('SlackAdapter — webUiBaseUrl normalization', () => {
    ...
    const baseUrlOf = (adapter) => adapter.webUiBaseUrl;
    const make = (webUiBaseUrl) => new SlackAdapter({
        botToken: 'xoxb-fake',
        appToken: 'xapp-fake',
        signingSecret: 'sig-fake',
        webUiBaseUrl,
    ...
    it('preserves a path-prefixed base URL', () => {
        expect(baseUrlOf(make('https://company.example.com/ethos'))).toBe('https://company.example.com/ethos');
    });
