...
import { CLARIFY_ANSWER_ACTION_ID, CLARIFY_CANCEL_ACTION_ID, CLARIFY_CHOICE_ACTION_ID, CLARIFY_MODAL_CALLBACK_ID, } from './blocks/clarify';
...
export class SlackAdapter {
    ...
    async start() {
        ...
        registerMessageEvents(this.app, {
            botKey: this.botKey,
        ...
        if (this.binding) {
            registerMemberEvents(this.app, {
                ...
                resolveChannelMode: (channel) => resolveChannelMode(channel, {
                    botKey: this.botKey,
                    defaultChannelMode: this.defaultChannelMode,
                    channelOverrides: this.channelOverrides,
                }),
    ...
    addReceiptReaction(msg) {
        ...
        while (this.pendingReactions.size >= this.pendingReactionsMaxEntries &&
    ...
    async postApprovalCard(input) {
        ...
        try {
            ...
            const ts = result.ts;
            if (!ts)
                return { error: 'Slack accepted the approval card but returned no message ts' };
            return { messageTs: ts };
        }
        catch (err) {
    ...
    async postClarifyCard(input) {
        try {
            ...
            const ts = result.ts;
            if (!ts)
                return { error: 'Slack accepted the clarify card but returned no message ts' };
            return { messageTs: ts };
        }
        catch (err) {
            return { error: err instanceof Error ? err.message : String(err) };
