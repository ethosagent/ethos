 ...
 */
export async function handleClarifyAction(payload, handlers) {
    if (!payload.userId)
        return;
    ...
    if (payload.actionId === CLARIFY_ANSWER_ACTION_ID) {
        if (!payload.value || !payload.triggerId)
            return;
        try {
            await handlers.onAction({
                kind: 'open-modal',
...
export async function handleClarifyModalSubmission(payload, handlers) {
    if (payload.callbackId !== CLARIFY_MODAL_CALLBACK_ID)
        return;
    if (!payload.userId)
        return;
    let requestId;
    try {
        const parsed = JSON.parse(payload.privateMetadata);
    ...
    const answer = payload.values[CLARIFY_MODAL_INPUT_BLOCK_ID]?.[CLARIFY_MODAL_INPUT_ACTION_ID]?.value;
