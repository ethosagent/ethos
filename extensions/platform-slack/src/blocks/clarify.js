...
export function clarifyPendingBlocks(input) {
    const blocks = [
        header('Question'),
    ...
    if (options.length > 0) {
        ...
        const elements = capped.map((label, idx) => ({
            type: 'button',
    ...
    if (input.default !== undefined) {
        ctx.push(`default at <!date^${epoch(input.defaultDeadlineAt)}^{time}|${escapeMrkdwn(input.defaultDeadlineAt)}>: \`${escapeMrkdwn(truncate(input.default, DEFAULT_LABEL_MAX))}\``);
    }
    else {
        ctx.push(`no default — answer by <!date^${epoch(input.defaultDeadlineAt)}^{time}|${escapeMrkdwn(input.defaultDeadlineAt)}> or cancel`);
    }
    blocks.push(context(ctx));
    return blocks;
}
export function clarifyResolvedBlocks(input) {
    ...
    switch (input.source) {
        ...
        case 'timeout-default':
            blocks.push(section(`_(timed out — used default \`${escapeMrkdwn(truncate(input.answer, ANSWER_TEXT_MAX))}\`)_`));
            break;
...
export function clarifyHomeEntryBlocks(row) {
...
export function clarifyModalView(input) {
    return {
        type: 'modal',
