...
//
// **Honest scope.** PATTERNS is regex matching against the raw command
// string. The inline-eval wrappers (`sh -c` and its siblings, `eval`,
// `python -c`, `node -e`, anything piped into a shell) are hardline since
// D1(b) of plan openclaw-2026.9.6-gaps, read from each simple command's argv
// by a small tokenizer (`inlineEvalReason` below — its section comment says
// exactly which shapes); case-variant `rm` is the /i on the rm entry.
// Command substitution (`$(…)` and backticks) is NOT hardline: it requires
// approval (APPROVAL_PATTERNS below), so a surface with a human asks and a
// surface without one refuses. What STILL defeats the hardline, and is not
// caught:
//   - variable indirection: `a=rm; b=-rf; c=/; $a $b $c`, `$SHELL -c id`
//   - other interpreters' eval flags: `perl -e`, `ruby -e`, `php -r`,
//     `deno eval`, `bun -e`, fish's `-C`, `env -S 'sh -c …'`,
//     `awk 'BEGIN{system(…)}'`, `find -exec`, `xargs rm`
//   - a wrapper outside WRAPPERS in front of a pipe's shell or `eval`
//     (`| stdbuf -o0 sh`, `| setsid bash`) — interpreter FLAGS are checked
//     at every word, but a bare shell or `eval` only at the command's head
//   - process substitution and here-docs/here-strings fed to a shell:
//     `sh <(…)`, `tee >(sh)`, `bash <<< "…"`, `sh script-the-agent-just-wrote.sh`
//   - quoting inside a word defeats the regex entries: `r''m -rf /` (the
//     tokenizer does strip quotes, so `"ba"sh -c` IS caught)
