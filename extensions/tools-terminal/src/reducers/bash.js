...
function reduceGitStatus(result) {
    ...
    if (counts.renamed)
        parts.push(`${counts.renamed} renamed`);
...
function reduceTestRun(result) {
    ...
    for (const line of lines) {
        ...
        if (inSummary || /\b(FAIL|Error:)\b/.test(line))
            kept.push(line);
    }
...
function reduceGeneric(result) {
    if (!result.ok)
        return result;
    if (result.value.length <= GENERIC_THRESHOLD_BYTES)
        return result;
    const lines = result.value.split('\n');
...
export const bashReducer = {
    ...
    reduce(result, ctx) {
        ...
        if (isGitStatus(cmd))
            return reduceGitStatus(result);
        if (isTestRun(cmd))
            return reduceTestRun(result);
        if (isInstall(cmd))
            return reduceInstall(result);
