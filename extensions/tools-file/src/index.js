 ...
 */
function canonicalizeForRead(path) {
    return resolve(path);
}
function isWriteBlocked(abs) {
    const normalized = resolve(abs);
    if (BLOCKED_WRITE_PATHS.some((p) => resolve(p) === normalized))
        return true;
 ...
 */
async function checkStaleWrite(abs, readMtimes, fs) {
    if (!readMtimes)
        return null;
...
function reachFailure(kind, path) {
    return {
        ok: false,
...
export const readFileTool = {
    name: 'read_file',
    description: 'Read a file from the filesystem. Supports line ranges for large files. Paths starting with ~/ are expanded to the home directory.',
    toolset: 'file',
    ...
    async execute(args, ctx) {
        const { ref, path, start_line, end_line } = args;
        ...
        if (!resolvedPath)
            return { ok: false, error: 'path is required', code: 'input_invalid' };
...
export const writeFileTool = {
    name: 'write_file',
    description: 'Write content to a file. Creates parent directories if needed. Blocked for ~/.ethos/config.yaml and session storage.',
    toolset: 'file',
    capabilities: {
...
export const patchFileTool = {
    name: 'patch_file',
    description: 'Replace an exact block of text in a file with new content. old_text must match the file content exactly (including whitespace and indentation). Use read_file first to confirm the exact text.',
    toolset: 'file',
    capabilities: {
        fs_reach: { read: 'from-personality', write: 'from-personality' },
    },
    ...
    async execute(args, ctx) {
        ...
        if (!path)
            return { ok: false, error: 'path is required', code: 'input_invalid' };
        if (!old_text)
            return { ok: false, error: 'old_text is required', code: 'input_invalid' };
        const abs = expandPath(path, ctx.workingDir);
        const fs = fsOf(ctx);
        if (!('mtime' in fs))
            return fs;
...
async function walkAndSearch(fs, dir, pattern, glob, matches, maxMatches, depth) {
    if (depth > 6 || matches.length >= maxMatches)
        return;
    let entries;
    try {
...
export const searchFilesTool = {
    name: 'search_files',
    description: 'Search for a text pattern across files in a directory. Returns file paths, line numbers, and matching lines.',
    toolset: 'file',
    ...
    async execute(args, ctx) {
        const { pattern, path, glob, max_results } = args;
        if (!pattern)
            return { ok: false, error: 'pattern is required', code: 'input_invalid' };
        ...
        try {
            ...
            if (isReachError(err))
                return reachFailure('read', searchDir);
            throw err;
        }
        if (matches.length === 0) {
