...
describe('tools-file — fs_reach boundary enforcement', () => {
    ...
    it('researcher CAN read its own MEMORY.md', async () => {
        ...
        const result = await readFileTool.execute({ path: join(dataDir, 'personalities', 'researcher', 'MEMORY.md') }, ctx);
        expect(result.ok).toBe(true);
        if (result.ok)
            expect(result.value).toContain('mine');
    });
    it("researcher CANNOT read engineer's MEMORY.md", async () => {
        const ctx = makeCtx({ workingDir: cwd, scopedFs: researcherFs() });
        const result = await readFileTool.execute({ path: join(dataDir, 'personalities', 'engineer', 'MEMORY.md') }, ctx);
    ...
    it('researcher CAN read files in its working dir', async () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok)
            expect(result.value).toContain('cwd notes');
    });
