...
import { createFileTools, patchFileTool, readFileTool, searchFilesTool, writeFileTool, } from '../index';
...
describe('read_file', () => {
    ...
    it('returns a range of lines', async () => {
        ...
        const result = await readFileTool.execute({ path, start_line: 2, end_line: 4 }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        if (result.ok) {
...
describe('write_file', () => {
    it('writes a new file', async () => {
        ...
        const result = await writeFileTool.execute({ path, content: 'export const x = 1;' }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        const readBack = await readFileTool.execute({ path }, makeCtx(testDir));
        expect(readBack.ok).toBe(true);
        if (readBack.ok)
            expect(readBack.value).toContain('export const x = 1;');
    });
...
describe('patch_file', () => {
    it('replaces old_text with new_text', async () => {
        ...
        await writeFile(path, 'const x = 1;\nconst y = 2;\n');
        const result = await patchFileTool.execute({ path, old_text: 'const y = 2;', new_text: 'const y = 42;' }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        const readBack = await readFileTool.execute({ path }, makeCtx(testDir));
        if (readBack.ok)
            expect(readBack.value).toContain('const y = 42;');
    });
    it('returns error if old_text not found', async () => {
        const path = join(testDir, 'patch.ts');
        await writeFile(path, 'hello world\n');
        const result = await patchFileTool.execute({ path, old_text: 'not there', new_text: 'replacement' }, makeCtx(testDir));
        expect(result.ok).toBe(false);
        if (!result.ok)
            expect(result.code).toBe('execution_failed');
    });
    it('returns error when old_text matches more than once', async () => {
        ...
        await writeFile(path, 'const x = 1;\nconst x = 1;\n');
        const result = await patchFileTool.execute({ path, old_text: 'const x = 1;', new_text: 'const x = 2;' }, makeCtx(testDir));
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.code).toBe('execution_failed');
...
describe('search_files', () => {
    it('finds matching lines across files', async () => {
        ...
        const result = await searchFilesTool.execute({ pattern: 'ethos', path: testDir }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        if (result.ok) {
    ...
    it('filters by glob', async () => {
        ...
        const result = await searchFilesTool.execute({ pattern: 'found', path: testDir, glob: '*.ts' }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        if (result.ok) {
    ...
    it('returns no-matches message when nothing found', async () => {
        const result = await searchFilesTool.execute({ pattern: 'zzznomatch', path: testDir }, makeCtx(testDir));
        expect(result.ok).toBe(true);
        if (result.ok)
            expect(result.value).toContain('No matches');
    });
