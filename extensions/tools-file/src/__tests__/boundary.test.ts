...
import { FsStorage, defaultAlwaysDeny } from '@ethosagent/storage-fs';
...
describe('tools-file — fs_reach boundary enforcement', () => {
  ...
  it('rejects a path matching the alwaysDeny floor even when allow includes its parent', async () => {
    ...
    const permissive = new ScopedFsImpl(new FsStorage(), reachIncludingEtc, reachIncludingEtc, defaultAlwaysDeny());
