...
describe('subagent task contract — task lives only in child first user message', () => {
    it('PINEAPPLE-MARKER never appears in any system prompt and exactly once across user messages (in child loop)', async () => {
        ...
        for (const req of seen) {
            expect(req.system ?? '', 'task marker must never appear in any system prompt').not.toContain(MARKER);
        }
        ...
        expect(userMarkerCount, 'task marker must appear exactly once across user-role messages — in the child first user message').toBe(1);
        ...
        expect(childRequest, 'expected the child loop to issue at least one complete() call').toBeDefined();
