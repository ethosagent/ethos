 ...
 */
function assertCanDelegateTo(callerPersonality, targetPersonality) {
    // No target specified or no caller personality → no restriction.
    if (!targetPersonality || !callerPersonality)
        return undefined;
    // Same personality → allowed.
    if (targetPersonality === callerPersonality)
        return undefined;
    // Cross-personality delegation blocked.
    return (`Cannot delegate to personality "${targetPersonality}": ` +
...
// ---------------------------------------------------------------------------
async function runSubAgent(loop, prompt, opts) {
    let output = '';
...
// ---------------------------------------------------------------------------
export function createDelegateTaskTool(loop) {
    return {
        name: 'delegate_task',
        description: 'Spawn a sub-agent to handle a specific task and return its output. ' +
        ...
        schema: {
            ...
            properties: {
                ...
                personality: {
                    type: 'string',
                    description: 'Personality for the sub-agent (e.g. "researcher", "reviewer"). Defaults to current personality.',
                },
                label: {
                    type: 'string',
        ...
        async execute(args, ctx) {
            const { prompt, personality, label } = args;
            if (!prompt)
...
// ---------------------------------------------------------------------------
export function createMixtureOfAgentsTool(loop) {
    return {
        name: 'mixture_of_agents',
        description: 'Run multiple sub-agents in parallel, each with a different prompt or personality, ' +
        ...
        schema: {
            ...
            properties: {
                ...
                synthesis_prompt: {
                    type: 'string',
                    description: "Optional prompt to synthesise the agents' outputs into a final answer. " +
        ...
        async execute(args, ctx) {
            const { agents, synthesis_prompt } = args;
            if (!agents?.length) {
                return {
                    ok: false,
...
async function callMeshAgent(host, port, prompt, personalityId, signal, fetchImpl) {
    ...
    if (promptData.error)
        throw new Error(promptData.error.message ?? 'Remote agent error');
    return promptData.result?.text ?? '';
}
function personalityFromAgentId(agentId) {
    const idx = agentId.indexOf(':');
    return idx > 0 ? agentId.slice(0, idx) : agentId;
}
function selectCandidates(entries, capability) {
    return entries
        .filter((e) => e.capabilities.includes(capability))
        .sort((a, b) => a.activeSessions !== b.activeSessions
...
async function routeWithFailover(params) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
            ...
            const text = await callMeshAgent(agent.host, agent.port, params.prompt, params.personalityId, signal, params.fetchImpl);
...
export function createListTeamTool(registryPath = defaultRegistryPath()) {
    return {
        name: 'list_team',
        description: 'List live mesh peers with personality, capabilities, and load. ' +
...
export function createRouteToAgentTool(registryPath = defaultRegistryPath()) {
    return {
        name: 'route_to_agent',
        description: 'Route a task to the best available mesh agent advertising a given capability. ' +
        ...
        async execute(args, ctx) {
            const { capability, prompt, timeout_s, retries } = args;
            if (!capability)
                return { ok: false, error: 'capability is required', code: 'input_invalid' };
            if (!prompt)
                return { ok: false, error: 'prompt is required', code: 'input_invalid' };
...
export function createDispatchTeamTool(registryPath = defaultRegistryPath()) {
    return {
        name: 'dispatch_team',
        description: 'Dispatch multiple capability-scoped tasks across mesh peers in parallel. ' +
...
export function createBroadcastToAgentsTool(registryPath = defaultRegistryPath()) {
    return {
        name: 'broadcast_to_agents',
        description: 'Send a prompt to all live agents in the mesh and return their combined responses. ' +
        ...
        async execute(args, ctx) {
            ...
            const results = await Promise.allSettled(agents.map(async (agent) => {
                const text = await callMeshAgent(agent.host, agent.port, prompt, undefined, ctx.abortSignal, fetchFn);
...
// Factory
// ---------------------------------------------------------------------------
export function createDelegationTools(loop, registryPath) {
    return [
