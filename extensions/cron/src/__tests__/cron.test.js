...
describe('CronScheduler updateJob', () => {
    ...
    it('rejects update with no fields', async () => {
        ...
        await expect(scheduler.updateJob('empty-update', {})).rejects.toThrow('At least one of name, schedule, or prompt is required');
    });
    it('rejects update with invalid schedule', async () => {
        ...
        await expect(scheduler.updateJob('bad-schedule-update', { schedule: 'garbage' })).rejects.toThrow('Invalid schedule');
    });
...
describe('CronScheduler tick', () => {
    it('skip policy does not run overdue job', async () => {
        ...
        if (!updated?.nextRunAt)
            throw new Error('expected updated.nextRunAt to be set');
...
describe('readRunOutput — path containment', () => {
    ...
    it('rejects an absolute path outside outputDir', async () => {
        const scheduler = makeScheduler();
        await expect(scheduler.readRunOutput('/etc/passwd')).rejects.toThrow('Path outside output directory');
    });
    it('rejects a path with ".." traversal', async () => {
        const scheduler = makeScheduler();
        await expect(scheduler.readRunOutput(join(testDir, 'output', '..', 'escape.md'))).rejects.toThrow('Path outside output directory');
    });
});
