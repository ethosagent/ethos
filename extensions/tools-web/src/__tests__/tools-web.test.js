 ...
 */
function makeScopedFetch(policy = {}) {
    const stubFetch = async (_url, _init) => new Response('OK', { status: 200 });
    const stubResolver = async (_hostname) => ['93.184.216.34']; // public IP
    return {
...
describe('web_search', () => {
    it('isAvailable returns false when ETHOS_EXA_API_KEY is not set', () => {
        ...
        if (saved)
            process.env.ETHOS_EXA_API_KEY = saved;
    });
...
describe('web_extract', () => {
    ...
    it('returns not_available when capability backends are missing', async () => {
        const result = await webExtractTool.execute({ url: 'https://example.com' }, ctxWithoutCapabilities);
        expect(result.ok).toBe(false);
        if (!result.ok)
            expect(result.code).toBe('not_available');
    });
