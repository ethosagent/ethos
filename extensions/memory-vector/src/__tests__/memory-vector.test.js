...
let provider;
beforeEach(async () => {
    testDir = join(tmpdir(), `ethos-vector-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('VectorMemoryProvider', () => {
    ...
    describe('list', () => {
        ...
        it('attaches summaries when requested', async () => {
            await provider.sync([{ action: 'add', key: 'doc', content: 'Heading paragraph.\n\nDetails.' }], ctx);
    ...
    describe('sync — remove', () => {
        it('removes matching lines but keeps the rest', async () => {
            await provider.sync([{ action: 'add', key: 'k1', content: 'keep this\nremove specific line\nalso keep' }], ctx);
    ...
    describe('legacy memory_chunks migration', () => {
      ...
      `);
            ...
            legacyDb
                .prepare('INSERT INTO memory_chunks (store, content, embedding, created_at) VALUES (?, ?, ?, ?)')
