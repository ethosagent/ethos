...
export class VectorMemoryProvider {
    ...
    migrateLegacyChunks() {
        ...
        if (!tableExists)
            return;
        const rows = this.db
            .prepare('SELECT id, store, content, embedding, created_at FROM memory_chunks ORDER BY id ASC')
            .all();
        if (rows.length === 0) {
            this.db.exec('DROP TABLE memory_chunks');
            return;
        }
        const insert = this.db.prepare(`INSERT OR IGNORE INTO memory_entries
    ...
    async read(key, ctx) {
        const row = this.db
            .prepare('SELECT scope_id, key, content, created_at, updated_at FROM memory_entries WHERE scope_id = ? AND key = ?')
            .get(ctx.scopeId, key);
        if (!row)
            return null;
        return {
    ...
    async search(query, ctx, opts) {
        ...
        if (!trimmed)
            return [];
        if (mode === 'keyword')
            return this.keywordSearch(trimmed, ctx, limit);
        if (mode === 'hybrid')
            return this.hybridSearch(trimmed, ctx, limit);
    ...
    async list(ctx, opts) {
        ...
        const sql = 'SELECT key, content, updated_at FROM memory_entries WHERE scope_id = ? ORDER BY created_at ASC, key ASC' +
            (limit !== undefined ? ' LIMIT ?' : '');
        const stmt = this.db.prepare(sql);
        const rows = (limit !== undefined ? stmt.all(ctx.scopeId, limit) : stmt.all(ctx.scopeId));
        return rows.map((row) => {
            const ref = { key: row.key };
    ...
    showRecent(limit = 20) {
        const rows = this.db
            .prepare('SELECT scope_id, key, content, created_at, updated_at FROM memory_entries ORDER BY created_at DESC, rowid DESC LIMIT ?')
            .all(limit);
        return rows.map((r) => ({
            scopeId: r.scope_id,
            key: r.key,
    ...
    async exportAll(outputPath) {
        const rows = this.db
            .prepare('SELECT scope_id, key, content, created_at FROM memory_entries ORDER BY created_at ASC, key ASC')
            .all();
        if (rows.length === 0)
            return 0;
        const lines = [`# Memory Export — ${new Date().toISOString()}`, ''];
        for (const row of rows) {
    ...
    count() {
        return this.db.prepare('SELECT COUNT(*) AS n FROM memory_entries').get().n;
    }
    // ---------------------------------------------------------------------------
    ...
    async migrateFromMarkdown() {
        if (this.count() > 0)
            return { migrated: false, memoryChunks: 0, userChunks: 0 };
        let memoryChunks = 0;
        let userChunks = 0;
    ...
    async upsert(scopeId, key, content) {
        ...
        this.db
            .prepare(`INSERT INTO memory_entries (scope_id, key, content, embedding, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
    ...
    async hybridSearch(query, ctx, limit) {
