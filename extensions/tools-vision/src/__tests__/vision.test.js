...
function makeThrowingProvider(errorMessage) {
    return {
        name: 'stub',
...
function happyImageChunks(text) {
    return [
...
// ---------------------------------------------------------------------------
function makeScopedFs(allowedPrefixes) {
...
describe('vision_analyze — happy path (image)', () => {
    it('returns the streamed text, model, cost, and tokens for an image', async () => {
        ...
        if (!result.ok)
            throw new Error(`unexpected failure: ${result.code} ${result.error}`);
...
describe('vision_analyze — happy path (PDF)', () => {
    it('returns text + usage for a PDF input', async () => {
        ...
        const result = await tool.execute({ file_path: path, prompt: 'summarize' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        if (!result.ok)
            throw new Error(`unexpected failure: ${result.code} ${result.error}`);
        const parsed = JSON.parse(result.value);
...
describe('vision_analyze — input validation', () => {
    it('rejects missing prompt', async () => {
        ...
        const result = await tool.execute({ file_path: '/tmp/x.png' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
    ...
    it('rejects when no input key is set', async () => {
        ...
        const result = await tool.execute({ prompt: 'hi' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
        expect(result.code).toBe('input_invalid');
...
describe('vision_analyze — capability gate', () => {
    it('returns VISION_NOT_SUPPORTED for an image on a non-vision model', async () => {
        ...
        const result = await tool.execute({ file_path: path, prompt: 'what?' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
    ...
    it('returns PDF_NOT_SUPPORTED for a PDF on a vision-only model', async () => {
        ...
        const result = await tool.execute({ file_path: path, prompt: 'summary' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
        expect(result.code).toBe('not_available');
...
describe('vision_analyze — model fallback chain', () => {
    it('falls back to defaultModel when args.model and auxiliaryVisionModel are unset', async () => {
        ...
        const result = await tool.execute({ file_path: path, prompt: 'p' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        if (!result.ok)
            throw new Error(`unexpected failure: ${result.code} ${result.error}`);
    ...
    it('args.model overrides both auxiliaryVisionModel and defaultModel', async () => {
        ...
        const result = await tool.execute({ file_path: path, prompt: 'p', model: 'gpt-5' }, makeCtx({ workingDir: tmpDir, allowed: [tmpDir] }));
        if (!result.ok)
            throw new Error(`unexpected failure: ${result.code} ${result.error}`);
