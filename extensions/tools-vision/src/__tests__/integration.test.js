...
function buildRegistryWithVision(opts) {
    ...
    const tools = createVisionTools({
        resolveProvider: (model) => {
            if (model === opts.defaultModel)
                return opts.primary;
            if (opts.auxProvider && opts.auxModel && model === opts.auxModel)
                return opts.auxProvider;
            return null;
        },
    ...
    if (!tool)
        throw new Error('vision_analyze did not register');
...
describe('vision_analyze — P3 wiring integration', () => {
    it('routes through the wiring factory and is gated by the personality toolset', async () => {
        ...
        const results = await registry.executeParallel([{ toolCallId: 'tc-1', name: 'vision_analyze', args: { file_path: path, prompt: 'what?' } }], makeCtx(tmpDir), personalityToolset);
    ...
    it('reads a real JPEG via file_path through FsStorage (binary round-trip)', async () => {
        ...
        ], makeCtx(tmpDir), personalityToolset);
        const r = results[0]?.result;
        if (!r?.ok)
            throw new Error(`expected ok result, got ${JSON.stringify(r)}`);
    ...
    it('rejects vision_analyze when the personality toolset omits it', async () => {
        ...
        const results = await registry.executeParallel([{ toolCallId: 'tc-1', name: 'vision_analyze', args: { file_path: path, prompt: 'x' } }], makeCtx(tmpDir), ['read_file']);
        const r = results[0]?.result;
        if (!r || r.ok)
    ...
    it('routes to the auxiliary vision provider when auxiliary.vision.model is set', async () => {
        ...
        const results = await registry.executeParallel([{ toolCallId: 'tc-aux', name: 'vision_analyze', args: { file_path: path, prompt: 'q' } }], makeCtx(tmpDir), personalityToolset);
        const r = results[0]?.result;
        if (!r?.ok)
            throw new Error(`expected ok result, got ${JSON.stringify(r)}`);
        const envelope = JSON.parse(r.value);
