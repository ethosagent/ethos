...
/** Fields the adapter extracts from a button-click `MessageComponentInteraction`. */
export interface ClarifyButtonPayload {
  customId: string;
  /** Discord user id of whoever clicked (snowflake string). */
  userId: string;
  /** Channel id the message lives in. */
  channelId: string;
  /** Message id of the prompt the buttons are on. */
  messageId: string;
}

/** Fields the adapter extracts from a `ModalSubmitInteraction`. */
export interface ClarifyModalPayload {
  customId: string;
  /** Discord user id of the submitter. */
  userId: string;
  /** Channel id the modal originated in. */
  channelId: string;
  /** The user-entered text of the answer input, trimmed. Empty when missing. */
  answer: string;
}

export type ClarifyInteractionEvent =
  | {
      kind: 'choice';
      requestId: string;
      choiceIndex: number;
  ...
  | { kind: 'cancel'; requestId: string; userId: string; channelId: string; messageId: string }
  | {
      kind: 'open-modal';
      requestId: string;
      userId: string;
...
export interface ClarifyInteractionHandlers {
  onEvent: (event: ClarifyInteractionEvent) => Promise<void>;
}
...
export async function handleClarifyButton(
  payload: ClarifyButtonPayload,
  handlers: ClarifyInteractionHandlers,
): Promise<void> {
...
export async function handleClarifyModal(
  payload: ClarifyModalPayload,
  handlers: ClarifyInteractionHandlers,
): Promise<void> {
  if (!payload.userId) return;
  const parts = payload.customId.split(':');
