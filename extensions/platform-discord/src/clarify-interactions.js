...
import { CLARIFY_ANSWER_KIND, CLARIFY_BUTTON_PREFIX, CLARIFY_CANCEL_KIND, CLARIFY_CHOICE_KIND, CLARIFY_MODAL_INPUT_ID, CLARIFY_MODAL_KIND, } from './clarify-blocks';
...
export async function handleClarifyButton(payload, handlers) {
    ...
    if (parts.length < 3)
        return;
    if (parts[0] !== CLARIFY_BUTTON_PREFIX)
        return;
    const kind = parts[1];
...
export async function handleClarifyModal(payload, handlers) {
    ...
    if (parts.length !== 3)
        return;
    if (parts[0] !== CLARIFY_BUTTON_PREFIX || parts[1] !== CLARIFY_MODAL_KIND)
        return;
    const requestId = parts[2];
    if (!requestId)
        return;
