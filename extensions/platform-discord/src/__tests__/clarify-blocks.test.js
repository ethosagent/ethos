...
import { CLARIFY_ANSWER_KIND, CLARIFY_BUTTON_PREFIX, CLARIFY_CANCEL_KIND, CLARIFY_CHOICE_KIND, CLARIFY_MODAL_KIND, clarifyModalPayload, clarifyPendingMessage, clarifyResolvedMessage, escapeMd, } from '../clarify-blocks';
...
describe('clarifyPendingMessage', () => {
    it('renders option buttons + a Cancel row with custom_ids encoded as `clr:choice:<requestId>:<idx>`', () => {
        ...
        expect(cancelRow?.components[0]?.custom_id).toBe(`${CLARIFY_BUTTON_PREFIX}:${CLARIFY_CANCEL_KIND}:r1`);
    });
    it('renders Answer + Cancel for free-form (no options)', () => {
        ...
        expect(msg.components[0]?.components[0]?.custom_id).toBe(`${CLARIFY_BUTTON_PREFIX}:${CLARIFY_ANSWER_KIND}:r2`);
    });
