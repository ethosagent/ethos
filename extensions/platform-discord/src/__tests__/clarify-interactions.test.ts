...
function buttonPayload(customId: string, overrides: Record<string, string> = {}) {
  return {
    customId,
    userId: 'U1',
    channelId: 'C1',
...
describe('handleClarifyButton', () => {
  it('routes a valid choice click', async () => {
    const onEvent = vi.fn();
    await handleClarifyButton(
      buttonPayload(`${CLARIFY_BUTTON_PREFIX}:${CLARIFY_CHOICE_KIND}:r1:2`),
      { onEvent },
    );
  ...
  it('routes an open-modal click', async () => {
    ...
    expect(onEvent).toHaveBeenCalledWith(
      expect.objectContaining({ kind: 'open-modal', requestId: 'r1' }),
    );
  });
  ...
  it('drops anonymous clicks', async () => {
    const onEvent = vi.fn();
    await handleClarifyButton(
      { ...buttonPayload(`${CLARIFY_BUTTON_PREFIX}:${CLARIFY_CHOICE_KIND}:r1:0`), userId: '' },
      { onEvent },
    );
  ...
  it('swallows callback errors', async () => {
    ...
    await expect(
      handleClarifyButton(buttonPayload(`${CLARIFY_BUTTON_PREFIX}:${CLARIFY_CHOICE_KIND}:r1:0`), {
        onEvent,
      }),
...
describe('handleClarifyModal', () => {
  ...
  it('drops payloads with the wrong prefix or kind', async () => {
    const onEvent = vi.fn();
    await handleClarifyModal(
      { customId: 'wrong:prefix:r1', userId: 'U1', channelId: 'C1', answer: 'a' },
      { onEvent },
    );
    await handleClarifyModal(
      {
