...
describe('migrateSessionKeys (SQLite integration)', () => {
    ...
    function seed(rows) {
        ...
        try {
            const insert = db.prepare(`INSERT INTO sessions (id, key, platform, model, provider, created_at, updated_at)
    ...
    function readAllKeys() {
        ...
        try {
            const rows = db.prepare('SELECT key FROM sessions ORDER BY key').all();
    ...
    const known = new Map([['telegram', new Set(['t1key'])]]);
    const primary = new Map([['telegram', 't1key']]);
