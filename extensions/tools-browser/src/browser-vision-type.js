...
export function createBrowserVisionTypeTool(visionOpts) {
    return {
        name: 'browser_vision_type',
        description: 'Type text into an element described in natural language. Uses accessibility tree first, falls back to vision model.',
        toolset: 'browser',
        maxResultChars: 500,
        capabilities: {
        ...
        async execute(args, ctx) {
            const { description, text, submit = false, } = args;
            if (!description) {
            ...
            if (!session) {
                return {
                    ok: false,
                    error: 'No active browser session under the current network policy. Call browse_url first.',
