...
function scoreMatch(nodeName, description) {
...
export function resolveByA11y(snapshotText, description) {
    let bestScore = 0;
    let bestName = null;
...
export async function resolveByVision(screenshotB64, description, context, opts) {
    if (!opts.apiKey)
        return null;
    ...
    try {
        if (provider === 'anthropic') {
            ...
            const cost_usd = response.usage.input_tokens * 0.0000008 + response.usage.output_tokens * 0.000004;
