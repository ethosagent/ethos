...
// ---------------------------------------------------------------------------
export const browserPressTool = {
    name: 'browser_press',
    description: 'Send keyboard keys (Enter, Tab, Escape, ArrowDown, Ctrl+A, etc.). Useful for form submission, navigation, and shortcuts.',
    toolset: 'browser',
    ...
    schema: {
        ...
        properties: {
            key: {
                type: 'string',
                description: 'Key or key combination (e.g. "Enter", "Tab", "Escape", "Control+a", "Meta+c")',
            },
        },
        required: ['key'],
    },
...
export const browserScrollTool = {
    name: 'browser_scroll',
    description: 'Scroll the page in a given direction. Useful for viewing content below the fold or navigating long pages.',
    toolset: 'browser',
    maxResultChars: 20_000,
    capabilities: {
        network: { allowedHosts: ['*'] },
    ...
    async execute(args, ctx) {
        ...
        if (!direction)
            return { ok: false, error: 'direction is required', code: 'input_invalid' };
        const session = findActiveSession(ctx.sessionId, ctx.networkPolicy ?? {});
        if (!session) {
            return {
                ok: false,
...
export const browserConsoleTool = {
    ...
    description: 'Get browser console messages (log, warn, error, etc.) captured since last read. Useful for debugging JavaScript errors and application state.',
    toolset: 'browser',
    maxResultChars: 20_000,
    capabilities: {
        network: { allowedHosts: ['*'] },
...
// ---------------------------------------------------------------------------
export const browserGetImagesTool = {
    name: 'browser_get_images',
    description: 'List all images on the current page with their src, alt text, and dimensions. Useful for understanding page media content.',
    toolset: 'browser',
    maxResultChars: 20_000,
    capabilities: {
        network: { allowedHosts: ['*'] },
    ...
    async execute(_args, ctx) {
        ...
        try {
            ...
            const lines = images.map((img, i) => `[${i + 1}] ${img.alt ? `"${img.alt}"` : '(no alt)'} ${img.width}x${img.height} — ${img.src}`);
...
// ---------------------------------------------------------------------------
export const browserDialogTool = {
    name: 'browser_dialog',
    description: 'Check recent browser dialogs (alert, confirm, prompt). Dialogs are auto-handled to prevent page deadlock: alerts are accepted, confirms/prompts are dismissed. This tool reports what dialogs appeared.',
    toolset: 'browser',
    maxResultChars: 20_000,
    capabilities: {
        network: { allowedHosts: ['*'] },
...
export const browserNavigateTool = {
    ...
    description: 'Navigate a browser to a URL and return an accessibility tree with @e{n} element references. Alias for browse_url.',
    toolset: 'browser',
    maxResultChars: 20_000,
    capabilities: {
        network: { allowedHosts: ['*'] },
    ...
    async execute(args, ctx) {
        ...
        if (!url)
            return { ok: false, error: 'url is required', code: 'input_invalid' };
        let parsed;
        try {
            parsed = new URL(url);
        }
