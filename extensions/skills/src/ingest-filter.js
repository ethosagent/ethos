 ...
 */
let envResolverOpts;
export function setEnvResolverOptions(opts) {
 ...
 */
export function filterSkill(skill, personality, toolNames, onWarn) {
    const cfg = personality.skills?.global_ingest;
    const mode = cfg?.mode ?? 'capability';
    ...
    switch (mode) {
        ...
        default:
            return capabilityCheck(skill, toolNames, cfg?.fallback_unknown ?? 'allow', onWarn, personality);
    }
}
function capabilityCheck(skill, toolNames, fallback, onWarn, personality) {
    ...
    if (required.length === 0) {
        ...
        if (fallback === 'warn') {
            onWarn?.(`[boot] ${personality.id}: skill '${skill.qualifiedName}' has no required_tools — loading (fallback: warn)`);
        }
...
function checkToolReach(skill, toolNames, onWarn, personalityId) {
    // Merge top-level required_tools with permissions.tools_required
    const required = [...(skill.required_tools ?? []), ...(skill.permissions?.tools_required ?? [])];
    if (required.length === 0)
        return null;
    const missing = required.filter((t) => !toolNames.has(t));
    if (missing.length > 0) {
        onWarn?.(`[boot] ${personalityId}: skill '${skill.qualifiedName}' allowed but requires tool '${missing[0]}' ` +
...
function checkEnv(skill, onWarn, personalityId) {
    ...
    if (result.missingEnv.length > 0)
        parts.push(`env unset: ${result.missingEnv.join(', ')}`);
 ...
 */
function checkFallbackForTools(skill, toolNames) {
 ...
 */
function disallowedValues(declared, policy) {
    if (!policy)
        return [...declared];
    if (policy === true)
        return [];
    return declared.filter((v) => !policy.includes(v));
}
/**
 ...
 * network category.
 */
function checkSkillPermissions(skill, personality, onWarn) {
    ...
    if (perms.fs_read && perms.fs_read.length > 0) {
        ...
        onWarn?.(`[boot] ${personalityId}: skill '${skill.qualifiedName}' declares fs_read: [${perms.fs_read.join(', ')}]`);
    }
    if (perms.fs_write && perms.fs_write.length > 0) {
        ...
        onWarn?.(`[boot] ${personalityId}: skill '${skill.qualifiedName}' declares fs_write: [${perms.fs_write.join(', ')}]`);
    }
    if (perms.network && perms.network.length > 0) {
        ...
        onWarn?.(`[boot] ${personalityId}: skill '${skill.qualifiedName}' declares network access: [${perms.network.join(', ')}]`);
    }
    if (perms.mcp_env_passthrough && perms.mcp_env_passthrough.length > 0) {
        ...
        onWarn?.(`[boot] ${personalityId}: skill '${skill.qualifiedName}' requests MCP env passthrough: [${perms.mcp_env_passthrough.join(', ')}]`);
    }
    return null;
}
/**
 ...
 */
export function warnMissingAllowList(personalityId, allow, pool, onWarn) {
    for (const name of allow) {
        if (!pool.has(name)) {
            onWarn(`[boot] ${personalityId}: skill '${name}' referenced but not found in any source — skipping`);
        }
    }
}
