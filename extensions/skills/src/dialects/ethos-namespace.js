...
export function parseEthosPermissions(data) {
  ...
  const p = perms;
  const result = {};
  if (Array.isArray(p.fs_read)) result.fs_read = p.fs_read.filter((x) => typeof x === 'string');
  if (Array.isArray(p.fs_write)) result.fs_write = p.fs_write.filter((x) => typeof x === 'string');
  if (Array.isArray(p.network)) result.network = p.network.filter((x) => typeof x === 'string');
