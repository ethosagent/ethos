...
export class SkillsInjector {
    ...
    constructor(personalities, optionsOrDir) {
        ...
        const opts = typeof optionsOrDir === 'string' ? { globalSkillsDir: optionsOrDir } : (optionsOrDir ?? {});
    ...
    async resolveSkills(personalityId) {
        ...
        if (allow.length > 0) {
            warnMissingAllowList(personality.id, allow, globalPool, (msg) => process.stdout.write(`${msg}\n`));
        }
        const toolNames = this.toolNamesForPersonality
            ? this.toolNamesForPersonality(personality)
            : new Set();
        for (const [, skill] of globalPool) {
            if (perPersonalityDirs.some((d) => skill.filePath.startsWith(d)))
                continue;
            const result = filterSkill(skill, personality, toolNames, (msg) => process.stdout.write(`${msg}\n`));
            if (!result.include) {
                this.onSkip?.(skill.qualifiedName, result.reason);
                continue;
            }
            if (skill.dialect === 'openclaw') {
                const parsed = parseSkillFrontmatter(skill.body.length > 0
    ...
    async inject(ctx) {
        ...
        function addSection(content, name) {
            if (budget > 0 && totalChars + content.length > budget)
                return false;
        ...
        if (injectionMode === 'index') {
            ...
            if (eligibleGlobal.length > 0) {
                const rows = eligibleGlobal
                    ...
                    .join('\n');
                const indexBlock = `## Available Skills\n\n` +
            ...
            if (budgetExceeded) {
                process.stdout.write(`[skills] injection budget (${budget} chars) reached — some global-pool skills were skipped. ` +
