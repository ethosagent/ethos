...
describe('SkillsLibrary', () => {
    ...
    describe('listSkills', () => {
        ...
        it('parses frontmatter and returns sorted by name', async () => {
            ...
            await storage.write(join(DATA, 'skills', 'zebra.md'), '---\nname: Zebra skill\ndescription: about zebras\n---\n\nbody');
    ...
    describe('approvePending', () => {
        it('moves the file from .pending into the live dir', async () => {
            ...
            await storage.write(join(DATA, 'skills', '.pending', 'cand.md'), '---\nname: Candidate\n---\n\nthe body');
    ...
    describe('listPending', () => {
        it('returns the .pending dir contents, newest first', async () => {
            await storage.mkdir(join(DATA, 'skills', '.pending'));
            await storage.write(join(DATA, 'skills', '.pending', 'first.md'), '---\nname: First\n---\n\na');
            ...
            await storage.write(join(DATA, 'skills', '.pending', 'second.md'), '---\nname: Second\n---\n\nb');
    ...
    describe('per-personality skills', () => {
        it('writes under personalities/<id>/skills/', async () => {
            ...
            expect(await storage.read(join(DATA, 'personalities', 'p', 'skills', 'note.md'))).toContain('name: A note');
        });
        it('importGlobalIntoPersonality copies global into per-personality dir byte-for-byte', async () => {
            ...
            expect(await storage.read(join(DATA, 'personalities', 'p', 'skills', 'shared.md'))).toContain('name: Shared');
        });
    ...
    describe('system skills (catalogDir)', () => {
        ...
        beforeEach(async () => {
            ...
            await storage.write(join(CATALOG, 'web-search', 'SKILL.md'), '---\nname: Web Search\ndescription: Search the web\n---\n\nSearch body');
            await storage.mkdir(join(CATALOG, 'summarize'));
            await storage.write(join(CATALOG, 'summarize', 'SKILL.md'), '---\nname: Summarize\n---\n\nSummarize body');
        });
        ...
        it('discovers skills nested under category directories', async () => {
            ...
            await storage.write(join(CATALOG, 'github', 'create-ticket', 'SKILL.md'), '---\nname: Create Ticket\ndescription: Create a GitHub issue\n---\n\nCreate ticket body');
            await storage.mkdir(join(CATALOG, 'github', 'debug-issue'));
            await storage.write(join(CATALOG, 'github', 'debug-issue', 'SKILL.md'), '---\nname: Debug Issue\n---\n\nDebug body');
            const skills = await catLib.listSkills();
        ...
        it('throws SKILL_EXISTS when two categories have the same leaf skill id', async () => {
            ...
            await storage.write(join(CATALOG, 'linear', 'create-ticket', 'SKILL.md'), '---\nname: Linear Create\n---\n\nbody');
