...
describe('shouldInject', () => {
    ...
    it('injects when env vars are all present', () => {
        expect(shouldInject({ requires: { env: ['EXAMPLE_KEY'] } }, { env: { EXAMPLE_KEY: 'x' } })).toEqual({ inject: true });
    });
    ...
    it('skips when none of the required bins are present', () => {
        const verdict = shouldInject({ requires: { bins: ['__definitely_not_a_real_binary__'] } }, { env: { PATH: '/nonexistent' } });
        expect(verdict.inject).toBe(false);
...
describe('applySubstitutions', () => {
    it('replaces ETHOS_SKILL_DIR and ETHOS_SESSION_ID', () => {
        ...
        expect(applySubstitutions(input, '/skills/foo', 'sess-123')).toBe('Run /skills/foo/script.sh in session sess-123.');
    });
