...
describe('Step 4 — explicit mode allow list', () => {
    ...
    it('explicit mode with only summarize-doc in allow: shows just that skill', async () => {
        ...
        expect(filterSkill(summarize, researcher, toolNames).include).toBe(true);
        expect(filterSkill(deploy, researcher, toolNames).include).toBe(false);
        expect(filterSkill(deploy, researcher, toolNames).reason).toBe('not in allow list (mode: explicit)');
    });
    it('missing-reference warning fires for allow-listed skill absent from pool', async () => {
        ...
        warnMissingAllowList('researcher', ['ethos/summarize-doc', 'ethos/nonexistent'], pool, (m) => warnings.push(m));
...
describe('Step 5 — cross-source discovery from ~/.claude/skills/', () => {
    ...
    it('discovers citation-formatter from claude-code source', async () => {
        ...
        const skill = must(pool.get('claude-code/citation-formatter'), 'missing claude-code/citation-formatter');
...
describe('Unreadable skill source is skipped, not fatal', () => {
    ...
    it('EACCES on one source does not poison the scan and is reported via onSkip', async () => {
        ...
        const skip = must(skipped.find((s) => s.dir === BAD_SKILLS), 'expected onSkip for the unreadable source');
