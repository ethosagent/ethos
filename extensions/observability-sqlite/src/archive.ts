...
function parseJsonl(buf: Buffer | undefined): Record<string, unknown>[] {
...
export async function archiveMonth(
  ...
  try {
    ...
    const snapshotIds = [
      ...new Set(traces.map((t) => t.snapshot_id as string | null).filter(Boolean)),
    ] as string[];
...
export async function restoreArchive(
  ...
  try {
    const result: ArchiveResult = { traces: 0, spans: 0, events: 0, snapshots: 0 };
...
export async function listArchives(
  storage: Storage,
  archiveDir: string,
): Promise<Array<{ month: string; path: string }>> {
