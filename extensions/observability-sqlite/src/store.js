...
export class SQLiteObservabilityStore {
    ...
    insertTrace(trace) {
        ...
        this.db
         ...
         VALUES (?,?,?,?,?,?,?,?,?)`)
            .run(trace.traceId, trace.sessionId ?? null, trace.kind, trace.startTs, trace.endTs ?? null, trace.status ?? null, trace.subjectId ?? null, trace.snapshotId ?? null, attrsJson);
    }
    ...
    insertSpan(span, extraRedactPatterns) {
        ...
        this.db
         ...
         VALUES (?,?,?,?,?,?,?,?,?)`)
            .run(span.spanId, span.traceId, span.parentSpanId ?? null, span.kind, span.name, span.startTs, span.endTs ?? null, span.status ?? null, attrsJson);
    }
    ...
    insertEvent(event, extraRedactPatterns) {
        ...
        this.db
         ...
         VALUES (?,?,?,?,?,?,?,?,?)`)
            .run(event.eventId, event.traceId ?? null, event.spanId ?? null, event.ts, event.category, event.severity, event.code ?? null, cause, detailsJson);
    }
    getEvents(filter) {
        const conditions = [];
        const values = [];
