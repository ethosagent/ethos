...
export async function archiveMonth(dbPath, storage, archiveDir, month) {
    ...
    try {
        ...
        const traces = db
            ...
            .all(since, until);
        if (traces.length === 0)
            return { traces: 0, spans: 0, events: 0, snapshots: 0 };
        const traceIds = traces.map((t) => String(t.trace_id));
        const ph = traceIds.map(() => '?').join(',');
        const spans = db
            .prepare(`SELECT * FROM spans WHERE trace_id IN (${ph})`)
            .all(...traceIds);
        const events = db
            .prepare(`SELECT * FROM events WHERE trace_id IN (${ph})`)
            .all(...traceIds);
        const snapshotIds = [
            ...new Set(traces.map((t) => t.snapshot_id).filter(Boolean)),
        ];
        const snapshots = snapshotIds.length > 0
            ? db
                .prepare(`SELECT * FROM snapshots WHERE snapshot_id IN (${snapshotIds.map(() => '?').join(',')})`)
                .all(...snapshotIds)
            : [];
        const files = new Map();
        ...
        files.set('snapshots.jsonl', Buffer.from(snapshots.map((r) => JSON.stringify(r)).join('\n'), 'utf8'));
        ...
        const verifiedTraceCount = verified.get('traces.jsonl')?.toString('utf8').split('\n').filter(Boolean).length ?? 0;
        if (verifiedTraceCount !== traces.length) {
            throw new Error(`Archive integrity check failed: expected ${traces.length} traces, archive contains ${verifiedTraceCount}`);
        }
        ...
        db.transaction(() => {
            ...
            db.prepare('DELETE FROM snapshots WHERE snapshot_id NOT IN (SELECT DISTINCT snapshot_id FROM traces WHERE snapshot_id IS NOT NULL)').run();
        })();
        return {
            traces: traces.length,
            spans: spans.length,
...
export async function restoreArchive(dbPath, storage, archiveDir, month) {
    ...
    if (!raw)
        throw new Error(`Archive not found: ${month}.tar.gz`);
    ...
    try {
        ...
        db.transaction(() => {
            const insertTrace = db.prepare('INSERT OR IGNORE INTO traces (trace_id, session_id, kind, start_ts, end_ts, status, subject_id, snapshot_id, attrs) VALUES (?,?,?,?,?,?,?,?,?)');
            const insertSpan = db.prepare('INSERT OR IGNORE INTO spans (span_id, trace_id, parent_span_id, kind, name, start_ts, end_ts, status, attrs) VALUES (?,?,?,?,?,?,?,?,?)');
            const insertEvent = db.prepare('INSERT OR IGNORE INTO events (event_id, trace_id, span_id, ts, category, severity, code, cause, details) VALUES (?,?,?,?,?,?,?,?,?)');
            const insertSnapshot = db.prepare('INSERT OR IGNORE INTO snapshots (snapshot_id, taken_at, subject_id, body) VALUES (?,?,?,?)');
            for (const t of traces) {
                insertTrace.run(t.trace_id, t.session_id ?? null, t.kind, t.start_ts, t.end_ts ?? null, t.status ?? null, 
                // Pre-rename archives stored `personality_id`; accept either key.
                t.subject_id ?? t.personality_id ?? null, t.snapshot_id ?? null, t.attrs ?? null);
                result.traces++;
            }
            for (const s of spans) {
                insertSpan.run(s.span_id, s.trace_id, s.parent_span_id ?? null, s.kind, s.name, s.start_ts, s.end_ts ?? null, s.status ?? null, s.attrs ?? null);
                result.spans++;
            }
            for (const e of events) {
                insertEvent.run(e.event_id, e.trace_id ?? null, e.span_id ?? null, e.ts, e.category, e.severity, e.code ?? null, e.cause ?? null, e.details ?? null);
                result.events++;
            }
            for (const snap of snapshots) {
                insertSnapshot.run(snap.snapshot_id, snap.taken_at, 
                // Pre-rename archives stored `personality_id`; accept either key.
                snap.subject_id ?? snap.personality_id, snap.body);
                result.snapshots++;
            }
        })();
        return result;
