...
describe('createTarGz / readTarGz', () => {
  ...
  it('handles empty file content', () => {
    const files = new Map<string, Buffer>([['empty.txt', Buffer.alloc(0)]]);
    const output = readTarGz(createTarGz(files));
  ...
  it('handles binary content round-trip', () => {
    ...
    const files = new Map<string, Buffer>([['data.bin', binary]]);
    const output = readTarGz(createTarGz(files));
  ...
  it('produces a smaller buffer than input (gzip compression active)', () => {
    ...
    const files = new Map<string, Buffer>([['big.txt', content]]);
...
describe('SQLiteObservabilityStore Wave C queries', () => {
  ...
  function insertTrace(overrides: Partial<Trace> = {}): Trace {
    const t: Trace = {
      traceId: randomUUID(),
      kind: 'turn',
      startTs: Date.now(),
  ...
  function insertSpan(traceId: string, overrides: Partial<Span> = {}): Span {
    const s: Span = {
      spanId: randomUUID(),
      traceId,
      kind: 'tool_call',
  ...
  function insertEvent(traceId: string, overrides: Partial<ObsEvent> = {}): ObsEvent {
    const e: ObsEvent = {
      eventId: randomUUID(),
      traceId,
      ts: Date.now(),
...
describe('archiveMonth / restoreArchive', () => {
  ...
  it('archives completed traces from the target month and removes them from the live DB', async () => {
    ...
    const t3: Trace = { traceId: randomUUID(), kind: 'turn', startTs: june1, endTs: june1 + 1000 }; // June — not archived
  ...
  it('restore is idempotent (INSERT OR IGNORE)', async () => {
    ...
    const t: Trace = { traceId: randomUUID(), kind: 'turn', startTs: ts, endTs: ts + 1 };
    store.insertTrace(t);
    store.close();
...
describe('support bundle privacy invariants', () => {
  it('stripSecrets removes keys matching secret pattern from nested objects', () => {
    ...
    const safe = strip(sensitiveData) as Record<string, unknown>;
    ...
    expect((safe.nested as Record<string, unknown>).name).toBe('engineer');
  });
