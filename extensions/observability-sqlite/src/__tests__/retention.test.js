...
describe('pruneObservability', () => {
  ...
  it('actually deletes old rows and keeps recent ones', () => {
    ...
    expect(result.spans).toBe(1);
    expect(result.events).toBe(1);
    const remainingTraces = db.prepare('SELECT COUNT(*) as n FROM traces').get().n;
    const remainingSpans = db.prepare('SELECT COUNT(*) as n FROM spans').get().n;
    const remainingEvents = db.prepare('SELECT COUNT(*) as n FROM events').get().n;
  ...
  it('excludeSubjectIds: global pass does not delete rows belonging to excluded subjects', () => {
    ...
    const remaining = db
      .prepare('SELECT trace_id FROM traces')
      .all()
      .map((r) => r.trace_id);
  ...
  it('messages pruning uses ISO timestamp column and prunes old messages', () => {
    ...
    const remaining = sessDb.prepare('SELECT COUNT(*) as n FROM messages').get().n;
