...
function insertTrace(id, startTs) {
    db.prepare(`INSERT INTO traces (trace_id, kind, start_ts) VALUES (?, 'turn', ?)`).run(id, startTs);
}
function insertSpan(id, traceId, startTs) {
    db.prepare(`INSERT INTO spans (span_id, trace_id, kind, name, start_ts) VALUES (?, ?, 'internal', 'test', ?)`).run(id, traceId, startTs);
}
function insertEvent(id, category, ts) {
    db.prepare(`INSERT INTO events (event_id, category, severity, ts) VALUES (?, ?, 'info', ?)`).run(id, category, ts);
}
describe('pruneObservability', () => {
    ...
    it('actually deletes old rows and keeps recent ones', () => {
        ...
        expect(result.spans).toBe(1);
        expect(result.events).toBe(1);
        const remainingTraces = db.prepare('SELECT COUNT(*) as n FROM traces').get()
            .n;
        const remainingSpans = db.prepare('SELECT COUNT(*) as n FROM spans').get().n;
        const remainingEvents = db.prepare('SELECT COUNT(*) as n FROM events').get()
            .n;
    ...
    it('respects "forever" — install events are never deleted', () => {
        ...
        const remaining = db.prepare('SELECT COUNT(*) as n FROM events').get().n;
    ...
    it('orphaned snapshots are pruned; snapshots referenced by surviving traces are kept', () => {
        ...
        db.prepare(`INSERT INTO traces (trace_id, kind, start_ts, snapshot_id) VALUES (?, 'turn', ?, ?)`).run('old-trace', OLD, 'snap-A');
        db.prepare(`INSERT INTO traces (trace_id, kind, start_ts, snapshot_id) VALUES (?, 'turn', ?, ?)`).run('new-trace', RECENT, 'snap-B');
        for (const [id] of [['snap-A'], ['snap-B'], ['snap-C']]) {
            db.prepare(`INSERT INTO snapshots (snapshot_id, taken_at, subject_id, body) VALUES (?, ?, 'eng', '{}')`).run(id, RECENT);
        }
        const result = pruneObservability(db, RETENTION_DEFAULTS, { dryRun: false, now: NOW });
        expect(result.traces).toBe(1); // old-trace pruned
        expect(result.snapshots).toBe(2); // snap-A (now orphaned) + snap-C (always orphaned)
        const snaps = db.prepare('SELECT snapshot_id FROM snapshots').all().map((r) => r.snapshot_id);
    ...
    it('per-subject retention override: longer TTL prevents pruning', () => {
        ...
        const remaining = db.prepare('SELECT COUNT(*) as n FROM traces').get().n;
        expect(remaining).toBe(1);
    });
    ...
    it('excludeSubjectIds: global pass does not delete rows belonging to excluded subjects', () => {
        ...
        db.prepare(`INSERT INTO traces (trace_id, kind, start_ts, subject_id) VALUES (?, 'turn', ?, ?)`).run('a-trace', OLD, 'subject-a');
        ...
        const remaining = db.prepare('SELECT trace_id FROM traces').all().map((r) => r.trace_id);
    ...
    `);
        ...
        const remaining = sessDb.prepare('SELECT COUNT(*) as n FROM messages').get()
            .n;
...
describe('pruneObservability — referenced blobs survive', () => {
    ...
    `);
        ...
        obsDb
            ...
            .run(OLD);
        obsDb
            .prepare(`INSERT INTO spans (span_id, trace_id, kind, name, start_ts, attrs) VALUES ('old-s', 'old-t', 'tool_call', 'bash', ?, ?)`)
        ...
        obsDb
            ...
            .run(RECENT);
        obsDb
            .prepare(`INSERT INTO spans (span_id, trace_id, kind, name, start_ts, attrs) VALUES ('new-s', 'new-t', 'tool_call', 'bash', ?, ?)`)
