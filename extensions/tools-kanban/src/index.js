export { registerPostmortemHandler } from './postmortem';
export { createKanbanRoleGateHook, } from './role-gate';
...
const WORKSPACE_MODES = ['scratch', 'worktree', 'dir'];
function isStatus(s) {
    return typeof s === 'string' && STATUS_VALUES.includes(s);
}
function isWorkspaceMode(s) {
    return typeof s === 'string' && WORKSPACE_MODES.includes(s);
}
function summariseTask(t) {
    return {
        id: t.id,
        title: t.title,
        status: t.status,
...
function storeError(err) {
    ...
    if (/no open run:/.test(msg))
        return errorResult(msg, 'execution_failed');
    if (/not found/.test(msg))
        return errorResult(msg, 'input_invalid');
    return errorResult(msg, 'execution_failed');
}
function createKanbanCreate(store) {
    return {
        name: 'kanban_create',
        description: 'Create a new durable task. Returns { task_id, status }. ' +
        ...
        schema: {
            ...
            properties: {
                ...
                max_retries: {
                    type: ['integer', 'null'],
                    description: 'Retry budget. Omit (or null) for unlimited retries. When set, the task is ' +
                        'moved to status=failed once it has been re-claimed more than this many times.',
                },
                acceptance_criteria: {
                    type: 'string',
                    description: 'Optional. What "done" means for this task — checked by a before_ticket_complete ' +
        ...
        async execute(rawArgs, ctx) {
            ...
            if (args.scheduled_for !== undefined &&
            ...
            if (args.workspace_mode !== undefined && !isWorkspaceMode(args.workspace_mode)) {
                return errorResult(`workspace_mode must be one of ${WORKSPACE_MODES.join(', ')}`, 'input_invalid');
            }
...
function createKanbanCreateGoal(store) {
    return {
        name: 'kanban_create_goal',
        description: 'Create a top-level GOAL. A goal is a kanban task with no assignee — the goal itself is never executed; its children are. Use this when the human gives you a multi-part objective, then call `kanban_create` once per child sub-task with `parents=[goal_id]` and `assignee=<specialist>`. Returns { task_id, status }.\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
...
function tooLong(field, value, cap) {
    if (value.length > cap) {
        return errorResult(`${field} too long (${value.length} > ${cap} chars)`, 'input_invalid');
    }
    return null;
}
function createKanbanList(store) {
    return {
        name: 'kanban_list',
        description: `List tasks (default ${LIST_DEFAULT_LIMIT}, hard max ${LIST_MAX_LIMIT}). Filters (assignee, status, parent_id, q) AND together. q is an FTS5 query over title+body+comments.\n` +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
        ...
        async execute(rawArgs) {
            ...
            if (args.q !== undefined) {
                ...
                if (args.q.length === 0) {
                    return errorResult('q must be non-empty (empty FTS phrase is not a useful query)', 'input_invalid');
                }
            }
...
// kanban_show
// ---------------------------------------------------------------------------
function createKanbanShow(store) {
    return {
        name: 'kanban_show',
        description: 'Full view of one task: comments, last 10 runs, last 20 events. Use before deciding next action on a long-lived task.\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
...
// ---------------------------------------------------------------------------
function createKanbanUpdateStatus(store) {
    return {
        name: 'kanban_update_status',
        description: 'Set a task to a new status. Setting status="running" auto-opens a task run; use kanban_complete / kanban_block to close it.\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
...
// kanban_comment
// ---------------------------------------------------------------------------
function createKanbanComment(store) {
    return {
        name: 'kanban_comment',
...
// ---------------------------------------------------------------------------
function createKanbanComplete(store, hooks, autonomyTierOf) {
    return {
        ...
        async execute(rawArgs, ctx) {
            ...
            try {
                ...
                if (hooks !== undefined) {
                    ...
                    const assigneeInfo = task.assignee && autonomyTierOf ? autonomyTierOf(task.assignee) : undefined;
...
function createKanbanBlock(store) {
    return {
        name: 'kanban_block',
        description: 'End the open run with outcome=blocked, set status=blocked. Reason is recorded as a comment for context.\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
...
function createKanbanUnblock(store) {
    return {
        name: 'kanban_unblock',
        description: 'Flip a blocked task to ready (if all parents are done/archived) or todo (if any parent is still pending).\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
        ...
        async execute(rawArgs, ctx) {
            ...
            if (task.status !== 'blocked') {
                return errorResult(`task ${args.task_id} is not blocked (status=${task.status}); use kanban_update_status to change other statuses`, 'execution_failed');
            }
            ...
            const next = allParentsDone ? 'ready' : 'todo';
            try {
...
// ---------------------------------------------------------------------------
function createKanbanHeartbeat(store) {
    return {
...
function createKanbanLink(store) {
    return {
        name: 'kanban_link',
        description: 'Add a parent → child edge. Rejected with a cycle error if it would close one. Use to express "X must finish before Y".\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
...
function createKanbanAssign(store) {
    return {
        name: 'kanban_assign',
...
function createKanbanArchive(store) {
    return {
        name: 'kanban_archive',
        description: 'Soft-delete the task by setting status=archived. The audit trail is preserved; use this instead of deletion.\n' +
            RULES,
        toolset: 'kanban',
        maxResultChars: MAX_RESULT_CHARS,
        capabilities: {},
