...
function makeTickets(count: number, status = 'todo') {
...
describe('kanbanListReducer', () => {
  it('≤10 tickets → passes through unchanged', () => {
    ...
    const result: ToolResult = { ok: true, value: JSON.stringify(tickets) };
  ...
  it('non-JSON result → passes through unchanged', () => {
    const result: ToolResult = { ok: true, value: 'not json at all' };
    const reduced = kanbanListReducer.reduce(result, ctx);
    expect(reduced).toEqual(result);
  });

  it('non-array JSON result → passes through unchanged', () => {
    const result: ToolResult = { ok: true, value: JSON.stringify({ foo: 'bar' }) };
    const reduced = kanbanListReducer.reduce(result, ctx);
    expect(reduced).toEqual(result);
  });

  it('error result → passes through unchanged', () => {
    const result: ToolResult = { ok: false, error: 'store error', code: 'execution_failed' };
    const reduced = kanbanListReducer.reduce(result, ctx);
    expect(reduced).toEqual(result);
  });
