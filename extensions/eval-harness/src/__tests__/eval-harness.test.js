...
function makeLoop(responsesByTaskId) {
    let callIndex = 0;
    return {
...
function makeLoopWithMeta(responsesByTaskId, skillFiles) {
    return {
        run: async function* (_prompt, opts) {
            const taskId = opts.sessionKey?.replace('eval:', '') ?? '';
            const text = responsesByTaskId[taskId] ?? '';
            yield { type: 'context_meta', data: { skillFilesUsed: skillFiles } };
...
describe('exactMatchScorer', () => {
    const expected = { id: 't1', expected: 'hello world' };
...
describe('containsScorer', () => {
    const expected = { id: 't1', expected: '42' };
...
describe('EvalRunner', () => {
    it('scores a passing task as 1 and failing task as 0', async () => {
        ...
        const expectedMap = parseExpectedJsonl('{"id":"task1","expected":"42"}\n{"id":"task2","expected":"42"}');
