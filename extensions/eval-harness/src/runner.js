...
class Writer {
    ...
    async init(truncate) {
        if (truncate)
            await this.storage.write(this.path, '');
    }
    append(record) {
        this.chain = this.chain.then(() => this.storage.append(this.path, `${JSON.stringify(record)}\n`));
        return this.chain;
    }
}
class Semaphore {
    ...
    resolveScorer(match) {
        ...
        switch (kind) {
            ...
            case 'llm': {
                ...
                if (!llmProvider)
                    throw new Error('llm scorer requires llmProvider in EvalRunOptions');
...
export function parseExpectedJsonl(src) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (typeof obj.id !== 'string' || typeof obj.expected !== 'string') {
            throw new Error(`Line ${i + 1}: each expected record must have string fields "id" and "expected"`);
        }
