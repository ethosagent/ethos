...
beforeEach(() => {
    dataDir = join(tmpdir(), `ethos-lifecycle-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('reconcileRegistry', () => {
    it('flips a running entry with a dead pid to orphan and bumps lastTouchedAt', async () => {
        ...
        expect(new Date(after?.lastTouchedAt ?? 0).getTime()).toBeGreaterThan(new Date(stale).getTime());
    });
    ...
    it('is a no-op and does not throw on a missing registry', async () => {
        const emptyDir = join(tmpdir(), `ethos-lifecycle-empty-${Date.now()}-${Math.random().toString(36).slice(2)}`);
