...
import { loadRegistry, reapStale, saveRegistry, updateEntry, updateEntryIf, withRegistryLock, } from '../registry';
let dataDir;
...
describe('loadRegistry / saveRegistry', () => {
    ...
    it('round-trips a registry through save and load', () => {
        const reg = { a: makeEntry('a'), b: makeEntry('b') };
...
describe('updateEntry', () => {
    it('applies a patch and bumps lastTouchedAt', async () => {
        ...
        expect(new Date(entry?.lastTouchedAt ?? 0).getTime()).toBeGreaterThan(new Date('2000-01-01T00:00:00.000Z').getTime());
    });
...
describe('withRegistryLock', () => {
    ...
    it('releases the lock even when the callback throws', async () => {
        await expect(withRegistryLock(dataDir, async () => {
    ...
    it('serializes concurrent read-modify-write so no entries are lost', async () => {
        const N = 30;
        await Promise.all(Array.from({ length: N }, (_, i) => withRegistryLock(dataDir, async () => {
            const reg = loadRegistry(dataDir);
