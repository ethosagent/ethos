 ...
 */
function boundaryStorage(allowed) {
    const isAllowed = (p) => allowed.some((a) => p === a || p.startsWith(`${a}/`));
    const denyRead = (p) => {
        if (!isAllowed(p))
            throw new BoundaryError('read', p, allowed);
    };
    const denyWrite = (p) => {
        if (!isAllowed(p))
            throw new BoundaryError('write', p, allowed);
    };
    return {
        async read(p) {
            denyRead(p);
...
describe('process_start', () => {
    ...
    it('uses a custom name when provided', async () => {
        const start = getTool(tools, 'process_start');
        const result = await start.execute({ command: 'sleep 30', name: 'my-server' }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('records started_by from ctx.personalityId', async () => {
        ...
        if (!result.ok)
            return;
        const { id, pid } = JSON.parse(result.value);
    ...
    it('cap is per-personality: personality A at the cap does not block personality B', async () => {
        ...
        if (!resultB.ok)
            return;
        const { pid } = JSON.parse(resultB.value);
        process.kill(pid, 'SIGKILL');
    });
    it('honors a custom capMax via createProcessTools opts', async () => {
        ...
        if (!first.ok)
            return;
        const { pid } = JSON.parse(first.value);
        const second = await start.execute({ command: 'echo hi' }, ctx);
        expect(second.ok).toBe(false);
        if (!second.ok)
            expect(second.error).toContain('PROCESS_CAP_EXCEEDED');
        process.kill(pid, 'SIGKILL');
    });
    it('falls back to the default cap when capMax is not a positive integer', async () => {
        ...
        for (const bad of [0, -1, Number.NaN]) {
            const dir = join(tmpdir(), `ethos-proc-cap-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it('allows an explicit cwd that is inside the personality allowlist', async () => {
        ...
        if (!result.ok)
            return;
        const { pid } = JSON.parse(result.value);
        process.kill(pid, 'SIGKILL');
    });
...
describe('process_list', () => {
    it('shows a running process', async () => {
        ...
        const startResult = await start.execute({ command: 'sleep 30', name: 'sleeper' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id, pid } = JSON.parse(startResult.value);
        const listResult = await list.execute({}, makeCtx(workDir));
        expect(listResult.ok).toBe(true);
        if (!listResult.ok)
            return;
        const items = JSON.parse(listResult.value);
...
describe('process_logs', () => {
    it('returns last N lines from stdout', async () => {
        ...
        const startResult = await start.execute({ command: 'for i in 1 2 3 4 5; do echo "line$i"; done; sleep 30' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id, pid } = JSON.parse(startResult.value);
...
describe('process_stop', () => {
    it('stops a running process with SIGTERM', async () => {
        ...
        if (!stopResult.ok)
            return;
        const data = JSON.parse(stopResult.value);
...
describe('error-code prefixes', () => {
    ...
    it('process_start prefixes a spawn failure with SPAWN_FAILED', async () => {
        ...
        const result = await start.execute({ command: 'echo hi', cwd: join(workDir, 'missing-dir-xyz') }, makeCtx(workDir));
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.error).toContain('SPAWN_FAILED');
            expect(result.code).toBe('execution_failed');
    ...
    it('process_stop rejects an unsupported signal with SIGNAL_NOT_SUPPORTED', async () => {
        ...
        const { id, pid } = JSON.parse(startResult.value);
        const stop = getTool(tools, 'process_stop');
        const result = await stop.execute({ id, signal: 'SIGHUP' }, makeCtx(workDir));
        expect(result.ok).toBe(false);
        if (!result.ok) {
...
describe('process_watch', () => {
    it('returns PROCESS_NOT_FOUND for a non-existent id', async () => {
        ...
        const result = await watch.execute({ id: 'does-not-exist', patterns: ['hello'] }, makeCtx(workDir));
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.error).toContain('PROCESS_NOT_FOUND');
            expect(result.code).toBe('execution_failed');
    ...
    it('matches a substring pattern from stdout', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'echo "listening on port 3000"' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['listening on port'], timeout_s: 5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('matches a regex pattern', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'echo "error TS2345: something"' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['/error TS\\d+/'], timeout_s: 5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('returns timed_out when no pattern matches within timeout', async () => {
        ...
        if (!startResult.ok)
            return;
        const { id, pid } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['will-never-match'], timeout_s: 0.5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('collects multiple matches with stop_on_first_match: false', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'echo "match-A"; echo "match-B"; echo "match-C"' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['match-'], stop_on_first_match: false, timeout_s: 5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('returns process_exited when process exits before matching', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'echo "nothing relevant"; exit 0' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['will-not-match'], timeout_s: 5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('streams filter: watching only stdout does not pick up stderr matches', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'echo "stderr-marker" >&2; echo "stdout-marker"' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['stderr-marker'], streams: 'stdout', timeout_s: 3 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
    ...
    it('handles output split across multiple writes', async () => {
        const start = getTool(tools, 'process_start');
        const watch = getTool(tools, 'process_watch');
        const startResult = await start.execute({ command: 'printf "partial-"; sleep 0.3; echo "complete"' }, makeCtx(workDir));
        expect(startResult.ok).toBe(true);
        if (!startResult.ok)
            return;
        const { id } = JSON.parse(startResult.value);
        const result = await watch.execute({ id, patterns: ['partial-complete'], timeout_s: 5 }, makeCtx(workDir));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        const data = JSON.parse(result.value);
