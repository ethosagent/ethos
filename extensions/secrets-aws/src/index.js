...
export class AwsSecretsManagerResolver {
    ...
    async get(ref) {
        ...
        try {
            ...
            if (isAwsError(err, 'AccessDeniedException')) {
                this.credentialVerified = true;
                throw new Error(`AWS Secrets Manager GetSecretValue failed: AccessDeniedException for ${key}. ` +
    ...
    async set(ref, value) {
        ...
        try {
            ...
            if (isAwsError(err, 'AccessDeniedException')) {
                this.credentialVerified = true;
                throw new Error(`AWS Secrets Manager write failed: AccessDeniedException for ${key}. ` +
    ...
    async delete(ref) {
        ...
        try {
            ...
            if (isAwsError(err, 'AccessDeniedException')) {
                this.credentialVerified = true;
                throw new Error(`AWS Secrets Manager delete failed: AccessDeniedException for ${key}. ` +
...
function isCredentialError(err) {
    if (!(err instanceof Error))
        return false;
    const name = err.name;
    return (name === 'CredentialsProviderError' ||
