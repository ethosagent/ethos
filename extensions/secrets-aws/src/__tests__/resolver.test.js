import { CreateSecretCommand, DeleteSecretCommand, GetSecretValueCommand, ListSecretsCommand, PutSecretValueCommand, RestoreSecretCommand, SecretsManagerClient, } from '@aws-sdk/client-secrets-manager';
...
describe('get', () => {
    ...
    it('throws actionable error on AccessDeniedException', async () => {
        ...
        await expect(resolver.get('some/ref')).rejects.toThrow('See https://ethosagent.ai/docs/aws-secrets#iam-policy');
    });
...
describe('set', () => {
    ...
    it('throws on AccessDeniedException with IAM remediation pointer', async () => {
        ...
        await expect(resolver.set('some/ref', 'val')).rejects.toThrow('AccessDeniedException for ethos/engineer/some/ref');
        await expect(resolver.set('some/ref', 'val')).rejects.toThrow('See https://ethosagent.ai/docs/aws-secrets#iam-policy');
    });
...
describe('delete', () => {
    ...
    it('throws on AccessDeniedException', async () => {
        ...
        err.name = 'AccessDeniedException';
        smMock.on(DeleteSecretCommand).rejects(err);
        await expect(resolver.delete('some/ref')).rejects.toThrow('AccessDeniedException for ethos/engineer/some/ref');
    });
