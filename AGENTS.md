...
extensions/cron/src/jobs-lock.ts open/readFile/stat/unlink/readFileSync — `withJobsFileLock`, the cross-process lock around the read-modify-write of `<cronDir>/jobs.json` (the file itself goes through `Storage.writeAtomic`). An advisory `wx`-flag exclusive create whose body records `{pid, boot, token}`; a waiter reclaims it only when the pid is not running, belongs to an earlier Linux boot, or the body names no pid and is older than 30s (the critical section is a few-KB JSON rewrite). A live holder is never preempted by age. It replaced an empty-body lock with no stale rule, which a killed process left behind for ever — every later `cron` mutation waited 5s and failed with `Could not acquire lock`. Same primitive and carve-out as `extensions/gateway/src/channel-digest-lock.ts` (whose `currentBootId` it copies — `packages/wiring`'s `acquireSentinelLock` is not importable from `extensions/`); pinned by `extensions/cron/src/__tests__/jobs-lock.test.ts`
