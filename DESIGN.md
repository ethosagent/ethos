...
| 2026-09-26 | Docs content pages adopt the Option C "refined classic" theme | User-approved 2026-09-26; pixel source is the approved mockup (ethos-docs-c-classic.html). Doc typography opens up (16.5px/1.78 body, 720px measure, Bricolage Grotesque h1 38px / h2 26px — Geist from h3 down); the sidebar regroups the flat how-to list into topical collapsed subcategories under four static dot-labeled groups (docs/sidebars.ts + `sidebar-group--*` CSS); code blocks are a warm dark surface (`--ethos-code-*`: #141311/#1B1A17 light, lifted one step in dark) via a custom Prism theme in docusaurus.config.ts; `.ethos-figure` / `.ethos-figure-pad` / figcaption in custom.css is the figure contract diagram pages wrap themed SVGs in — class names are load-bearing; front-matter `kind` renders as a badge beside breadcrumbs (DocBreadcrumbs swizzle). All new styles read `--ethos-*` tokens in both palettes. |
