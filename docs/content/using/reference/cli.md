...
| `show <id>` | Print the generated [character sheet](../../building/explanation/personality-governance.md) — identity, routing, memory scope, toolset, MCP servers, plugins, `fs_reach`, and the prompt size, including the project context of a declared `fs_reach.workdir`. |
...
### ethos gateway status {#ethos-gateway-status}

Read the gateway lock and heartbeat without touching either. Source: [`apps/ethos/src/commands/gateway-status.ts`](https://github.com/ethosagent/ethos/blob/main/apps/ethos/src/commands/gateway-status.ts).

| Flag | Type | Default | Description |
|---|---|---|---|
| `--json` | boolean | off | Print `{ state, pid, heartbeatAgeMs, lockPath, spool, ledger }` on one line. `spool` and `ledger` are per-status counts, or `null` when that database does not exist yet. |

| Output | Exit code | Meaning |
|---|---|---|
| `running (pid N, heartbeat 4s ago)` | `0` | A live process holds the lock and wrote a heartbeat in the last 30 s. |
| `stale lock (pid N not running) — next start will take it over` | `1` | The lock names a process that is gone. |
| `stopped` | `1` | No lock. |
| `unhealthy (pid N alive, heartbeat 95s old)` | `2` | A live process holds the lock but its heartbeat is older than 30 s. |

```bash
ethos gateway status --json
```

```
{"state":"running","pid":4242,"heartbeatAgeMs":3811,"lockPath":"/home/me/.ethos/gateway.lock","spool":{"received":0,"processing":1,"done":57,"dead":0},"ledger":{"pending":0,"redelivering":0,"delivered":61,"abandoned":0}}
```

### ethos gateway spool {#ethos-gateway-spool}

Act on one dead-lettered inbound message in the [inbound spool](../how-to/recover-messages-after-a-crash.md). `ethos doctor` lists dead rows with their ids.

| Subcommand | Args | Effect |
|---|---|---|
| `replay` | `<id>` | Dead → owed, attempts reset. A running gateway replays it within 60 s; otherwise its next start does. |
| `discard` | `<id>` | Dead → done, without a turn. |

Exits `0` on success, `1` when the id is unknown, the row is not dead, or no spool exists.

```bash
ethos gateway spool discard 6f1c2a3e-0b7d-4c55-9a51-1f0e8d2b7c44
```

```
Discarded 6f1c2a3e-0b7d-4c55-9a51-1f0e8d2b7c44.
```
...
## ethos bench {#ethos-bench}

Measure the per-turn context cost of every personality. `context` is the only subcommand.

Synopsis: `ethos bench context [--cwd <dir>] [--live] [--scenario <id>] [--turns <n>] [--write-baseline]`

| Flag | Type | Default | Description |
|---|---|---|---|
| `--cwd <dir>` | path | current directory | Directory the project context (`AGENTS.md`/`CLAUDE.md`) is measured in. A personality with its own `fs_reach.workdir` is measured there instead. |
| `--live` | flag | off | Run live scenarios against the configured provider and record per-turn token usage. |
| `--scenario <id>` | string | all | Run one live scenario: `hi`, `one-tool`, `multi-tool` or `long-session`. |
| `--turns <n>` | integer | `10` | Turns for the `long-session` scenario. |
| `--write-baseline` | flag | off | Write the results to `evals/local/context-baseline.json`. |

The static table always prints, with no LLM calls:

| Column | Meaning |
|---|---|
| `soul ch` | `SOUL.md` size in characters. |
| `tools` | Number of tool schemas the personality's toolset exposes. |
| `schema ch` | Serialized tool-schema size in characters. |
| `tool_loading_chars` | Serialized tools sent on a turn's first step when on-demand tool loading is active. |
| `project ch` | The `## Project Context` block a turn sends from the measured directory. |
| `~tokens` | The static prefix at chars/4: SOUL, tool schemas, injection-defense prelude and project context. The same arithmetic as the small-window check (`measureStaticFloor`). |

```bash
ethos bench context
ethos bench context --cwd ~/code/my-repo
ethos bench context --live --scenario hi
```

Exits `1` on an unknown flag or scenario. Source: [`apps/ethos/src/commands/bench.ts`](https://github.com/ethosagent/ethos/blob/main/apps/ethos/src/commands/bench.ts).
