...
| `before_tool_call` | `BeforeToolCallPayload` → `BeforeToolCallResult` | Before each tool's `execute` runs — handlers can amend `args` or set `error` to reject. Amended `args` fire the hook a second time, so guard handlers judge what will run; a second, different amendment refuses the call (`enforceBeforeToolCall`, `packages/core/src/agent-loop/stages/per-call-enforcement.ts`). |
