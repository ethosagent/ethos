...
The `@ethosagent/skill-evolver` extension that drives [skill evolution](#skill-evolution). It drafts new skills and rewrites from eval output, recent sessions, the post-turn fork and the nightly pass, and submits each draft as a [skill proposal](#skill-proposal) to the learning inbox through its learning port (`extensions/skill-evolver/src/learning-port.ts`). It never writes a live skill file itself.
