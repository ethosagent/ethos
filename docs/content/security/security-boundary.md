...
**Tier 2 — extensions, not owned.** The remaining 104 packages. Best-effort. A bug is a bug, not a CVE. They inherit exactly what the kernel enforces and nothing more.
...
- **Enforced at.** [`tool-registry.ts:206`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/tool-registry.ts#L206) — `toDefinitions` filters what the LLM sees — and [`tool-registry.ts:372`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/tool-registry.ts#L372), where `executeParallel` re-checks the identical condition. The intent is stated in the source at [`:274`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/tool-registry.ts#L274): *"allowedTools + filterOpts enforce tool access at execution time (belt-and-suspenders)."* The shared refusal path is [`tool-rejection.ts:56`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/tool-rejection.ts#L56) (`emitToolRejection`), and the script surface is derived at [`script-safe.ts:121`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/script-safe.ts#L121) (`scriptCallableFor`).
