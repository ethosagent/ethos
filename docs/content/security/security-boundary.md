...
- **Enforced at.** [`danger-predicate.ts:282`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/danger-predicate.ts#L282), [`smart-approver.ts:144`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/smart-approver.ts#L144) (`{ decision: 'ask', reason: 'reviewer gave no usable verdict' }`) and [`:149`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/smart-approver.ts#L149) (`'reviewer error (fail-closed): …'`), with the union at [`danger-predicate.ts:259`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/danger-predicate.ts#L259). The single fire site every tool call passes through is [`per-call-enforcement.ts:55`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/per-call-enforcement.ts#L55) (`enforceBeforeToolCall`, where a handler error *blocks* the call, and where `safety.denyRules` refuses a matching call at [`:59`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/per-call-enforcement.ts#L59) before any handler runs), over the sequential execution model at [`hook-registry.ts:94`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/hook-registry.ts#L94) (`fireModifying`). The two seams that make the mode dispatch actually consulted are bound at [`approval-seams.ts:118`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/approval-seams.ts#L118), and the posture guard that refuses an unbacked `gated` claim is [`approval-posture.ts:38`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/approval-posture.ts#L38).
