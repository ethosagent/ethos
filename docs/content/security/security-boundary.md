...
- **Enforced at.** [`wrap.ts:35`](https://github.com/ethosagent/ethos/blob/main/packages/safety/injection/src/wrap.ts#L35) (`wrapUntrusted`), [`classifier.ts:60`](https://github.com/ethosagent/ethos/blob/main/packages/safety/injection/src/classifier.ts#L60) (`createLLMClassifier`), [`downgrade.ts:23`](https://github.com/ethosagent/ethos/blob/main/packages/safety/injection/src/downgrade.ts#L23) (`resolveDowngradedTools`); applied at [`context-assembly.ts:456`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/context-assembly.ts#L456) (the prelude, pushed unconditionally), [`agent-loop.ts:689`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop.ts#L689) (the post-read downgrade tool set), and [`tool-processing.ts:761`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/tool-processing.ts#L761) (`handleUntrustedResult` on every untrusted tool result), whose wrap-then-two-tier-classify body is [`result-defense.ts:26`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/result-defense.ts#L26). When an operator enables a decision provider for the injection site (`decisions.sites.injection`), the LLM tier is composed at [`decision-injection-classifier.ts:73`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/decision-injection-classifier.ts#L73) (`createDecisionInjectionClassifier`), which falls back to `createLLMClassifier` and acts on the provider's answer only at or above the measured threshold ([`decision-site.ts:124`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/decision-site.ts#L124), `meetsThreshold`); the redaction, shadow and fallback rules are [`decision-site.ts:133`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/decision-site.ts#L133) (`runDecisionSite`). Whichever classifier runs can only add a flag: [`result-defense.ts:74`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/result-defense.ts#L74) ORs its verdict with the Tier-1 hit.
...
- **Enforced at.** [`danger-predicate.ts:282`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/danger-predicate.ts#L282), [`smart-approver.ts:255`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/smart-approver.ts#L255) (`{ decision: 'ask', reason: 'reviewer gave no usable verdict' }`) and [`:263`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/smart-approver.ts#L263) (`'reviewer error (fail-closed): …'`); with a decision provider on the approver site, its `approve` / `deny` is acted on only at or above `decisions.thresholds.approver.approve` / `.deny` at [`:113`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/smart-approver.ts#L113) (`approverVerdictFrom`), and anything else falls to the LLM reviewer, with the union at [`danger-predicate.ts:259`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/danger-predicate.ts#L259). The single fire site every tool call passes through — the LLM batch path, the script bridge, and the realtime voice host's direct dispatch ([`realtime-host.ts`](https://github.com/ethosagent/ethos/blob/main/extensions/tools-voice/src/realtime-host.ts), `createRealtimeToolHost`) — is [`per-call-enforcement.ts:60`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/per-call-enforcement.ts#L60) (`enforceBeforeToolCall`, where a handler error *blocks* the call, and where `safety.denyRules` refuses a matching call at [`:64`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/stages/per-call-enforcement.ts#L64) before any handler runs), over the sequential execution model at [`hook-registry.ts:94`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/hook-registry.ts#L94) (`fireModifying`). The two seams that make the mode dispatch actually consulted are bound at [`approval-seams.ts:123`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/approval-seams.ts#L123), and the posture guard that refuses an unbacked `gated` claim is [`approval-posture.ts:38`](https://github.com/ethosagent/ethos/blob/main/packages/core/src/agent-loop/approval-posture.ts#L38).
...
- **Enforced at.** [`rules.ts:20`](https://github.com/ethosagent/ethos/blob/main/packages/safety/watcher/src/rules.ts#L20) (`rateLimitRule`), [`:79`](https://github.com/ethosagent/ethos/blob/main/packages/safety/watcher/src/rules.ts#L79) (`tokenBudgetRule`), [`:110`](https://github.com/ethosagent/ethos/blob/main/packages/safety/watcher/src/rules.ts#L110) (`compoundingErrorRule`), [`:161`](https://github.com/ethosagent/ethos/blob/main/packages/safety/watcher/src/rules.ts#L161) (`suspiciousSequenceRule`), composed at [`:205`](https://github.com/ethosagent/ethos/blob/main/packages/safety/watcher/src/rules.ts#L205); constructed in wiring at [`build-agent-loop.ts:555`](https://github.com/ethosagent/ethos/blob/main/packages/wiring/src/build-agent-loop.ts#L555).
