...
**Web and API surface.** The optional web API uses bearer-token authentication for programmatic clients and dual-auth (bearer or cookie) for the dashboard. The admin namespace is cookie-only: it has no entry in `SCOPE_MAP`, so `dualAuth` (`apps/web-api/src/middleware/dual-auth.ts`) refuses every bearer key there, and an exfiltrated API key cannot reach admin procedures. Every admin procedure also calls `requireAdmin`, which refuses with `403` unless `admin.enabled: true` is set; the default is disabled. The fail-closed branch is pinned by `dual-auth-scope.test.ts` through another unmapped namespace, and the `admin.enabled` gate by `admin.test.ts`. No test sends a bearer key to an admin procedure directly. CORS origins are restricted by configuration. Per-route rate limiting is applied where it matters most — the bearer-auth endpoint, attachment uploads, and pairing-code generation — so brute-force and abuse paths face an enforced ceiling before they reach application logic.

**Browser session cookie.** The dashboard's credential is the `ethos_auth` cookie. `/auth/exchange` sets it after checking and rotating the one-time URL token, as `HttpOnly`, `SameSite=Strict`, path `/`, with a 30-day max age (`authRoutes` in `apps/web-api/src/routes/auth.ts`, pinned by `apps/web-api/src/__tests__/routes/auth-and-rpc.test.ts`). `Secure` is conditional. `ethos serve` sets it when the web API binds to a non-loopback host or `webBaseUrl` starts with `https://`; the first-run setup server sets it only for a non-loopback bind (`secureCookie` in `apps/ethos/src/commands/serve.ts`). On the default loopback bind over plain HTTP the cookie is issued without `Secure`. No test pins the `Secure` decision.

**Cross-site request protection.** There is no synchronizer token. The web API checks the request's origin on every state-changing method (`POST`, `PUT`, `PATCH`, `DELETE`) under `/rpc/*` and `/openapi/*` (`csrfMiddleware` in `apps/web-api/src/middleware/csrf.ts`, mounted in `apps/web-api/src/routes/index.ts`). It reads `Origin`, falls back to the origin of `Referer`, and refuses the request when neither header is present. If `ETHOS_ALLOWED_ORIGINS` is set, only its entries pass: exact origins, or `*.domain` wildcards that also match the bare domain. The localhost rule then no longer applies. If it is unset, only a loopback origin (`localhost`, `127.0.0.1`, `[::1]`) whose host and port equal the request's `Host` header passes (`isSameOriginLocalhost` in `apps/web-api/src/lib/same-origin.ts`). A page on another localhost port is refused, and `localhost` and `127.0.0.1` on the same port count as different origins. A `/rpc/*` request authenticated by bearer token skips the check, because the API key is the credential there, not a cookie. Pinned by `apps/web-api/src/__tests__/middleware/csrf.test.ts`.

**WebSocket upgrade origin check.** A WebSocket upgrade is a `GET`, so the check above never sees it, and `SameSite` does not distinguish ports. The voice (`/voice/ws`), satellite (`/satellite/ws`) and browser-takeover (`/browser/takeover/ws`) sockets therefore check `Origin` themselves before authenticating (`originAllowed` in `apps/web-api/src/voice/voice-socket.ts`). An upgrade with no `Origin` header goes on to the credential check, because a voice satellite is a daemon, not a browser. An origin listed verbatim in `ETHOS_ALLOWED_ORIGINS` passes. Otherwise only a same-origin loopback origin passes, by the same `isSameOriginLocalhost` comparison, and anything else is refused with `403`. Two differences from the HTTP check: allow-list entries match exactly, so a `*.domain` wildcard never matches a socket origin, and the localhost rule stays in force when an allow-list is set. Pinned by `apps/web-api/src/voice/__tests__/voice-socket.test.ts` and `apps/web-api/src/browser/__tests__/takeover-socket.test.ts`.
