...
import { isServerAllowed } from '@ethosagent/tools-mcp';
...
function makeMockSessionView() {
    ...
    const view = {
        registerSessionServers: vi.fn(async (configs, allowlist) => {
            const registered = [];
            const rejected = [];
...
describe('session/registerMcpServers', () => {
    it('filters servers by personality allowlist', async () => {
        ...
        const resolveAllowlist = (personalityId) => {
            if (personalityId === 'researcher')
                return ['github-*', 'docs'];
            return undefined;
        };
    ...
    it('rejected servers include reason', async () => {
        const { view } = makeMockSessionView();
        const resolveAllowlist = () => ['only-this'];
        const { input, output } = makeServer({
            resolveAllowlist,
            createSessionView: () => view,
        });
    ...
    it('open mode (undefined allowlist) allows all servers', async () => {
        const { view } = makeMockSessionView();
        const resolveAllowlist = () => undefined;
        const { input, output } = makeServer({
            resolveAllowlist,
            createSessionView: () => view,
        });
