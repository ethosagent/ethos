...
function makeStore() {
    ...
    return {
        ...
        async updateSession(id, patch) {
            ...
            if (s)
                sessions.set(id, { ...s, ...patch, updatedAt: new Date() });
        },
        async deleteSession(id) {
            sessions.delete(id);
            messages.delete(id);
        },
        async listSessions(_filter) {
            return [...sessions.values()];
        },
        async appendMessage(msg) {
            const stored = { ...msg, id: randomUUID(), timestamp: new Date() };
...
function makeServer(runner, store) {
...
function send(input, req) {
    input.write(`${JSON.stringify(req)}\n`);
}
function readLines(output, count) {
...
describe('AcpServer', () => {
    ...
    it('new_session returns a unique acp: sessionKey each call', async () => {
        ...
        const key1 = r1.result.sessionKey;
        const key2 = r2.result.sessionKey;
    ...
    it('prompt sends $/stream notification for each AgentEvent', async () => {
        ...
        const streams = msgs.filter((m) => m.method === '$/stream');
    ...
    it('prompt sends final result with accumulated text and turnCount', async () => {
        ...
        const msgs = await lines;
        const result = msgs.find((m) => m.id === 1);
    ...
    it('cancel fires AbortSignal for in-flight prompt', async () => {
        ...
        const msgs = await lines;
        const cancelResp = msgs.find((m) => m.id === 2);
    ...
    it('fork_session creates new session with copied message history', async () => {
        ...
        const [resp] = await lines;
        const newKey = resp.result.sessionKey;
        expect(newKey).toMatch(/^acp:fork:/);
        const forked = await store.getSessionByKey(newKey);
        expect(forked).not.toBeNull();
        if (!forked)
            throw new Error('Expected forked session to exist');
