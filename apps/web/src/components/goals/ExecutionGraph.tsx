...
import { useToolResult } from '../../features/goals/api/queries';
...
interface GraphNode {
  ...
  /** Original event type */
  eventType: string;
  /** Parallel batch id — only set on TOOL nodes. Shared across one maximal run of consecutive tool_start events. */
  batchId?: number;
}
...
function classifyEvent(eventType: string): NodeKind {
  switch (eventType) {
    ...
    case 'steer':
      return 'STEER';
    case 'error':
      // An error is a failure boundary — reuse the existing REJECTED visual.
      return 'REJECTED';
...
function buildNodes(events: ExecutionGraphProps['events']): GraphNode[] {
  ...
  // toolKey → index of the tool's node in mainNodes (dedupes start/end pairs).
  ...
  for (const ev of events) {
    ...
    // skip them so they never produce an empty/meaningless graph node.
    if (ev.eventType === 'usage') continue;

    const kind = classifyEvent(ev.eventType);

    if (kind === 'TOOL') {
      // Every tool call is a first-class node in the main sequence, keyed by a
      // stable id so tool_start and tool_end collapse into one node.
      const toolKey =
        (ev.payload?.toolCallId as string) ?? (ev.payload?.toolName as string) ?? `tool-${ev.id}`;

      if (ev.eventType === 'tool_start') {
        // First start for this key creates the node; a duplicate start is ignored.
        if (toolIndexByKey.has(toolKey)) {
          // A duplicate start is still a start in the stream — it keeps the run alive.
          prevWasToolStart = true;
          continue;
        }
        // A consecutive run of starts shares one batchId; a gap opens a new one.
      ...
      // tool_end: merge into the existing start node if present, else push a
      // standalone done node so the call is never dropped.
      ...
      if (existingIdx != null) {
        ...
        // A tool_end is a non-start event: the next tool_start begins a new batch.
        prevWasToolStart = false;
      } else {
        // Standalone end with no matching start gets its own fresh batch.
        currentBatchId += 1;
        mainNodes.push({
          kind,
          id: ev.id,
...
/** Human-readable "what happened" title per node kind. */
...
/** Payload-derived input for non-tool nodes. Returns null when there's nothing to show. */
...
/** Error text for failure nodes; null when the node is not a failure. */
...
function NodeDetailModal({
  ...
  // Only the tool RPC is gated to the modal being open; non-tool nodes never fetch.
  const { data, isLoading } = useToolResult(goalId, toolCallId, isTool);
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Error — prominent, only when the node represents a failure */}
          {errorText && (
            <div
              style={{
                marginBottom: 16,
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {isTool ? (
              <pre style={preStyle}>{data?.input ?? formatPayloadValue(node.payload?.args)}</pre>
            ) : (
              <pre style={preStyle}>{nonToolInput ?? '(none)'}</pre>
            )}
          </div>

          {/* Output — tool nodes always show a section; non-tool only when present */}
          {isTool ? (
            <div>
              <div style={sectionLabelStyle}>Output</div>
              {isLoading ? (
                <span style={{ color: 'var(--text-tertiary)' }}>Loading output&#8230;</span>
              ) : data?.found && data.output && data.output.trim().length > 0 ? (
                <pre style={preStyle}>{data.output}</pre>
              ) : (
...
export function ExecutionGraph({
  ...
  // Track the visible container size so positions can center the node row.
  useEffect(() => {
  ...
  // The node currently executing: the last in-flight (running) tool if any,
  // otherwise the last node while the goal is active. -1 when nothing is active.
  ...
  // Collapse mainNodes into slots: a >1-tool batch is one 'parallel' slot;
  // everything else (incl. size-1 tool batches) is its own 'single' slot.
  const slots = useMemo(() => {
  ...
  // The slot containing the active node, for breathing + auto-scroll.
  ...
  // Compute positions — one entry per SLOT, not per node.
  const positions = useMemo(() => {
    // Per-slot heights, and the tallest node height (for vertical centering).
    ...
    // PASS 1: lay the row out from a 0 base; track the row's total content width.
    ...
    // PASS 2: pick a dynamic left start, then shift every node by it.
    const w = containerSize.w;
    const LEFT_MIN = 16; // leftmost padding
    const RIGHT_PAD = 24; // breathing room on the right before overflow
    // Pre-measure fallback: keep the previous fixed offset so nothing breaks.
    const leftOffset =
      w > 0
        ? (() => {
            const startThird = Math.max(LEFT_MIN, w / 3);
            // Keep content's right edge near the container's right edge; clamp so it
            // never sits further right than ~1/3 (few nodes) nor further left than
            // LEFT_MIN (many nodes / overflow).
  ...
  // Build arrow connections (main row only — tools are first-class main nodes now)
  ...
  // One-time initial centering for inactive/terminal goals: the GOAL node is
  // already placed at ~1/3 from the left and vertically centered by the
  // positions math, so we just reset scroll to the canvas origin so that
  // placement is what the viewer sees first. The active-scroll effect below
  // takes over once the goal is running.
  ...
  // Keep the active / newest node in view while the goal is running.
  useEffect(() => {
    if (isActive !== true) return;
    const container = scrollRef.current;
    if (!container) return;
    ...
    // positions changes identity whenever the slot list grows, so it covers
    // "slot count changed" — mainNodes.length would be a redundant dep (Biome).
  }, [isActive, activeSlotIndex, positions, zoom]);

  // Ctrl/Cmd + wheel zooms. Attached as a non-passive native listener so
  // preventDefault actually suppresses the browser's pinch-zoom / scroll.
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
    ...
    transition-duration: 0.001ms !important;
        ...
        {mainNodes.length === 0 ? (
          ...
          <div
            ...
            style={{
              position: 'relative',
              // Size the scaled layer to at least cover the viewport once scaled,
              // so the dotted background fills the container even when zoomed out
              // and the overflow gives the outer container a correct scroll range.
