...
function pendingState(context) {
    const value = context._mcpPendingState;
    return typeof value === 'string' ? value : undefined;
}
/** Read the derived OAuth `redirect_uri` threaded into context by the Hono layer. */
function requestOrigin(context) {
    const value = context._mcpRequestOrigin;
    return typeof value === 'string' ? value : undefined;
}
export const mcpRouter = {
    start: os.mcp.start.handler(({ input, context }) => context.mcp.start(input, requestOrigin(context))),
    complete: os.mcp.complete.handler(({ input, context }) => context.mcp.complete(input, pendingState(context))),
    ...
    attachPersonalities: os.mcp.attachPersonalities.handler(({ input, context }) => context.mcp.attachPersonalities(input)),
    ...
    personalityServers: os.mcp.personalityServers.handler(({ input, context }) => context.mcp.personalityServers(input)),
    addServer: os.mcp.addServer.handler(({ input, context }) => context.mcp.addServer(input)),
    refreshToken: os.mcp.refreshToken.handler(({ input, context }) => context.mcp.refreshToken(input)),
    ...
    validateConfig: os.mcp.validateConfig.handler(({ input, context }) => context.mcp.validateConfig(input)),
};
