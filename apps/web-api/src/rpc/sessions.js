...
export const sessionsRouter = {
    list: os.sessions.list.handler(({ input, context }) => context.sessions.list({
    ...
    fork: os.sessions.fork.handler(({ input, context }) => context.sessions.fork(input.id, input.personalityId)),
    delete: os.sessions.delete.handler(async ({ input, context }) => {
        await context.sessions.delete(input.id);
        return { ok: true };
    }),
    update: os.sessions.update.handler(({ input, context }) => context.sessions.update(input.id, { title: input.title })),
    export: os.sessions.export.handler(({ input, context }) => context.sessions.export(input.id, input.format)),
