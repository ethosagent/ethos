...
export const cronRouter = {
    ...
    create: os.cron.create.handler(({ input, context }) => context.cron.create({
        name: input.name,
    ...
    update: os.cron.update.handler(({ input, context }) => context.cron.update(input.id, {
    ...
    history: os.cron.history.handler(({ input, context }) => context.cron.history(input.id, input.limit)),
};
