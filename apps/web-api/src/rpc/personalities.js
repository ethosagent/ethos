...
export const personalitiesRouter = {
    ...
    characterSheet: os.personalities.characterSheet.handler(({ input, context }) => context.personalities.characterSheet(input.id)),
    create: os.personalities.create.handler(({ input, context }) => context.personalities.create({
        id: input.id,
        name: input.name,
    ...
    update: os.personalities.update.handler(async ({ input, context }) => {
        const { id, mcp_servers, mcp_tools, plugins, capabilities, provider, fs_reach, ...rest } = input;
    ...
    delete: os.personalities.delete.handler(async ({ input, context }) => {
        ...
        const dependent = jobs.filter((j) => j.personalityId === input.id);
        if (dependent.length > 0) {
            const names = dependent.map((j) => j.name).join(', ');
            console.warn(`[personalities] Deleting "${input.id}" but ${dependent.length} cron job(s) still reference it: ${names}`);
        }
        await context.personalities.delete(input.id);
        return { ok: true };
    }),
    duplicate: os.personalities.duplicate.handler(({ input, context }) => context.personalities.duplicate(input.id, input.newId)),
    skillsList: os.personalities.skillsList.handler(({ input, context }) => context.personalities.skillsList(input.personalityId)),
    skillsGet: os.personalities.skillsGet.handler(({ input, context }) => context.personalities.skillsGet(input.personalityId, input.skillId)),
    skillsCreate: os.personalities.skillsCreate.handler(({ input, context }) => context.personalities.skillsCreate(input.personalityId, input.skillId, input.body)),
    skillsUpdate: os.personalities.skillsUpdate.handler(({ input, context }) => context.personalities.skillsUpdate(input.personalityId, input.skillId, input.body)),
    ...
    skillsImportGlobal: os.personalities.skillsImportGlobal.handler(({ input, context }) => context.personalities.skillsImportGlobal(input.personalityId, input.skillIds)),
