...
export class ConfigRepository {
    ...
    async read() {
        ...
        const config = { modelRouting: {}, providers: [], passthrough: {} };
        ...
        for (const line of src.split('\n')) {
            ...
            if (mr) {
                ...
                if (id && value)
                    config.modelRouting[id] = stripQuotes(value);
                continue;
            }
     ...
     */
    async deletePassthroughKeys(keys) {
        let current;
        const op = this.writeChain
            .catch(() => { })
            .then(async () => {
            ...
            for (const key of keys)
                delete current.passthrough[key];
    ...
    async write(config) {
        ...
        const lines = [];
        if (config.provider)
            lines.push(`provider: ${yamlScalar(config.provider)}`);
        if (config.model)
            lines.push(`model: ${yamlScalar(config.model)}`);
        if (config.apiKey)
            lines.push(`apiKey: ${yamlScalar(config.apiKey)}`);
        if (config.personality)
            lines.push(`personality: ${yamlScalar(config.personality)}`);
        if (config.memory)
            lines.push(`memory: ${yamlScalar(config.memory)}`);
        if (config.baseUrl)
            lines.push(`baseUrl: ${yamlScalar(config.baseUrl)}`);
        if (config.skin)
            lines.push(`skin: ${yamlScalar(config.skin)}`);
        ...
        for (let i = 0; i < config.providers.length; i++) {
            ...
            if (p.apiKey)
                lines.push(`providers.${i}.apiKey: ${yamlScalar(p.apiKey)}`);
            if (p.model)
                lines.push(`providers.${i}.model: ${yamlScalar(p.model)}`);
            if (p.baseUrl)
                lines.push(`providers.${i}.baseUrl: ${yamlScalar(p.baseUrl)}`);
        }
