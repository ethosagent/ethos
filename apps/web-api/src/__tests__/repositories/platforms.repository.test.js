...
describe('PlatformsRepository multi-bot telegram', () => {
    ...
    it('addTelegramBot appends at next index', async () => {
        ...
        const yaml = await storage.read(join(DATA, 'config.yaml'));
        expect(yaml).toContain(`telegram.bots.0.token: "\${secrets:telegram/bots/${first.botKey}/token}"`);
        expect(yaml).toContain(`telegram.bots.1.token: "\${secrets:telegram/bots/${second.botKey}/token}"`);
...
describe('PlatformsRepository multi-bot slack', () => {
    ...
    it('addSlackApp writes all three tokens through the resolver and refs into config', async () => {
        const app = await repo.addSlackApp({ botToken: 'xoxb-1', appToken: 'xapp-1', signingSecret: 'shh' }, { type: 'personality', name: 'coder' });
    ...
    it('removeSlackApp deletes the entry, the three secrets, and re-indexes', async () => {
        const first = await repo.addSlackApp({ botToken: 'xoxb-1', appToken: 'xapp-1', signingSecret: 's1' }, { type: 'personality', name: 'coder' });
        await repo.addSlackApp({ botToken: 'xoxb-2', appToken: 'xapp-2', signingSecret: 's2' }, { type: 'team', name: 'eng' });
