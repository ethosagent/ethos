...
describe('createWebApprovalHook', () => {
    ...
    it('dangerous + approve resolves the hook with null (= allow)', async () => {
        const isDangerous = () => 'destructive command';
        const hook = createWebApprovalHook({ approvals, isDangerous });
        let approvalId = '';
    ...
    it('passes the danger reason through to the SSE pending payload', async () => {
        const isDangerous = (p) => p.toolName === 'terminal' ? 'recursive force-delete of root' : null;
        const hook = createWebApprovalHook({ approvals, isDangerous });
        let pendingReason;
        approvals.onPending((_, req) => {
    ...
    it('an allowlist hit auto-allows without surfacing pending', async () => {
        ...
        const isDangerous = () => 'force-delete';
        const hook = createWebApprovalHook({ approvals, isDangerous });
        let pending = false;
        approvals.onPending(() => {
            pending = true;
...
async function tickUntil(predicate, timeoutMs = 1000) {
    const start = Date.now();
    while (!predicate()) {
        if (Date.now() - start > timeoutMs)
            throw new Error('tickUntil timed out');
