...
describe('OnboardingService', () => {
    ...
    function makeService(extras = {}) {
        ...
        const personalities = makeStubPersonalityRegistry(extras.personalities ?? [{ id: 'researcher', name: 'Researcher' }], DATA);
    ...
    describe('state', () => {
        ...
        it('returns personality when provider+key set but no personality picked yet', async () => {
            await storage.write(join(DATA, 'config.yaml'), ['provider: anthropic', 'apiKey: sk-something'].join('\n'));
            const service = makeService();
            const state = await service.state();
    ...
    describe('validateProvider', () => {
        it('returns model list on a 200 response', async () => {
            const fetchFn = (async () => new Response(JSON.stringify({ data: [{ id: 'claude-opus-4-7' }, { id: 'claude-haiku-4' }] }), {
                status: 200,
            }));
