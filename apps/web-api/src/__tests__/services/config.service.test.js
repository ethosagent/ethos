...
describe('ConfigService', () => {
    ...
    it('update with empty apiKey is a no-op (does not erase the existing key)', async () => {
        await storage.write(join(DATA, 'config.yaml'), ['provider: anthropic', 'model: m', 'apiKey: sk-keep-this', 'personality: researcher'].join('\n'));
    ...
    it('get returns empty providers array when none are configured', async () => {
        await storage.write(join(DATA, 'config.yaml'), ['provider: anthropic', 'model: claude-opus-4-7', 'personality: researcher'].join('\n'));
        const result = await service.get();
        expect(result.providers).toEqual([]);
    });
    it('update can replace the apiKey when a non-empty value is supplied', async () => {
        await storage.write(join(DATA, 'config.yaml'), ['provider: anthropic', 'model: m', 'apiKey: sk-old', 'personality: researcher'].join('\n'));
