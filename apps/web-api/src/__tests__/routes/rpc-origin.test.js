...
describe('deriveMcpRequestOrigin — Origin header', () => {
    ...
    it('accepts RFC 1918 172.16-31.x.x', () => {
        expect(deriveMcpRequestOrigin(makeReq({ origin: 'http://172.16.0.1' }), undefined)).toBe('http://172.16.0.1');
        expect(deriveMcpRequestOrigin(makeReq({ origin: 'http://172.31.255.255' }), undefined)).toBe('http://172.31.255.255');
    });
    it('rejects 172.15.x (outside RFC 1918)', () => {
        expect(deriveMcpRequestOrigin(makeReq({ origin: 'http://172.15.0.1' }), undefined)).toBeUndefined();
    });
    it('rejects 172.32.x (outside RFC 1918)', () => {
        expect(deriveMcpRequestOrigin(makeReq({ origin: 'http://172.32.0.1' }), undefined)).toBeUndefined();
    });
    ...
    it('accepts a public origin only when it matches webBaseUrl', () => {
        ...
        expect(deriveMcpRequestOrigin(req, 'https://ethos.example.com')).toBe('https://ethos.example.com');
    });
    ...
    it('webBaseUrl with a path component still matches on origin only', () => {
        const req = makeReq({ origin: 'https://ethos.example.com' });
        expect(deriveMcpRequestOrigin(req, 'https://ethos.example.com/admin/')).toBe('https://ethos.example.com');
    });
});
