...
describe('tools.approve / tools.deny — full inversion loop', () => {
    ...
    beforeEach(async () => {
        ...
        app = createWebApi({
            ...
            dangerPredicate: (p) => p.toolName === 'terminal' ? 'every terminal call requires approval (test rule)' : null,
        }).app;
...
function rpcHeaders(cookieHeader) {
    return {
 ...
 */
async function waitForPendingApproval(app, cookieHeader, sessionId, timeoutMs = 1000) {
...
async function readApprovalIdFromStream(body, timeoutMs = 200) {
    ...
    try {
        while (Date.now() - start < timeoutMs) {
            ...
            const timeout = new Promise((r) => setTimeout(() => r({ done: true, value: undefined }), 50));
