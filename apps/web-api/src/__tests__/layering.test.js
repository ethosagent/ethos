...
describe('web-api layering (Phase 26 Done-when #24)', () => {
    it('src/rpc/*.ts does not import any extension package directly', () => {
        ...
        expect(offenders, `RPC handlers must not import extensions directly. Move data access into a repository:\n${offenders.join('\n')}`).toEqual([]);
    });
    it('src/rpc/*.ts does not touch the filesystem or SQLite', () => {
        ...
        expect(offenders, `RPC handlers must not touch the filesystem/SQLite. Move I/O into a repository:\n${offenders.join('\n')}`).toEqual([]);
    });
    it('src/rpc/*.ts handlers stay thin (heuristic: each file ≤120 lines)', () => {
        ...
        for (const file of listTs(RPC_DIR)) {
            ...
            if (lines > 120)
                fat.push({ file: relative(SRC_ROOT, file), lines });
        }
        expect(fat).toEqual([]);
    });
});
