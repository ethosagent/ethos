...
export function resolveScope(rpcPath) {
...
export function cookieOnlyGuard() {
    return async (c, next) => {
        const method = c.get('authMethod');
        if (method === 'bearer') {
            throw new EthosError({
                code: 'FORBIDDEN',
                cause: 'This endpoint requires cookie authentication — bearer tokens cannot manage API keys.',
                action: 'Use the Ethos web UI to manage API keys.',
            });
        }
        return next();
