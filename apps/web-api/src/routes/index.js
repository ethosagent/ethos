...
export function createRoutes(opts) {
    ...
    app.get('/healthz', async (c) => {
        ...
        try {
            ...
            if (!raw)
                throw new Error('no storage or file missing');
        ...
        const allAdaptersOk = gatewayBlock.adapters.length > 0 && gatewayBlock.adapters.every((a) => a.ok);
    ...
    app.use('*', cors({
        origin: (origin) => {
            if (!origin)
                return null; // same-origin / non-browser request
    ...
    app.route('/auth', authRoutes({ tokens: opts.tokens, ...(opts.secureCookie ? { secureCookie: true } : {}) }));
