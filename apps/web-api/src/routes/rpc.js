...
export function rpcRoutes(opts) {
    ...
    app.all('/*', async (c) => {
        ...
        const context = mcpPendingState || mcpRequestOrigin || mcpPendingPersonalityId
            ? Object.assign(Object.create(null), opts.services, mcpPendingState ? { _mcpPendingState: mcpPendingState } : {}, mcpPendingPersonalityId ? { _mcpPendingPersonalityId: mcpPendingPersonalityId } : {}, mcpRequestOrigin ? { _mcpRequestOrigin: mcpRequestOrigin } : {})
            : opts.services;
        ...
        if (path === mcpRpcPath('cancel')) {
            const headers = new Headers(response.headers);
            headers.append('Set-Cookie', `${MCP_PENDING_COOKIE}=; HttpOnly; SameSite=Strict; Path=/rpc; Max-Age=0`);
            return new Response(response.body, {
                status: response.status,
                statusText: response.statusText,
                headers,
