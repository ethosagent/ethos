...
export function openAiChatRoutes(opts) {
    const app = new Hono();
    app.post('/completions', async (c) => {
        ...
        if (warnings.length > 0)
            c.header('x-ethos-warning', warnings.join('; '));
        ...
        const sessionKeyOverride = rawSessionKey && apiKeyRecord ? `${apiKeyRecord.id}:${rawSessionKey}` : rawSessionKey;
        const input = {
            req,
...
function streamCompletion(c, service, input) {
...
function resolveModel(model, personalities) {
    if (model.startsWith('team:'))
        return { kind: 'team' };
...
function hasImageContent(req) {
    return req.messages.some((msg) => Array.isArray(msg.content) &&
        msg.content.some((part) => typeof part === 'object' && 'type' in part && part.type === 'image_url'));
}
function rejectUnsupported(req) {
    for (const msg of req.messages) {
        if (msg.role === 'system') {
            return {
                message: 'System messages are not supported. The personality owns the system prompt. ' +
    ...
    if (req.tools && req.tools.length > 0) {
        return {
            message: 'Client-side `tools` lands in C1. Drop the field or wait for the C1 release for client-tools mode.',
