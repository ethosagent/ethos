    ...
    async getBoard(team) {
        ...
        try {
            ...
            const eventRows = store.db
                .prepare('SELECT id, task_id, kind, actor, data_json, created_at FROM task_events ORDER BY id DESC LIMIT ?')
                .all(RECENT_EVENTS_CAP);
            const recentEvents = eventRows.reverse().map((r) => ({
                id: r.id,
                taskId: r.task_id,
                kind: r.kind,
                actor: r.actor,
            ...
            const memberStats = [...store.getMemberStats().values()].map(toWireMemberStats);
            return {
                board: {
                    team: summary,
                    tasks,
...
// Mappers
// ---------------------------------------------------------------------------
function toTeamSummary(name, description, manifest, runtime, rootDir) {
    ...
    const dispatchMode = manifest.dispatch_mode ?? (manifest.coordinator !== undefined ? 'coordinator' : 'self-routing');
