...
import { ConfidentialClientUnsupported, DcrUnsupported, deleteTokens, McpInstallFlow, OAuthDiscoveryError, revokeToken, } from '@ethosagent/tools-mcp';
export class McpService {
    ...
    async start(input, redirectUri) {
        try {
            ...
            const detail = err instanceof SsrfError ? err.message : 'URL points to a private or reserved IP range';
        ...
        try {
            ...
            if (input.personalityId !== undefined)
                startOpts.personalityId = input.personalityId;
            if (input.name !== undefined)
                startOpts.name = input.name;
            if (redirectUri !== undefined)
                startOpts.redirectUri = redirectUri;
    ...
    async complete(input, cookieState) {
        ...
        try {
            ...
            const code = kind === 'expired_state' || kind === 'already_completed'
                ? 'expired_state'
    ...
    async personalityServers(input) {
        ...
        const configs = await this.mcpJsonStore.read();
        const servers = await Promise.all(mcpServers.map(async (name) => {
    ...
    async reconnect(input) {
        ...
        if (input.personalityId !== undefined)
            startInput.personalityId = input.personalityId;
