...
export class ChatService {
    ...
    async abort(sessionId) {
        ...
        if (!bridge)
            return; // No bridge → nothing to abort. Idempotent.
        bridge.abortTurn();
    }
    ...
    wireBridge(sessionId, bridge) {
        ...
        bridge.on('thinking_delta', (thinking) => this.append(sessionId, { type: 'thinking_delta', thinking }));
        bridge.on('tool_start', (toolCallId, toolName, args) => this.append(sessionId, { type: 'tool_start', toolCallId, toolName, args }));
        bridge.on('tool_progress', (toolName, message, percent) => this.append(sessionId, {
            type: 'tool_progress',
            toolName,
            message,
        ...
        bridge.on('tool_end', (toolCallId, toolName, ok, durationMs, result) => this.append(sessionId, {
            type: 'tool_end',
            toolCallId,
            toolName,
            ok,
        ...
        bridge.on('usage', (inputTokens, outputTokens, estimatedCostUsd) => this.append(sessionId, {
            type: 'usage',
            inputTokens,
            outputTokens,
            estimatedCostUsd,
        }));
        bridge.on('dry_run_summary', (plan, capped) => this.append(sessionId, { type: 'dry_run_summary', plan, capped }));
        bridge.on('run_start', (provider, model, source) => this.append(sessionId, { type: 'run_start', provider, model, source }));
    ...
    async titleSession(sessionId, firstUserMessage) {
        if (!this.opts.titleFn)
            return;
        try {
            const title = await this.opts.titleFn('Generate a title for this conversation in 6 words or fewer. Reply with only the title, no punctuation.', firstUserMessage);
