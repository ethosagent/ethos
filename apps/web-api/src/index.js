...
export function createWebApi(opts) {
    ...
    const secrets = opts.secrets ?? new FileSecretsResolver({ dir: join(opts.dataDir, 'secrets'), storage });
    ...
    if (opts.dangerPredicate) {
        opts.agentLoop.hooks.registerModifying('before_tool_call', createWebApprovalHook({
