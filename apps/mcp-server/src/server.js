...
export class EthosMcpServer {
    ...
    constructor(config) {
        this._config = config;
        this._server = new Server({ name: 'ethos', version: config.version ?? 'dev' }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
    ...
    async serveHttp(opts) {
        ...
        if (host !== '127.0.0.1' && host !== 'localhost' && host !== '::1') {
            throw new Error('MCP HTTP server only binds to loopback (127.0.0.1). Non-loopback binds are not supported until an auth story ships.');
        }
