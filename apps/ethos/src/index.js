...
const USAGE = 'Usage: ethos [setup | chat | sessions | serve | dashboard | status | run-all | set | team | mesh | process | logs | gateway | cron | personality | memory | acp | batch | eval | evolve | plugin | skills | keys | secrets | fallback | slack | api-key | claw | doctor | upgrade | mcp | backup | import | trace | audit | security | errors | perf | tail | retention | data | support | archive | systemd-unit | usage] [--version | --help]';
const args = process.argv.slice(2);
const command = args[0] ?? '';
const inferredChatFromQueryFlag = command === '-q' || command === '--query' || command.startsWith('--query=');
const inferredChatFromResumeFlag = command === '--continue' || command === '-c' || command === '--resume' || command === '-r';
...
function extractSingleQuery(argv) {
    const rest = [];
    let query;
...
const isMetadataCommand = effectiveCommand === '--version' ||
    effectiveCommand === '-v' ||
...
`);
        ...
        case '': {
            ...
            const resumeFlagIdx = chatArgs.indexOf('--resume') !== -1 ? chatArgs.indexOf('--resume') : chatArgs.indexOf('-r');
            ...
            if (continueFlag || resumeQuery !== undefined) {
                ...
                try {
                    const target = continueFlag
                        ? { type: 'continue' }
                        : { type: 'resume', query: resumeQuery ?? '' };
                    const session = await resolveResumeSession(store, target);
                    if (!session) {
                        console.error(continueFlag ? 'No sessions found to resume.' : `Session not found: ${resumeQuery}`);
                        process.exit(1);
                    }
            ...
            if (!config) {
                ...
                if (setupResult) {
                    ...
                    if (skinFlag)
                        withFlags.skin = skinFlag;
                    if (teamFlag)
                        withFlags.activeContext = { type: 'team', name: teamFlag };
        ...
        case 'personality': {
            ...
            if (sub === 'list' || sub === '') {
                ...
                console.log('Usage: ethos personality [list | create [name] [--blank | --from <id>] | show <id> | set <id> | duplicate <src> <dst> | export <id> [--output <path>] | import <file> [--force] [--secrets <manifest>] | mcp <id> [--attach <name> [--token-stdin] | --detach <name> | --token-stdin <server>] | plugins <id> [--attach <plugin-id> | --detach <plugin-id>]]');
            }
            break;
        }
        case 'plugins': {
        ...
        case 'gateway': {
            ...
            if (sub === 'setup') {
                ...
                if (gwResult?.launchChat)
                    await runChat(gwResult.config);
            }
            else if (sub === 'start') {
...
async function runPersonalityMcp(argv) {
    const id = argv[0];
    if (!id) {
        console.log('Usage: ethos personality mcp <id> [--attach <name> [--token-stdin] | --detach <name> | --token-stdin <server>]');
        return;
    }
    ...
    if (flags.attach && argv.includes('--token-stdin')) {
        ...
        if (serverCfg && serverCfg.auth?.type !== 'bearer') {
            console.warn(`⚠ Server "${flags.attach}" auth is not { type: 'bearer' }. Token stored but will not be used until auth is updated in mcp.json.`);
        }
    ...
    if (argv.includes('--token-stdin')) {
        ...
        if (serverCfg && serverCfg.auth?.type !== 'bearer') {
            console.warn(`⚠ Server "${serverName}" auth is not { type: 'bearer' }. Token stored but will not be used until auth is updated in mcp.json.`);
        }
...
// ---------------------------------------------------------------------------
async function runPersonalityPlugins(argv) {
    const id = argv[0];
    if (!id) {
        console.log('Usage: ethos personality plugins <id> [--attach <plugin-id> | --detach <plugin-id>]');
        return;
    }
    const flags = parseCliFlags(argv.slice(1));
    const { createPersonalityRegistry } = await import('@ethosagent/personalities');
...
// ---------------------------------------------------------------------------
async function runPersonalityDuplicate(argv) {
    const src = argv[0];
    const dst = argv[1];
    if (!src || !dst) {
...
async function runPersonalityShow(argv) {
    ...
    const { createPersonalityRegistry, renderCharacterSheet } = await import('@ethosagent/personalities');
    const { ethosDir } = await import('./config');
    const storage = getStorage();
    const reg = await createPersonalityRegistry({ storage, userPersonalitiesDir: ethosDir() });
    await reg.loadFromDirectory(join(ethosDir(), 'personalities'));
...
// ---------------------------------------------------------------------------
async function runPluginsStatus() {
    const { createPersonalityRegistry } = await import('@ethosagent/personalities');
    const { scanInstalledPlugins } = await import('@ethosagent/plugin-loader');
    const { ethosDir } = await import('./config');
    const storage = getStorage();
    ...
    console.log();
    const unattached = allPlugins.filter((p) => allPersonalities.every((pers) => !(pers.plugins ?? []).includes(p.id)));
    if (unattached.length > 0) {
        console.log(`  ⚠  ${unattached.length} plugin(s) installed but not attached to any personality - they're inert until attached.`);
        console.log();
    }
}
