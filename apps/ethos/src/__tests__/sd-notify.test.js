...
let prevNotifySocket;
beforeEach(() => {
    prevNotifySocket = process.env.NOTIFY_SOCKET;
    delete process.env.NOTIFY_SOCKET;
});
afterEach(() => {
    if (prevNotifySocket === undefined)
        delete process.env.NOTIFY_SOCKET;
    else
        process.env.NOTIFY_SOCKET = prevNotifySocket;
});
describe('sd-notify', () => {
...
describe('startWatchdog', () => {
    let prevWatchdogUsec;
    beforeEach(() => {
        prevWatchdogUsec = process.env.WATCHDOG_USEC;
        delete process.env.WATCHDOG_USEC;
    });
    afterEach(() => {
        if (prevWatchdogUsec === undefined)
            delete process.env.WATCHDOG_USEC;
        else
            process.env.WATCHDOG_USEC = prevWatchdogUsec;
    });
