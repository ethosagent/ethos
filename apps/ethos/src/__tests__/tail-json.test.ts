...
function jsonRoundTrip(event: ObsEvent): ObsEvent {
  return JSON.parse(JSON.stringify(event)) as ObsEvent;
}
...
describe('tail --json round-trip', () => {
  ...
  it('round-trips strings containing quotes and newlines without corruption', () => {
    ...
    const parsed = JSON.parse(line.trimEnd()) as ObsEvent;
  ...
  it('each NDJSON line is a complete self-contained JSON object', () => {
    ...
    for (const line of ndjson.split('\n')) {
      ...
      const parsed = JSON.parse(line) as ObsEvent;
