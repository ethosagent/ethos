...
import { ethosDir, loadConfigStrict, validateNoPlaintextSecrets, } from '../config';
function secretRef(path) {
...
describe('validateNoPlaintextSecrets', () => {
    ...
    it('rejects Anthropic API key in apiKey field (field-aware)', () => {
        ...
        expect(() => validateNoPlaintextSecrets(config)).toThrow(/field 'apiKey'.*field requires \$\{secrets:ref\}/);
    ...
    it('detects secrets in nested provider config (field-aware)', () => {
        ...
        expect(() => validateNoPlaintextSecrets(config)).toThrow(/field 'providers\[0\]\.apiKey'.*field requires \$\{secrets:ref\}/);
    });
    it('detects secrets in slack apps config (field-aware)', () => {
        ...
        expect(() => validateNoPlaintextSecrets(config)).toThrow(/field 'slack\.apps\[0\]\.botToken'.*field requires \$\{secrets:ref\}/);
    });
    ...
    it('detects secrets in auxiliary compression apiKey (field-aware)', () => {
        ...
        expect(() => validateNoPlaintextSecrets(config)).toThrow(/field 'auxiliary\.compression\.apiKey'.*field requires \$\{secrets:ref\}/);
    });
    ...
    it('rejects an opaque Telegram token in a secret field', () => {
        ...
        expect(() => validateNoPlaintextSecrets(config)).toThrow(/field 'telegramToken'.*field requires \$\{secrets:ref\}/);
    });
...
describe('loadConfigStrict skips validation without secrets resolver', () => {
    it('loads config with plaintext keys when no secrets resolver is provided', async () => {
        ...
        await storage.write(join(ethosDir(), 'config.yaml'), 'provider: anthropic\nmodel: claude-opus-4-7\napiKey: sk-ant-local-dev-key\npersonality: researcher\n');
...
describe('loadConfigStrict rejects plaintext secrets when resolver is present', () => {
    it('throws when secrets resolver is configured and config has plaintext key', async () => {
        ...
        await expect(loadConfigStrict(storage, secrets)).rejects.toThrow(/field requires \$\{secrets:ref\}/);
    });
