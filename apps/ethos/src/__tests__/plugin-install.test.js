...
describe('findInstalledPkgDir', () => {
    it('returns the installed package directory when it exists', async () => {
        ...
        await writeFile(join(testDir, 'package.json'), JSON.stringify({ dependencies: { 'my-plugin': '^1.0.0' } }));
        await mkdir(join(testDir, 'node_modules', 'my-plugin'), { recursive: true });
        await writeFile(join(testDir, 'node_modules', 'my-plugin', 'package.json'), JSON.stringify({ name: 'my-plugin', version: '1.0.0' }));
    ...
    it('fails closed when the npm manifest lists a package but node_modules dir is absent', async () => {
        ...
        await writeFile(join(testDir, 'package.json'), JSON.stringify({ dependencies: { 'unresolvable-pkg': 'git+https://example.com/repo' } }));
    ...
    it('fails closed when manifest has multiple dependencies (ambiguous)', async () => {
        await writeFile(join(testDir, 'package.json'), JSON.stringify({ dependencies: { 'pkg-a': '1.0.0', 'pkg-b': '2.0.0' } }));
