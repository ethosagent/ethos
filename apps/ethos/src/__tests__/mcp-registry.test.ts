...
describe('mcp registry — package name derivation', () => {
   ...
   */
  function deriveServerName(packageName: string): string {
...
describe('mcp registry — duplicate detection', () => {
  ...
  function hasDuplicate(existing: MinimalConfig[], name: string): boolean {
...
describe('mcp registry — search URL construction', () => {
  ...
  it('builds URL without search term', () => {
    expect(buildSearchUrl('')).toBe(
      'https://registry.npmjs.org/-/v1/search?text=keywords:mcp-server&size=20',
    );
  });

  it('builds URL with search term', () => {
    expect(buildSearchUrl('sqlite')).toBe(
      'https://registry.npmjs.org/-/v1/search?text=keywords:mcp-server+sqlite&size=20',
    );
  });

  it('builds URL with multi-word search', () => {
    expect(buildSearchUrl('file system')).toBe(
      'https://registry.npmjs.org/-/v1/search?text=keywords:mcp-server+file system&size=20',
    );
  });
});

describe('mcp registry — search arg parsing', () => {
  function parseSearchArg(argv: string[]): string {
    let search = '';
