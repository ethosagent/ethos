...
const CONFIG_REFERENCE_DOC = join(REPO_ROOT, 'docs', 'content', 'using', 'reference', 'config-yaml.md');
const PERSONALITY_REFERENCE_DOC = join(REPO_ROOT, 'docs', 'content', 'using', 'reference', 'personality-yaml.md');
/**
 ...
 * can honor `@internal`.
 */
function extractFields(src, interfaceName) {
    const re = new RegExp(`export interface ${interfaceName}\\s*\\{`, 'm');
    const m = re.exec(src);
    if (!m)
        throw new Error(`${interfaceName} not found in source`);
 ...
 */
function isDocumented(field, doc) {
    ...
    for (const line of lines) {
        ...
        if (!fieldExact && !fieldGroup)
            continue;
        if (restCells.length >= 3 && /[a-zA-Z]/.test(restCells))
            return true;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (yamlMatch) {
            ...
            if (comment.length > 0 && /[a-zA-Z]/.test(comment))
                return true;
        ...
        if (new RegExp(`\\b${field}\\b`).test(line)) {
            ...
            for (let j = start; j <= end; j++) {
                ...
                if (!proseLine)
                    continue;
                if (proseLine.startsWith('|'))
                    continue; // table fragments handled in (a)
                if (proseLine.startsWith('#'))
                    continue; // markdown headings aren't sentences
                // Must contain the field name AND ≥ 5 letter-words.
                if (!new RegExp(`\\b${field}\\b`, 'i').test(proseLine))
                    continue;
...
describe('config surface doc-sync', () => {
    ...
    describe.skip('EthosConfig fields are documented in config-yaml.md', () => {
        ...
        for (const field of fields) {
            ...
            it(`documents \`${field.name}\``, () => {
                expect(isDocumented(field.name, doc), `EthosConfig.${field.name} is not documented in using/reference/config-yaml.md.\n` +
                    `Add either:\n` +
                    ...
                    `If \`${field.name}\` is not user-facing, mark it with \`@internal\` in apps/ethos/src/config.ts.`).toBe(true);
            });
        }
    });
    ...
    describe.skip('PersonalityConfig fields are documented in personality-yaml.md', () => {
        ...
        for (const field of fields) {
            if (field.internal)
                continue;
            it(`documents \`${field.name}\``, () => {
                expect(isDocumented(field.name, doc), `PersonalityConfig.${field.name} is not documented in using/reference/personality-yaml.md.\n` +
                    `Add either:\n` +
                    `  (a) a markdown table row whose first cell names \`${field.name}\` and whose remaining cells describe it, or\n` +
                    `  (b) a YAML code-block line: \`${field.name}: <value>  # <comment>\`.\n` +
                    `If \`${field.name}\` is internal/derived (populated by the loader, not user-set), mark it with \`@internal\` in packages/types/src/personality.ts.`).toBe(true);
            });
        }
    });
    describe('docs reference no removed fields', () => {
        ...
        const REMOVED_ETHOS_FIELDS = [];
        const REMOVED_PERSONALITY_FIELDS = [];
        it('config-yaml.md does not document removed EthosConfig fields', () => {
            const doc = safeRead(CONFIG_REFERENCE_DOC);
            for (const field of REMOVED_ETHOS_FIELDS) {
                expect(isDocumented(field, doc), `EthosConfig.${field} was removed from the schema but using/reference/config-yaml.md still documents it. Remove the doc entry.`).toBe(false);
            }
        });
        it('personality-yaml.md does not document removed PersonalityConfig fields', () => {
            const doc = safeRead(PERSONALITY_REFERENCE_DOC);
            for (const field of REMOVED_PERSONALITY_FIELDS) {
                expect(isDocumented(field, doc), `PersonalityConfig.${field} was removed from the schema but using/reference/personality-yaml.md still documents it. Remove the doc entry.`).toBe(false);
            }
        });
    });
});
