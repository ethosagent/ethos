...
import { EnvSecretsResolver, InMemorySecretsResolver, InMemoryStorage, MergedSecretsResolver, } from '@ethosagent/storage-fs';
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { ethosDir, readConfig, readKeys, readRawConfig, writeConfig, } from '../config';
function secretRef(path) {
    return ['${', 'secrets:', path, '}'].join('');
}
describe('secrets ref substitution in config', () => {
    async function load(yaml, secrets) {
...
describe('readKeys with secrets resolution', () => {
    it('resolves secret refs in key apiKey fields', async () => {
        ...
        await storage.mkdir(ethosDir());
        await storage.write(join(ethosDir(), 'keys.json'), JSON.stringify([{ apiKey: secretRef('rotation/backup'), priority: 50, label: 'backup' }]));
    ...
    it('works without secrets resolver', async () => {
        const storage = new InMemoryStorage();
        await storage.mkdir(ethosDir());
        await storage.write(join(ethosDir(), 'keys.json'), JSON.stringify([{ apiKey: 'sk-plain', priority: 50 }]));
...
describe('env-only resolution via MergedSecretsResolver', () => {
    ...
    async function loadWithMerged(yaml) {
        const storage = new InMemoryStorage();
        await storage.mkdir(ethosDir());
        await storage.write(join(ethosDir(), 'config.yaml'), yaml);
