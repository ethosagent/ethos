...
import { migrateSessionKeys } from '@ethosagent/session-sqlite';
import { deriveBotKey, ethosDir } from '../config';
...
export async function migrateSessionKeysIfNeeded(opts) {
    ...
    if (await opts.storage.exists(marker))
        return null;
    ...
    const knownByPlatform = new Map();
    const primaryByPlatform = new Map();
    for (const bot of opts.config.telegram?.bots ?? []) {
        ...
        if (!primaryByPlatform.has('telegram'))
            primaryByPlatform.set('telegram', k);
    }
    for (const app of opts.config.slack?.apps ?? []) {
        ...
        if (!primaryByPlatform.has('slack'))
            primaryByPlatform.set('slack', k);
    }
    ...
    opts.logger?.info(`[session-keys] migration done: ${result.migrated} migrated, ${result.alreadyMigrated} already, ` +
