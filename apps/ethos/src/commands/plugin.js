...
export async function runPlugin(args) {
    ...
    switch (sub) {
        case 'install': {
            ...
            try {
                ...
                if (err instanceof EthosError) {
                    ...
                    console.error(`${c.red}Install failed: ${err instanceof Error ? err.message : String(err)}${c.reset}`);
                }
                process.exit(1);
            }
            break;
        ...
        default:
            console.log('Usage: ethos plugin [install <pkg> | remove <pkg> | list | credentials <pluginId>]');
    }
}
// ---------------------------------------------------------------------------
...
async function installPlugin(pkg) {
    ...
    console.log(`${c.dim}Downloading ${c.reset}${c.bold}${pkg}${c.reset}${c.dim} for safety scan...${c.reset}\n`);
    // Step 1: download without running install scripts so we can scan first
    const pre = spawnSync('npm', ['install', '--prefix', tmpDir, '--ignore-scripts', '--no-audit', pkg], { stdio: 'inherit' });
    if (pre.status !== 0) {
    ...
    try {
        ...
        try {
            const rawMeta = JSON.parse(await readFile(join(pkgDir, 'package.json'), 'utf-8'));
            ...
            if (metaName && metaVersion)
                exactSpec = `${metaName}@${metaVersion}`;
        }
        catch {
        ...
        if (hasRed || hasYellow) {
            ...
            for (const f of findings) {
                ...
                console.log(`  ${color}${f.severity === 'red' ? '✗' : '⚠'} ${f.severity}${c.reset}  ${f.rule}${loc}`);
                if (f.message)
                    console.log(`     ${c.dim}${f.message}${c.reset}`);
                if (f.excerpt)
                    console.log(`     ${c.dim}${f.excerpt}${c.reset}`);
            }
        }
        // Step 7: decide
    ...
    const result = spawnSync('npm', ['install', '--prefix', dir, '--ignore-scripts', '--no-audit', exactSpec], { stdio: 'inherit' });
    if (result.status !== 0) {
 ...
 */
export async function findInstalledPkgDir(tmpDir, pkgArg) {
    let manifest = {};
    try {
        manifest = JSON.parse(await readFile(join(tmpDir, 'package.json'), 'utf-8'));
    }
    catch {
    ...
    try {
        ...
        throw new EthosError({
            ...
            action: 'The package may have installed under a different name. Use a plain npm package name rather than a tarball, git URL, or local path.',
        });
    }
    return pkgDir;
}
async function readPluginPermissions(pkgDir) {
    try {
        const raw = JSON.parse(await readFile(join(pkgDir, 'package.json'), 'utf-8'));
...
async function walkAndScan(dir, permissions, out) {
    let entries;
    try {
    ...
    for (const e of entries) {
        ...
        else if (/\.[jt]sx?$|\.(?:cjs|mjs)$/.test(name) &&
            !name.endsWith('.d.ts') &&
            !name.endsWith('.d.cts') &&
            !name.endsWith('.d.mts')) {
            const src = await readFile(join(dir, name), 'utf-8').catch(() => null);
            if (src)
                out.push(...scanPluginCode(src, permissions).findings);
        }
    }
}
...
// Credentials
// ---------------------------------------------------------------------------
async function runCredentials(args) {
    const pluginId = args[0];
    if (!pluginId) {
        console.log('Usage: ethos plugin credentials <pluginId> [--list | --set KEY[=VALUE] | --clear KEY]');
        process.exit(1);
    }
...
async function readPluginCredentialDeclarations(pluginId) {
    const dir = pluginsDir();
    const candidates = [
    ...
    for (const pkgPath of candidates) {
        try {
            ...
            const ethosField = raw.ethos;
            if (ethosField && Array.isArray(ethosField.credentials)) {
                return ethosField.credentials;
            }
        }
        catch {
...
async function listCredentials(pluginId, credDir) {
...
async function setCredential(pluginId, credDir, keyArg) {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(key)) {
        console.error(`${c.red}Invalid key "${key}" -- must be alphanumeric, underscores, hyphens.${c.reset}`);
        process.exit(1);
    }
    await mkdir(credDir, { recursive: true });
    await writeFile(join(credDir, key), value, { mode: 0o600 });
    await writeFile(join(credDir, `${key}.meta`), JSON.stringify({ updatedAt: new Date().toISOString() }), { mode: 0o600 });
...
async function clearCredential(credDir, key) {
