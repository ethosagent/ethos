...
const USAGE = 'Usage: ethos slack [manifest [--yaml] [-o <file>] [--name <appname>] [--request-url <url>]]';
...
function printPasteInstructions(toStderr = false) {
    ...
    write(`${c.dim}  2.${c.reset} Pick ${c.bold}"From a manifest"${c.reset} and select your workspace`);
    write(`${c.dim}  3.${c.reset} Paste the manifest above`);
    write(`${c.dim}  4.${c.reset} Click ${c.bold}"Create"${c.reset}, then ${c.bold}"Install to Workspace"${c.reset}`);
    write(`${c.dim}  5.${c.reset} Copy the bot/app/signing values into ${c.cyan}~/.ethos/config.yaml${c.reset}:`);
