...
export async function runSecrets(args) {
    ...
    switch (sub) {
        ...
        case 'set': {
            ...
            if (!ref || !value) {
                ...
                console.log(`${c.dim}Example: ethos secrets set providers/anthropic/apiKey sk-ant-...${c.reset}`);
                process.exit(1);
            }
            await resolver.set(ref, value);
            console.log(`${c.green}✓ Secret set${c.reset}  ${c.cyan}${ref}${c.reset}  ${maskValue(value)}`);
            break;
        }
        case 'get': {
        ...
        default:
            console.log('Usage: ethos secrets [list | set <ref> <value> | get <ref> | remove <ref> | path]');
    }
}
