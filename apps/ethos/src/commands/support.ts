...
function stripSecrets<T>(obj: T): T {
  ...
  if (Array.isArray(obj)) return obj.map((v) => stripSecrets(v)) as unknown as T;
...
function buildAnonymizer(cwd: string): (s: string) => string {
  const home = homedir();
  const host = hostname();
...
function parseBundleFlags(argv: string[]): BundleFlags {
  const flags: BundleFlags = { includeBodies: false, includeMemory: false, anonymize: false };
...
export async function runBundle(argv: string[]): Promise<void> {
  ...
  const anon = flags.anonymize ? buildAnonymizer(process.cwd()) : (s: string) => s;
  ...
  try {
    ...
    const snapshotIds = [
      ...new Set(traces.map((t) => t.snapshotId).filter((id): id is string => id != null)),
    ];
    ...
    if (flags.includeBodies) {
      ...
      const bodyRefs = spans
        .map((s) => s.attrs?.body_ref as string | undefined)
    ...
    // Print manifest
    if (traces.length > 0) {
      const kindCounts = traces.reduce<Record<string, number>>((acc, t) => {
...
interface BundleData {
  ...
  spans: Span[];
  events: ObsEvent[];
  snapshots: Array<{ snapshotId: string; personalityId: string; takenAt: number; body: string }>;
}

function parseJsonlBuf(buf: Buffer | undefined): unknown[] {
...
function loadBundle(bundlePath: string): BundleData {
  const raw = readFileSync(bundlePath);
  const files = readTarGz(raw);

  const sysRaw = files.get('system.json');
  const system = sysRaw ? (JSON.parse(sysRaw.toString('utf8')) as Record<string, unknown>) : {};

  const traces = parseJsonlBuf(files.get('traces.jsonl')) as Trace[];
  const spans = parseJsonlBuf(files.get('spans.jsonl')) as Span[];
  const events = parseJsonlBuf(files.get('events.jsonl')) as ObsEvent[];
  const snapshots = parseJsonlBuf(files.get('snapshots.jsonl')) as Array<{
    snapshotId: string;
...
export async function runInspect(argv: string[]): Promise<void> {
  ...
  const bundleId = (system.bundleId as string | undefined) ?? 'unknown';
  const version = (system.ethosVersion as string | undefined) ?? 'unknown';
  const platform = (system.platform as string | undefined) ?? 'unknown';
  const filter = system.filter as
    | { since: string | null; until: string | null; session: string | null }
    | undefined;
  ...
  if (transitions.length > 0) {
    ...
    for (const t of transitions) {
      ...
      if (t.details?.diff && typeof t.details.diff === 'object') {
        const diff = t.details.diff as Record<string, unknown>;
...
export async function runSupport(sub: string, argv: string[]): Promise<void> {
  if (sub === 'bundle') {
