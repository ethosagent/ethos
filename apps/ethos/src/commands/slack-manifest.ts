...
export async function runSlackManifest(args: string[]): Promise<void> {
...
function printPasteInstructions(toStderr = false): void {
  const write = toStderr ? (s: string) => process.stderr.write(`${s}\n`) : console.log;
  write('');
...
function buildManifest(opts: ManifestOpts): Record<string, unknown> {
