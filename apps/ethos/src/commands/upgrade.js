...
const CURRENT_VERSION = typeof __ETHOS_VERSION__ === 'string' ? __ETHOS_VERSION__ : (process.env.ETHOS_VERSION ?? 'dev');
export async function runUpgrade() {
    ...
    try {
        ...
        console.error(`${c.dim}  Check your network and try again, or install manually: npm install -g ${PACKAGE}@latest${c.reset}`);
        process.exit(1);
    }
    console.log(`\n  ${c.dim}Current:${c.reset} ${c.bold}${CURRENT_VERSION}${c.reset}`);
    ...
    console.log(`${c.dim}Running ${c.reset}${c.cyan}npm install -g ${PACKAGE}@latest${c.reset}${c.dim}...${c.reset}\n`);
    ...
    console.log(`${c.dim}  Run ${c.reset}${c.cyan}ethos --version${c.reset}${c.dim} in a fresh terminal to confirm.${c.reset}`);
}
/**
...
function detectInstallMethod() {
    ...
    if (path.includes('/node_modules/@ethosagent/cli/'))
        return 'npm';
    return 'source';
}
function printSourceInstructions() {
    console.log(`${c.yellow}⚠${c.reset} This binary is running from source ${c.dim}(${process.argv[1] ?? 'unknown path'})${c.reset}.`);
    console.log('');
