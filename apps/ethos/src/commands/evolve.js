...
import { loadEvolveConfig, runEvolveApply, runEvolveStatus, SkillEvolver, } from '@ethosagent/skill-evolver';
...
export async function runEvolve(args, config) {
...
async function runEvolveRun(args, config, dir) {
    ...
    try {
        ...
        if (!quiet)
            console.error(`${c.red}Failed to export sessions:${c.reset} ${msg}`);
        return;
    }
    if (!wroteRecords) {
        if (!quiet)
 ...
 */
async function exportSessionsToEval(dbPath, outPath) {
    ...
    try {
        ...
        const rows = db
            .prepare(`SELECT s.key AS session_key, m.role, m.content
           FROM messages m
...
async function runAnalyze(evalOutput, config, skillsDir, pendingDir, autoApprove) {
    try {
    ...
    console.log(`${c.bold}ethos evolve${c.reset}  ${c.dim}eval: ${evalOutput} · model: ${llm.model}${c.reset}`);
    ...
    if (result.rewritesWritten.length > 0) {
        ...
        for (const f of result.rewritesWritten)
            console.log(`  ${join(pendingDir, f)}`);
    }
    if (result.newSkillsWritten.length > 0) {
        console.log(`${c.green}new skills written:${c.reset}`);
        for (const f of result.newSkillsWritten)
            console.log(`  ${join(pendingDir, f)}`);
    }
    if (result.skipped.length > 0) {
        console.log(`${c.yellow}skipped:${c.reset}`);
        for (const s of result.skipped)
            console.log(`  ${s.kind} ${s.target} — ${s.reason}`);
    }
    if (result.rewritesWritten.length === 0 &&
...
async function listPending(pendingDir) {
    ...
    try {
        ...
        if (mds.length > 0)
            sections.push({ label: pendingDir, files: mds });
    }
    catch {
...
async function approveAll(pendingDir, skillsDir) {
    let entries;
    try {
...
async function approveOne(fileName, pendingDir, skillsDir) {
...
async function rejectOne(fileName, pendingDir) {
    const safe = ensureSafeFilename(fileName);
    if (!safe) {
        console.error(`${c.red}Invalid filename: ${fileName}${c.reset}`);
        process.exit(1);
...
async function autoPendingPaths(safe) {
