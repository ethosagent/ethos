...
function parseArgs(args) {
    ...
    if (!inputPath)
        throw new EthosError({
            ...
            action: 'Usage: ethos eval run <tasks.jsonl> --expected <expected.jsonl> [--scorer exact|contains|regex|llm] [--concurrency N] [--output out.jsonl] [--evolve [--auto-approve]]',
        });
    if (!expectedPath)
        throw new EthosError({
            code: 'INVALID_INPUT',
...
export async function runEval(subArgs, config) {
    ...
    try {
        ...
        console.error(`${c.red}Invalid expected file: ${err instanceof Error ? err.message : String(err)}${c.reset}`);
        process.exit(1);
    }
    if (tasks.length === 0) {
    ...
    console.log(`${c.bold}ethos eval${c.reset}  ${c.dim}${tasks.length} tasks · scorer: ${scorer} · concurrency ${concurrency}${c.reset}`);
    ...
    console.log(`${c.dim}output → ${outputPath}${c.reset}`);
    if (evolve)
        await runEvolveAfter(config, outputPath, autoApprove);
    if (stats.failed > 0)
        process.exit(1);
}
async function runEvolveAfter(config, evalOutputPath, autoApprove) {
    const dir = ethosDir();
    ...
    console.log(`  rewrites: ${result.rewritesWritten.length}  new: ${result.newSkillsWritten.length}  skipped: ${result.skipped.length}`);
    for (const f of result.rewritesWritten)
        console.log(`  ${c.green}rewrite${c.reset} ${f}`);
    ...
    if (autoApprove) {
        ...
        if (all.length > 0)
            console.log(`  ${c.green}auto-approved${c.reset} ${all.length} file(s)`);
    }
