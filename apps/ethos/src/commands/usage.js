...
export async function runUsage(argv) {
    ...
    try {
        ...
        if (jsonMode) {
            ...
            console.log(`  Total tokens: ${totals.inputTokens.toLocaleString()} in / ${totals.outputTokens.toLocaleString()} out`);
            ...
            if (result.byProvider.length > 0) {
                ...
                for (const p of result.byProvider) {
                    console.log(`    ${p.provider}/${p.model}: $${p.estimatedCostUsd.toFixed(2)}` +
            ...
            if (result.byPersonality.length > 0) {
                ...
                for (const p of result.byPersonality) {
                    console.log(`    ${p.personality}: $${p.estimatedCostUsd.toFixed(2)} (${p.turnCount} turns)`);
                }
            }
            if (truncated) {
