...
import { archiveMonth, BlobStore, getSqliteStats, listArchives, mergeRetentionConfig, ObservabilityService, pruneObservabilityByPath, SQLiteObservabilityStore, } from '@ethosagent/observability-sqlite';
...
async function runPrune(argv) {
    ...
    if (flags.olderThan && flags.category) {
        ...
        if (cat === 'traces')
            effectiveConfig.traces = dur;
        else if (cat === 'spans')
            effectiveConfig.spans = dur;
    ...
    console.log(`${prefix} ${result.traces} trace(s), ${result.spans} span(s), ${result.events} event(s), ${result.snapshots} orphaned snapshot(s), ${result.messages} message(s).`);
    if (flags.dryRun) {
        console.log('(Dry run — no changes made)');
    }
}
async function runReset(argv) {
    const flags = parseFlags(argv);
    const dir = ethosDir();
    const obsDbPath = join(dir, 'observability.db');
...
async function runArchiveCommand(argv) {
    const dir = ethosDir();
    const archiveDir = join(dir, 'archive');
    const obsDbPath = join(dir, 'observability.db');
    const storage = getStorage();
    ...
    console.log(`Archived ${result.traces} trace(s), ${result.spans} span(s), ${result.events} event(s), ${result.snapshots} snapshot(s) → archive/${month}.tar.gz`);
}
export async function runData(sub, argv) {
    if (sub === 'stats' || sub === '') {
        await runStats(argv);
        return;
    }
    ...
    console.log('Usage: ethos data [stats | prune [--dry-run] [--personality <id>] | reset [--dry-run] [--blobs-only] | archive [list | --month YYYY-MM | --dry-run]]');
}
