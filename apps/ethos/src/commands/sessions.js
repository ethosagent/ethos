...
export function formatSessionCost(usage) {
    const { inputTokens, outputTokens, cacheReadTokens, cacheCreationTokens, estimatedCostUsd } = usage;
    const totalInput = inputTokens + cacheReadTokens + cacheCreationTokens;
    const cacheSavingsPct = totalInput > 0 ? Math.min(99, Math.max(0, Math.round((cacheReadTokens / totalInput) * 90))) : 0;
    const tokenLine = `in=${inputTokens.toLocaleString()}  out=${outputTokens.toLocaleString()}` +
...
export async function runSessionShow(argv) {
    ...
    try {
        ...
        const compactionSuffix = compactionCount > 0 && firstCompaction
        ...
        switch (sub) {
            ...
            case '': {
                ...
                for (const item of items) {
                    console.log(item.id.slice(0, idW - 1).padEnd(idW) +
