...
function personalityBot(name: string): BotBindingEntry {
  return { binding: { type: 'personality', name } };
}

function teamBot(teamName: string): BotBindingEntry {
  return { binding: { type: 'team', name: teamName } };
}

function makeDeps(overrides: Partial<TeamSupervisorDeps> = {}): TeamSupervisorDeps {
  return {
