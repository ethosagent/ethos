 ...
 */
function craftTarEntry(name: string, content: Buffer, typeFlag = 0x30): Buffer {
...
describe('parseTar — Zip Slip defense', () => {
  ...
  it('rejects symlink entries (type flag 0x32)', () => {
    ...
    expect(() => parseTar(tar)).toThrow(
      'Unsupported tar entry type 50 for "personalities/test/link"',
    );
  });
