...
describe('team supervisor launch args', () => {
    it('uses tsx loader when entry point is TypeScript', () => {
        const args = buildSupervisorLaunchArgs('/repo/apps/ethos/src/index.ts', 'demo', '/Users/me/.ethos/teams/demo.yaml');
        expect(args).toEqual([
            '--import',
            'tsx',
    ...
    it('uses plain node entry when entry point is JavaScript', () => {
        const args = buildSupervisorLaunchArgs('/usr/local/lib/node_modules/@ethosagent/cli/dist/index.js', 'demo', '/tmp/demo.yaml');
        expect(args).toEqual([
