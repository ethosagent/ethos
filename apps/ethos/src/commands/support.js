...
function stripSecrets(obj) {
    ...
    if (Array.isArray(obj))
        return obj.map((v) => stripSecrets(v));
    const out = {};
...
export async function runBundle(argv) {
    ...
    try {
        ...
        const toJsonl = (arr) => Buffer.from(arr.map((r) => anon(JSON.stringify(r))).join('\n'), 'utf8');
        const files = new Map();
        ...
        console.log(`  ✓ ${allEvents.length} event(s)          (${errorEvents.length} error, ${auditEvents.length} audit)`);
        console.log(`  ✓ ${snapshots.length} policy snapshot(s)`);
        console.log(`  ✓ Ethos version + OS + Node version`);
        if (safeConfig)
            console.log(`  ✓ Config (sensitive fields removed)`);
        ...
        if (flags.anonymize)
            console.log('  (paths and hostnames anonymized)');
    }
    finally {
        store.close();
    }
...
export function diagnoseBundleLines(traces, spans, events) {
    ...
    const autoApprovals = events.filter((e) => e.category === 'audit.approval' && e.details?.auto === true);
    ...
    if (toolSpans.length > 0) {
        const slowest = [...toolSpans].sort((a, b) => (b.endTs ?? b.startTs) - b.startTs - ((a.endTs ?? a.startTs) - a.startTs))[0];
        if (slowest?.endTs) {
    ...
    if (lines.length === 0) {
        ...
        if (traces.length === 0)
            lines.push('  (bundle contains no traces — window may be empty)');
    }
    return lines;
}
export async function runInspect(argv) {
        ...
        for (const t of transitions) {
            ...
            if (t.details?.diff && typeof t.details.diff === 'object') {
                ...
                for (const [field, change] of Object.entries(diff)) {
                    const c = change;
                    if (c)
                        console.log(`    ⚠ ${field}: ${String(c.from)} → ${String(c.to)}`);
                }
            }
        }
        console.log();
    ...
    if (toolSpans.length > 0) {
        ...
        console.log(`TOOL CALLS (${toolSpans.length} total${omitted > 0 ? ` — showing first 20` : ''})`);
        for (const s of shown) {
            const dur = fmtDuration(s.startTs, s.endTs);
            const status = s.status === 'ok' ? '✓' : s.status === 'blocked' ? '✗ blocked' : (s.status ?? '?');
            const argsPreview = s.attrs?.args ? ` '${JSON.stringify(s.attrs.args).slice(0, 40)}'` : '';
            console.log(`  ${fmtTime(s.startTs)}  ${s.name.padEnd(20)}${argsPreview.padEnd(44)}  ${status}  ${dur}`);
        }
        if (omitted > 0)
            console.log(`  … ${omitted} more tool calls`);
        console.log();
    }
    // ── EVENTS OF NOTE ────────────────────────────────────────────────────────
    ...
    if (notable.length > 0) {
        ...
        if (notable.length > 15)
            console.log(`  … ${notable.length - 15} more events`);
        console.log();
    }
    // ── POLICY SNAPSHOTS ──────────────────────────────────────────────────────
...
export async function runSupport(sub, argv) {
    ...
    console.log('Usage: ethos support [bundle [--since <ts>] [--until <ts>] [--session <id>] [--include-bodies] [--include-memory] [--anonymize] | inspect <bundle.tar.gz> [--json]]');
}
