...
const CHANNEL_SDKS = [
    ...
    },
    {
        ...
        module: '@slack/bolt',
        required: false,
        configuredWhen: (cfg) => Boolean(cfg.slackBotToken && cfg.slackAppToken && cfg.slackSigningSecret),
    },
    {
        label: 'Email (IMAP)',
        module: 'imapflow',
...
async function checkSecrets(config) {
...
async function checkSdk(modulePath) {
    try {
...
async function checkAwsSecrets(config) {
...
export async function runDoctor(args = [], options) {
    ...
    if (jsonMode) {
        ...
        const exitCode = coreFailures.length > 0 || configuredMissing.length > 0 || awsFailed || requiredSecretMissing
            ? 1
            : 0;
        const result = {
    ...
    if (!config) {
        ...
        console.log(`      ${c.dim}Run ${c.reset}${c.cyan}ethos setup${c.reset}${c.dim} to create one.${c.reset}`);
        console.log(`      ${c.dim}Or: ${c.reset}${c.cyan}ethos doctor --fix${c.reset}${c.dim} → ${c.cyan}ethos setup auth${c.reset}${c.dim} → ${c.cyan}ethos setup model${c.reset}`);
    }
    else {
    ...
    for (const s of secretResults) {
        ...
        if (s.present) {
            ...
            console.log(`  ${c.red}✗${c.reset}  ${keyCol} ${c.red}missing${c.reset}   ${c.dim}${s.fillWith}${c.reset}`);
        }
    }
    console.log('');
    // -------------------------------------------------------------------------
    ...
    if (config?.aws?.secrets?.enabled) {
        ...
        try {
            ...
            console.log(`  ${c.green}✓${c.reset}  Reachable, ${refs.length} secret${refs.length === 1 ? '' : 's'} visible under ${c.dim}${config.aws.secrets.prefix ?? 'ethos'}/${c.reset}`);
        }
        catch (err) {
    ...
    if (await storage.exists(userPersonalitiesDir)) {
        console.log(`  ${c.green}✓${c.reset}  Custom personalities dir: ${c.dim}${userPersonalitiesDir}${c.reset}`);
    }
    else {
        console.log(`  ${c.dim}–  No custom personalities directory yet (built-ins still work). Create ${userPersonalitiesDir}/ to add your own.${c.reset}`);
    }
    console.log('');
    // -------------------------------------------------------------------------
    // Core SDKs
    ...
    for (const row of CORE_SDKS) {
        ...
        printRow(coreResults.at(-1));
    }
    console.log('');
    // -------------------------------------------------------------------------
    // Channel SDKs
    // -------------------------------------------------------------------------
    console.log(`${c.bold}Channel SDKs${c.reset}  ${c.dim}(optional — only matters when configured)${c.reset}`);
    const channelResults = [];
    for (const row of CHANNEL_SDKS) {
        const { ok } = await checkSdk(row.module);
        const inUse = config ? Boolean(row.configuredWhen?.(config)) : false;
        channelResults.push({ row, ok, inUse });
        printRow(channelResults.at(-1));
    }
    console.log('');
    // -------------------------------------------------------------------------
    // Skill prerequisites
    // -------------------------------------------------------------------------
    console.log(`${c.bold}Skill prerequisites${c.reset}  ${c.dim}(external CLIs declared by bundled skills)${c.reset}`);
    ...
    if (skillIssues.length === 0) {
        ...
        for (const issue of skillIssues) {
            console.log(`  ${c.yellow}⚠${c.reset}  ${c.bold}${issue.skill}${c.reset} needs ${c.cyan}${issue.cli}${c.reset} ${c.dim}(not on PATH)${c.reset}`);
        }
    }
    console.log('');
    // -------------------------------------------------------------------------
        ...
        try {
            ...
            if (results.length === 0) {
                ...
                for (const r of results) {
                    const icon = r.result.status === 'ok'
                        ? `${c.green}✓${c.reset}`
                        : r.result.status === 'warn'
                            ? `${c.yellow}⚠${c.reset}`
                            : `${c.red}✗${c.reset}`;
                    const duration = r.result.durationMs != null
            ...
            console.log(`  ${c.yellow}⚠${c.reset}  ${c.dim}Plugin health checks failed: ${err instanceof Error ? err.message : String(err)}${c.reset}`);
        }
        console.log('');
    }
    // -------------------------------------------------------------------------
    ...
    if (coreFailures.length > 0) {
        console.log(`${c.red}✗ Core SDK missing — ethos cannot run.${c.reset} Reinstall: ${c.cyan}npm install -g @ethosagent/cli${c.reset}`);
        exitCode = 1;
    }
    if (configuredButMissing.length > 0) {
        ...
        console.log(`${c.dim}  Install the listed packages globally, or remove the channel from ~/.ethos/config.yaml.${c.reset}`);
        console.log(`${c.dim}  Or try: ${c.reset}${c.cyan}ethos doctor --fix${c.reset}${c.dim} → ${c.cyan}ethos setup auth${c.reset}${c.dim} → ${c.cyan}ethos setup model${c.reset}`);
        exitCode = 1;
    }
...
async function runDoctorFix() {
    ...
    if (config) {
        ...
        if (!knownIds.includes(config.provider)) {
            ...
            console.log(`  ${c.yellow}→ Action needed:${c.reset}  Unknown provider '${config.provider}'. Did you mean '${closest}'?`);
        ...
        console.log(`  ${c.yellow}→ Action needed:${c.reset}  No config found. Run: ${c.cyan}ethos setup${c.reset}`);
        exitCode = 1;
    }
    console.log('');
    if (exitCode === 0) {
        ...
        console.log(`${c.yellow}⚠ Some issues need manual action. Try: ${c.cyan}ethos doctor --fix → ethos setup auth → ethos setup model${c.reset}`);
        process.exit(exitCode);
    }
}
async function runProviderProbe(jsonMode) {
    const storage = getStorage();
    const config = await readRawConfig(storage);
    if (!config) {
        const result = {
        ...
        if (jsonMode) {
            ...
            console.log(`Provider connectivity: ${c.red}✗${c.reset} No config found — run ${c.cyan}ethos setup${c.reset}`);
        }
        process.exit(1);
    }
    ...
    const result = { provider, model, reachable, latencyMs, error, exit };
    if (jsonMode) {
        process.stdout.write(`${JSON.stringify(result)}\n`);
    }
    else if (reachable) {
        console.log(`Provider connectivity: ${c.green}✓${c.reset} ${provider} (${model}) — ${latencyMs}ms`);
    }
    else {
...
// ---------------------------------------------------------------------------
function runRecentErrorsReport() {
    console.log('');
...
function readExternalCliRequirements(skill) {
    const ethos = skill.rawFrontmatter.ethos;
    ...
    for (const item of raw) {
        if (typeof item === 'string') {
            ...
            const list = item.any_of;
            if (Array.isArray(list)) {
                for (const candidate of list) {
                    if (typeof candidate === 'string')
                        anyOf.push(candidate);
                }
            }
        }
    }
...
async function buildSkillsCliJson() {
    const skillIssues = await checkSkillPrerequisites();
