...
function statusColor(status) {
    ...
    if (status === 'aborted')
        return `${c.yellow}aborted${c.reset}`;
    if (status === 'blocked')
        return `${c.yellow}blocked${c.reset}`;
    return status;
}
function printTrace(t) {
    const dur = formatDuration(t.startTs, t.endTs);
    const status = statusColor(t.status);
    console.log(`  ${c.cyan}${t.traceId.slice(0, 8)}${c.reset}  ` +
        `${t.kind.padEnd(18)} ` +
        `${formatTs(t.startTs)}  ` +
        `${dur.padStart(8)}  ` +
        `${status}` +
...
function printSpanTimeline(spans, traceStart) {
    for (const s of spans) {
        ...
        console.log(`${indent}${c.dim}+${offset}ms${c.reset}  ` +
            `${s.kind.padEnd(12)} ` +
            `${s.name.padEnd(30)} ` +
            `${dur.padStart(8)}  ` +
            status);
...
export async function runTrace(args) {
    ...
    try {
        if (flags.traceId) {
            ...
            if (trace.subjectId)
                console.log(`  personality: ${trace.subjectId}`);
            if (spans.length > 0) {
        ...
        console.log(`  ${'ID'.padEnd(10)} ${'KIND'.padEnd(18)} ${'STARTED'.padEnd(20)} ${'DUR'.padStart(8)}  STATUS`);
