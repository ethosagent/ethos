...
function getRetentionValue(cfg, category) {
    if (!cfg)
        return undefined;
    switch (category) {
        case 'messages':
...
function setRetentionValue(cfg, category, value) {
    const out = { ...cfg };
    switch (category) {
        case 'messages':
            out.messages = value;
...
function deleteRetentionValue(cfg, category) {
    const out = { ...cfg };
    switch (category) {
        case 'messages':
            delete out.messages;
...
export async function runRetention(sub, argv) {
    ...
    if (sub === 'set') {
        ...
        if (!CATEGORY_LABELS[category]) {
            console.error(`Unknown category: ${category}. Valid: ${Object.keys(CATEGORY_LABELS).join(', ')}`);
            process.exit(1);
        }
        // Validate duration
        try {
    ...
    console.log('Usage: ethos retention [show [--personality <id>] [--defaults] | set <category> <duration> [--personality <id>] | reset <category|--all> [--personality <id>]]');
}
