...
import { renderStatusBar } from '../lib/status-bar';
...
import { formatVerboseSummary } from './verbose-timing';
async function buildInventory(loop, _config) {
...
function renderStatusBarLine(state) {
...
// ---------------------------------------------------------------------------
export async function runChat(config, opts = {}) {
    ...
    if (config.evolverCronEnabled) {
        ...
        startEvolverCron(schedule, config).catch((err) => {
            process.stderr.write(`[ethos] evolve cron startup failed: ${err instanceof Error ? err.message : String(err)}\n`);
        });
    }
    ...
    await scanSkillsIntoRegistry(storage, globalSkillsDir, personalitySkillsDir, registry, skillCache);
    // FW-16 — register user-defined quick commands.
    const quickCommands = config.quick_commands ?? {};
    ...
    loop.clarifyBridge?.setPresenter((req) => {
        ...
        const unsubscribe = loop.clarifyBridge?.onResolved((row) => {
            if (row.requestId === req.requestId)
                finish();
        }) ?? (() => { });
        rl.once('line', onLine);
    });
...
// ---------------------------------------------------------------------------
function handleBusyInput(input, state) {
...
function pushSteer(text, state) {
    ...
    if (state.iterationsThisTurn === 0) {
        ...
        out(`${c.dim}[steer → queued (pre-first-iteration), depth ${state.inputQueue.length}]${c.reset}\n`);
        return;
    }
...
function handleBusyCommand(arg, state) {
    if (!arg || arg === 'status') {
...
// Turn runner
// ---------------------------------------------------------------------------
async function runTurn(input, state, loop) {
    ...
    if (state.verbosity !== 'quiet') {
        out(`${c.bold}ethos${c.reset} ${c.dim}${spinner.frame()} thinking ${spinner.elapsed()}${c.reset}`);
    }
    const spinnerInterval = setInterval(() => {
        spinner.tick(Date.now());
        if (!spinnerCleared && state.verbosity !== 'quiet') {
            out(`\r${c.bold}ethos${c.reset} ${c.dim}${spinner.frame()} thinking ${spinner.elapsed()}${c.reset}`);
        }
    ...
    const turnAttachments = state.pendingAttachments.length > 0 ? state.pendingAttachments.splice(0) : undefined;
    try {
        ...
        if (stranded.length > 0 && state.verbosity !== 'quiet') {
            out(`${c.yellow}[discarded ${stranded.length} unread steer${stranded.length === 1 ? '' : 's'} — no tool seam in turn]${c.reset}\n`);
        }
        if (state.verbosity !== 'quiet')
            out('\n\n');
    }
}
function renderEventForVerbosity(event, state, ctx) {
    ...
    switch (event.type) {
        ...
        case 'tool_start':
            ...
            break;
        case 'tool_progress':
            if (state.verbosity === 'default' && event.audience !== 'user')
                break;
...
// ---------------------------------------------------------------------------
async function runSingleQuery(loop, config, input) {
    const verbosity = config.displayVerbosity ?? (config.verbose ? 'verbose' : 'default');
    const turnStart = Date.now();
    let firstTextDeltaAt = null;
    const toolDurations = [];
    let turnUsage = null;
...
async function handleSlashCommand(raw, state, loop, rl, _config, registry, ctx) {
    ...
    switch (name) {
        ...
        case 'personality': {
            ...
            if (arg === 'list') {
                out(`${c.dim}Built-ins: researcher · engineer · reviewer · coach · operator\n` +
        ...
        case 'model': {
            ...
            out(`${c.yellow}Model switching takes effect on next restart. Edit ~/.ethos/config.yaml to persist.${c.reset}\n`);
            break;
        }
        case 'tier': {
            const validTiers = ['trivial', 'default', 'deep'];
            if (!arg || arg === 'status') {
                out(`${c.dim}Tier override: use /tier [trivial|default|deep] to set for next turn.${c.reset}\n`);
                break;
            }
        ...
        case 'budget': {
            ...
            out(`${c.dim}Session spend: $${spent.toFixed(5)}\n` +
        ...
        case 'verbose': {
            ...
            out(`${c.yellow}Invalid level '${arg}' — falling back to default. Valid: quiet|default|verbose|debug${c.reset}\n`);
        ...
        case 'attach': {
            ...
            try {
                ...
                out(`${c.dim}[attached ${filename} (${(bytes.byteLength / 1024).toFixed(1)} KB) — send a message to include it]${c.reset}\n`);
            }
            catch (err) {
                out(`${c.red}Failed to attach: ${err instanceof Error ? err.message : String(err)}${c.reset}\n`);
            }
            break;
        }
        case 'dry-run': {
        ...
        default: {
            ...
            if (cmd?.prefix === '[quick]') {
                ...
                if (!ctx.isQuickConsentGiven()) {
                    ...
                    process.stdout.write(`Quick commands let you run shell commands as \`${process.env.USER ?? 'user'}\`. Continue? [y/N] `);
...
// ---------------------------------------------------------------------------
function handleBackgroundCommand(arg, state, loop) {
    if (!arg || arg === 'list') {
