...
async function scaffoldBlank(name) {
    ...
    if (!isSafePathSegment(id)) {
        console.error(`Invalid personality name "${id}": must not contain path separators, "..", or start with "."`);
        process.exit(1);
    }
    ...
    writeFileSync(join(dir, 'config.yaml'), `name: ${yamlScalar(name)}\ndescription: \nmodel: claude-sonnet-4-6\n`);
...
async function scaffoldFrom(sourceId, targetName) {
    ...
    console.log(`${c.dim}Test: ethos chat --personality ${targetName.toLowerCase().replace(/\s+/g, '-')}${c.reset}\n`);
}
async function runAiAssisted(name) {
    ...
    if (!reg.get('personality-architect')) {
        console.error('personality-architect personality not found. Is the framework installed correctly?');
        process.exit(1);
    }
