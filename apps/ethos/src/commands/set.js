...
function resolveTeamExists(name) {
    // Check local ./team.yaml
    try {
        const src = readFileSync('./team.yaml', 'utf-8');
        if (src.match(new RegExp(`^name:\\s*${name}\\s*$`, 'm')))
            return;
    }
    catch {
        // not found
...
export async function runSet(args) {
    ...
    if (sub === 'team') {
        ...
        console.log(`${c.green}✓${c.reset} Active context set to ${c.bold}team:${name}${c.reset}\n` +
        ...
        console.log(`${c.green}✓${c.reset} Active context set to ${c.bold}personality:${name}${c.reset}`);
    }
}
