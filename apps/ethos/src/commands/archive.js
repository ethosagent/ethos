...
import { listArchives, parseDuration, pruneArchives, restoreArchive, } from '@ethosagent/observability-sqlite';
import { ethosDir } from '../config';
import { getStorage } from '../wiring';
export async function runArchive(sub, argv) {
    const dir = ethosDir();
    ...
    if (sub === 'restore') {
        ...
        try {
            ...
            console.log(`Restored ${result.traces} trace(s), ${result.spans} span(s), ${result.events} event(s), ${result.snapshots} snapshot(s) from ${month}.`);
        }
        catch (e) {
