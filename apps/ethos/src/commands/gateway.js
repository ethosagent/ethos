...
import { applyPlatformShim, deriveBotKey, ethosDir, loadConfigStrict, readRawConfig, validateBotBindings, writeConfig, } from '../config';
...
function withTimeout(promise, ms) {
    return Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error('health check timeout')), ms).unref()),
    ]);
}
export async function buildGatewayHeartbeat(adapters, startedAt) {
    const results = await Promise.allSettled(adapters.map((a) => withTimeout(a.health(), HEALTH_TIMEOUT_MS)));
...
async function loadAdapterModule(modulePath, label) {
    try {
        ...
        switch (modulePath) {
            ...
            default:
                throw new EthosError({
                    ...
                    action: 'Add a literal-string switch arm in apps/ethos/src/commands/gateway.ts so the bundler can inline this module.',
                });
        }
        return mod;
    }
        ...
        console.warn(`${c.yellow}⚠ ${label} adapter unavailable${c.reset} ${c.dim}(${reason})${c.reset}`);
        return null;
    }
}
const c = {
...
export async function runGatewaySetup() {
    ...
    console.log(`${c.dim}Create a Telegram bot at https://t.me/BotFather, then paste the token below.${c.reset}\n`);
    const token = (await ask('Telegram bot token: ')).trim();
    rl.close();
    if (!token) {
        console.log(`${c.yellow}No token entered. Run ethos gateway setup again to configure.${c.reset}`);
        return;
    }
    ...
    try {
        ...
        console.log(`${c.yellow}Warning: could not reach Telegram to validate token. Saving anyway.${c.reset}`);
    }
...
export async function runGatewayStart() {
    ...
    const resolveUserId = (platform, platformUserId, displayLabel) => identityMap.resolve(platform, platformUserId, displayLabel);
    const hasEmailConfig = config.emailImapHost && config.emailUser && config.emailPassword && config.emailSmtpHost;
    const hasAnyPlatform = config.telegramToken ||
        config.discordToken ||
        ...
        hasEmailConfig;
    if (!hasAnyPlatform) {
        console.log(`${c.dim}No platform configured — gateway idling. Run: ethos gateway setup to add one.${c.reset}`);
    }
    ...
    const multiBotConfigured = (config.telegram?.bots.length ?? 0) + (config.slack?.apps.length ?? 0) > 1;
    const legacyAdapterConfigured = !!config.discordToken ||
    ...
    if (multiBotConfigured && legacyAdapterConfigured) {
        console.log(`${c.yellow}⚠${c.reset} ${c.dim}Multi-bot routing is configured alongside email/discord. ` +
    ...
    if (migration && migration.migrated > 0) {
        console.log(`${c.dim}Migrated ${migration.migrated} session key(s) to the multi-bot lane format.${c.reset}`);
    }
    ...
    console.log(`${c.dim}Runs in the foreground. For always-on production, see https://ethosagent.ai/docs/guides/run-as-daemon (launchd / systemd / pm2). For interactive use, run ${c.reset}${c.bold}ethos chat${c.reset}${c.dim}.${c.reset}`);
    ...
    const scheduler = new CronScheduler({
        ...
        deliver: async (job, output) => {
            if (cronDeliverFn)
                await cronDeliverFn(job, output);
        },
        runJob: async (job) => {
            if (!systemLoop) {
                throw new EthosError({
    ...
    for (const [teamName, result] of supervisorResults) {
        if (result.status === 'spawned' && result.pid === undefined) {
            console.log(`${c.yellow}⚠ team supervisor${c.reset} ${c.bold}${teamName}${c.reset} ${c.yellow}spawned but did not publish a runtime file — team routing may be broken. Run 'ethos team status ${teamName}' to diagnose.${c.reset}`);
        }
        else {
            console.log(result.status === 'spawned'
    ...
    const { loop: systemLoopReady, setMessagingSend: setSystemMessagingSend } = await createAgentLoop(config, { cronScheduler: scheduler });
    ...
    let gatewayRef = null;
    const telegramClarifySurfaces = await buildTelegramClarifySurfaces(bots, adapters, (sessionId) => {
    ...
    const clarifyMessageCorrelator = telegramClarifySurfaces.length > 0
        ? async (msg) => {
    ...
    if (adapters.length > 0 && !config.channelFilter) {
        console.error(`${c.red}FATAL: Channel adapters configured without channel_filter safety config.${c.reset}\n` +
    ...
    for (const adapter of adapters) {
        ...
        if (config.channelFilter && !config.channelFilter[platform]) {
            console.error(`${c.red}FATAL: Adapter "${adapter.id}" has no channel_filter.${platform} config.${c.reset}\n` +
    ...
    const gatewayMessagingSend = async (platform, target, body) => gateway.sendTo(platform, target, body);
    ...
    if (adapters.length === 0) {
        console.log(`${c.dim}No adapters started — gateway idling. Configure a platform to activate.${c.reset}`);
    }
    ...
    const shutdown = async () => {
        ...
        await gateway.shutdown({
            notify: '⚠ Ethos was interrupted while answering. Please resend your last message — your session history is preserved.',
        });
...
async function buildGatewayBots(config, scheduler) {
    ...
    const buildOne = async (bot) => {
        ...
        if (bot.bind.type === 'team') {
            ...
            // system-loop's jobs.
            const result = await createAgentLoop({ ...config, personality: bot.bind.name }, { cronScheduler: scheduler });
            loop = result.loop;
    ...
    for (const bot of config.telegram?.bots ?? [])
        out.push(await buildOne(bot));
    for (const app of config.slack?.apps ?? [])
        out.push(await buildOne(app));
...
function wireApprovalFlow(gateway, bots, adapters) {
    ...
    for (const bot of bots) {
        if (!approvalBotKeys.has(bot.botKey))
            continue;
        bot.loop.hooks.registerModifying('before_tool_call', createSlackApprovalHook({ coordinator, isDangerous, resolveApprovalTarget }));
    }
...
async function validateBindings(config) {
    ...
    for (const entry of await storage.listEntries(join(ethosDir(), 'teams'))) {
        if (entry.name.endsWith('.yaml'))
            teamNames.add(entry.name.replace(/\.yaml$/, ''));
    }
...
export async function buildAdapters(config, loadAdapter, attachmentCache) {
    ...
    if ((config.telegram?.bots.length ?? 0) > 0) {
        ...
        if (mod) {
            ...
            for (const botCfg of config.telegram?.bots ?? []) {
                ...
                adapters.push(new mod.TelegramAdapter({
                    token: botCfg.token,
                    cache: telegramCache,
    ...
    if ((config.slack?.apps.length ?? 0) > 0) {
        ...
        if (mod) {
            ...
            for (const appCfg of config.slack?.apps ?? []) {
                ...
                const memory = appCfg.bind.type === 'personality'
                    ? createSlackMemoryReader(appCfg.bind.name)
                    : undefined;
                adapters.push(new mod.SlackAdapter({
    ...
    if (config.emailImapHost && config.emailUser && config.emailPassword && config.emailSmtpHost) {
        ...
        if (mod) {
            adapters.push(new mod.EmailAdapter({
...
async function buildTelegramClarifySurfaces(bots, adapters, getSessionRouting) {
    ...
    if (telegramAdapters.length === 0)
        return [];
    const mod = await loadAdapterModule('@ethosagent/platform-telegram/clarify-surface', 'Telegram clarify surface');
    if (!mod)
        return [];
    const surfaces = [];
    for (const adapter of telegramAdapters) {
        ...
        surfaces.push(new mod.TelegramClarifySurface({
            adapter: tgAdapter,
            bridge,
            store: bridge.store,
            getSessionRouting,
...
async function buildSlackClarifySurfaces(bots, adapters, getSessionRouting) {
    ...
    if (slackAdapters.length === 0)
        return [];
    const mod = await loadAdapterModule('@ethosagent/platform-slack/clarify-surface', 'Slack clarify surface');
    if (!mod)
        return [];
    const surfaces = [];
...
async function buildDiscordClarifySurfaces(bots, adapters, systemLoop, getSessionRouting) {
    ...
    if (discordAdapters.length === 0)
        return [];
    const mod = await loadAdapterModule('@ethosagent/platform-discord/clarify-surface', 'Discord clarify surface');
    if (!mod)
        return [];
    const surfaces = [];
    for (const adapter of discordAdapters) {
        ...
        surfaces.push(new mod.DiscordClarifySurface({
