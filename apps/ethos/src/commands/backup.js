...
import { createReadStream, createWriteStream, existsSync, lstatSync, mkdirSync, readdirSync, readFileSync, statSync, writeFileSync, } from 'node:fs';
...
export async function runBackup(argv) {
    ...
    const outPath = filtered[0] ?? `ethos-backup-${timestamp()}-${randomBytes(4).toString('hex')}.tar.gz`;
...
export async function runImport(argv) {
    ...
    const filtered = argv.filter((a, i) => a !== '--json' && a !== '--secrets' && !(i > 0 && argv[i - 1] === '--secrets'));
...
function walkDir(dir, relBase, entries, personalityId, strippedTokens) {
...
function collectEntries(dataDir) {
    const entries = [];
    const strippedTokens = new Map();
    for (const file of BACKUP_FILES) {
        const p = join(dataDir, file);
        if (existsSync(p))
            entries.push({ relPath: file, content: readFileSync(p) });
    }
...
function buildSecretsManifest(dataDir, strippedTokens) {
    const lines = [
...
export function buildTar(entries) {
    ...
    for (const entry of entries) {
        ...
        Buffer.from(`${Math.floor(Date.now() / 1000)
            .toString(8)
            .padStart(11, '0')}\0`).copy(header, 136); // mtime
...
function parseManifestHints(raw) {
    const hints = [];
    let section = 'none';
    ...
    for (const line of raw.split('\n')) {
        ...
        if (!trimmed || trimmed.startsWith('#') || trimmed.startsWith('backed_up_at:'))
            continue;
...
function parseSecretsManifest(raw) {
...
async function injectSecrets(secretsPath) {
    let raw;
    if (secretsPath === '-') {
        const chunks = [];
...
// ---------------------------------------------------------------------------
const USAGE_PERSONALITY_IMPORT = 'Usage: ethos personality import <file-or-dir> [--force] [--secrets <manifest>]';
export async function runPersonalityImport(argv) {
    ...
    const positional = argv.filter((a, i) => a !== '--force' && a !== '--secrets' && !(i > 0 && argv[i - 1] === '--secrets'));
    ...
    for (const [relPath, content] of entries) {
        ...
        if (relPath === 'secrets.manifest.yaml')
            continue;
        const dest = join(dataDir, relPath);
        const resolvedDest = resolve(dest);
    ...
    if (skippedCount > 0) {
        console.warn(`  ⚠ ${skippedCount} archive entry/entries outside personalities/${personalityId}/ were skipped.`);
    }
    if (secretsPath) {
        const count = await injectSecrets(secretsPath);
        console.log(`✓ Injected ${count} secret(s)`);
