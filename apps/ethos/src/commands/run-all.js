...
export async function runAll(opts = {}) {
    ...
    if (!entryPoint) {
        (opts.logger ?? console).error('run-all: cannot determine ethos entry point — process.argv[1] is empty.');
        process.exit(1);
    }
...
function startChild(sc, entryPoint, logsDir, spawn, log, onChildReady, rotation) {
    ...
    sc.rotationTimer = setInterval(() => {
        ...
        try {
            ...
            if (stat.size < rotation.maxBytes)
                return;
        }
        catch {
            return;
    ...
    sc.process = child;
    log.log(`${c.green}✓${c.reset} ${sc.spec.name.padEnd(8)} ${c.dim}pid ${child.pid} · logs: ${logPath}${c.reset}`);
    ...
    child.on('exit', (code, signal) => {
        ...
        if (sc.restarts.length > MAX_RESTARTS_IN_WINDOW) {
            log.error(`${c.red}run-all: ${sc.spec.name} crashed ${sc.restarts.length} times in ${RESTART_WINDOW_MS / 1000}s — giving up. Inspect ${logPath}.${c.reset}`);
            process.exitCode = 1;
        ...
        setTimeout(() => startChild(sc, entryPoint, logsDir, spawn, log, onChildReady, rotation), delay).unref();
