...
// list
// ---------------------------------------------------------------------------
async function runMeshList(args = []) {
    ...
    if (names.length === 0) {
        ...
        console.log(`${c.dim}No meshes found. Start a team or run 'ethos serve' to create one.${c.reset}`);
        return;
    }
    if (jsonMode) {
        const result = [];
...
// status
// ---------------------------------------------------------------------------
async function runMeshStatus(name, args = []) {
    const jsonMode = args.includes('--json');
    const effectiveName = name === '--json' ? undefined : name;
    if (!effectiveName) {
        if (jsonMode)
            writeJsonError('INPUT_REQUIRED', 'Usage: ethos mesh status <name>');
    ...
    for (const m of members) {
        ...
        console.log(`  ${m.agentId.padEnd(40)}${String(m.port).padEnd(8)}${caps.slice(0, 28).padEnd(30)}${m.activeSessions}`);
    }
    console.log();
}
// ---------------------------------------------------------------------------
// create
// ---------------------------------------------------------------------------
async function runMeshCreate(name) {
    if (!name) {
        console.error('Usage: ethos mesh create <name>');
        process.exit(1);
    }
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
        console.error(`${c.red}Invalid mesh name: ${name}. Use only letters, numbers, hyphens, and underscores.${c.reset}`);
        process.exit(1);
    }
...
// destroy
// ---------------------------------------------------------------------------
async function runMeshDestroy(name) {
    if (!name) {
    ...
    const members = await mesh.list();
    if (members.length > 0) {
        console.error(`${c.red}Cannot destroy mesh "${name}" — ${members.length} live member(s) still registered.${c.reset}`);
...
// Public entry point
// ---------------------------------------------------------------------------
export async function runMeshCommand(sub, args) {
    switch (sub) {
        case 'list':
        case '':
        ...
        case 'destroy':
            ...
            break;
        default:
            console.log(`${c.cyan}Usage:${c.reset} ethos mesh [list | status <name> | create <name> | destroy <name>]`);
    }
}
