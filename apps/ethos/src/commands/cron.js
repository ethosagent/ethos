...
function makeScheduler(config) {
    ...
    const scheduler = new CronScheduler({
        ...
        runJob: async (job) => {
            ...
            if (!loop)
                loop = (await createAgentLoop(config)).loop;
...
export async function runCronCommand(sub, args, config) {
    switch (sub) {
        case 'list': {
        ...
        case 'create': {
            ...
            if (!name || !schedule || !prompt) {
                console.log('Usage: ethos cron create --name "Job name" --schedule "0 8 * * *" --prompt "Your prompt"');
                return;
            }
            if (!isValidSchedule(schedule)) {
                console.log(`${c.red}Invalid schedule: "${schedule}"${c.reset}`);
                console.log(`${c.dim}Examples: "0 8 * * 1-5" (cron), "30m" (delay), "every 2h" (interval)${c.reset}`);
                return;
            }
            if (personality) {
            ...
            try {
                ...
                if (next)
                    console.log(`${c.dim}Next run: ${next.toLocaleString()}${c.reset}`);
            }
            finally {
                cleanup();
            }
        ...
        case 'update': {
            const id = args[0];
            if (!id) {
                console.log('Usage: ethos cron update <id> [--name "..."] [--schedule "..."] [--prompt "..."]');
                return;
            }
            ...
            try {
                ...
                if (updated.nextRunAt) {
                    console.log(`${c.dim}Next run: ${new Date(updated.nextRunAt).toLocaleString()}${c.reset}`);
                }
            }
            catch (err) {
        ...
        default:
            console.log('Usage: ethos cron [list [--personality <id>] | show <id> | create | update <id> | pause | resume | delete | run]');
    }
}
