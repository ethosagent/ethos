...
export async function runClaw(args) {
    ...
    for (const item of result.items) {
        if (item.status === 'copied') {
            ...
            console.log(`  ${c.yellow}⚠ skipped${c.reset}  ${item.label}${c.dim}  (${item.reason ?? 'unknown'})${c.reset}`);
        }
        else {
            console.log(`  ${c.red}✗ failed${c.reset}   ${item.label}${c.dim}  (${item.reason ?? 'unknown'})${c.reset}`);
        }
    }
    console.log();
    if (flags.dryRun) {
        console.log(`${c.dim}Dry run — no files written. Re-run without --dry-run to migrate.${c.reset}`);
        return;
    }
    if (result.failed > 0) {
        console.log(`${c.yellow}Migration finished with ${result.failed} failure${result.failed === 1 ? '' : 's'}.${c.reset}`);
    }
    else {
        console.log(`${c.green}Migration complete.${c.reset} Run \`ethos setup\` to verify your config.`);
    }
}
// ---------------------------------------------------------------------------
// Helpers
...
function printPlanSummary(plan, flags) {
    ...
    if (plan.detected.agents)
        console.log(`${c.dim}  workspace: ${c.reset}${plan.workspace}`);
    console.log(`${c.dim}  preset: ${c.reset}${flags.preset}${flags.overwrite ? ` ${c.yellow}(--overwrite)${c.reset}` : ''}`);
    console.log();
    console.log(`${c.dim}  ${plan.summary.memories} memory file${plural(plan.summary.memories)}, ${plan.summary.skills} skill${plural(plan.summary.skills)}, ${plan.summary.platformTokens} platform token${plural(plan.summary.platformTokens)}, ${plan.summary.apiKeys} API key${plural(plan.summary.apiKeys)}${c.reset}`);
    if (plan.personality.becomesMigrated) {
        console.log(`${c.dim}  personality: ${c.reset}${c.cyan}migrated${c.reset}${c.dim} (from SOUL.md${plan.personality.requested ? `, was \`${plan.personality.requested}\`` : ''})${c.reset}`);
    }
