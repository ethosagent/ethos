...
function parseFlags(args: string[]): Flags {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--lines') {
      ...
      if (Number.isFinite(n) && n > 0) lines = Math.floor(n);
      i++;
      continue;
    }
    if (a === '--interval-ms') {
      const n = Number(args[i + 1]);
      if (Number.isFinite(n) && n >= 200) intervalMs = Math.floor(n);
      i++;
    }
  }
...
async function listTailTargets(): Promise<string[]> {
  ...
  if (!(await storage.exists(teamRoot))) return targets;

  const teams = await storage.listEntries(teamRoot);
  for (const team of teams) {
    if (!team.isDir) continue;
...
function printPrefixed(path: string, block: string): void {
...
function summarizeErrors(raw: string | null): ErrorSummary {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (!trimmed) continue;
    try {
      const row = JSON.parse(trimmed) as { code?: string };
...
function summarizeSupervisor(raw: string | null): SupervisorSummary {
  const byEvent = new Map<string, number>();
  if (!raw) return { total: 0, byEvent };
  let total = 0;
  for (const line of raw.split('\n')) {
    const trimmed = line.trim();
    if (!trimmed) continue;
...
function printMapRows(title: string, rows: Map<string, number>): void {
...
function statusMark(exists: boolean): string {
...
async function runList(json: boolean): Promise<void> {
  ...
  console.log('');
  console.log(
    `  ${c.dim}Note: daemon manager logs may also live outside ~/.ethos/logs (pm2/journald).${c.reset}`,
  );
  console.log('');
}
...
async function runBundle(flags: Flags): Promise<void> {
  const storage = getStorage();
  const root = logsRoot();
  const out =
    flags.out ??
...
async function runTail(flags: Flags): Promise<void> {
  const storage = getStorage();
  const sizes = new Map<string, number>();
  let polling = false;

  async function refreshTargets(initial: boolean): Promise<void> {
    ...
    try {
      ...
      for (const [path, previousSize] of sizes) {
        ...
        if (currentSize < previousSize) {
          ...
          if (tail) printPrefixed(path, tail);
...
export async function runLogs(args: string[]): Promise<void> {
  ...
  console.log(
    'Usage: ethos logs [list | summary | note | bundle [--out <path>] [--lines <n>] | tail [--lines <n>] [--interval-ms <n>]]',
  );
}
