...
import { parseTeamManifest, readRuntime, removeRuntime, teamsDir, validateForStart, } from '@ethosagent/team-supervisor';
...
// ---------------------------------------------------------------------------
function serializeTeamManifest(manifest) {
    const lines = [
        `name: ${manifest.name}`,
    ...
    if (manifest.domain_capabilities.length === 0) {
        ...
        lines.push('domain_capabilities:');
        for (const cap of manifest.domain_capabilities)
            lines.push(`  - ${cap}`);
    }
    if (manifest.dispatch_mode)
        lines.push(`dispatch_mode: ${manifest.dispatch_mode}`);
    if (manifest.coordinator)
        lines.push(`coordinator: ${manifest.coordinator}`);
    if (manifest.coordinator_model)
        lines.push(`coordinator_model: ${manifest.coordinator_model}`);
    ...
    if (manifest.members.length === 0) {
        ...
        for (const m of manifest.members) {
            ...
            if (m.auto_restart !== undefined)
                lines.push(`    auto_restart: ${m.auto_restart}`);
            if (m.port !== undefined)
...
async function runTeamList(args = []) {
    ...
    if (jsonMode) {
        const result = files.map((file) => {
            ...
            if (health === 'stale')
                removeRuntime(teamName);
            const liveRt = health === 'running' ? rt : null;
            const liveMembers = liveRt?.members.filter((m) => m.status !== 'stopped' && m.status !== 'failed');
            return {
                name: teamName,
    ...
    for (const file of files) {
        ...
        console.log(`  ${c.bold}${teamName.padEnd(20)}${c.reset} ${status}  ${c.dim}${members}${c.reset}`);
    }
    console.log();
}
// ---------------------------------------------------------------------------
// start
// ---------------------------------------------------------------------------
async function runTeamStart(name) {
    if (!name) {
    ...
    if (existingHealth === 'running') {
        console.error(`Team ${name} already running (PID ${existingRuntime?.supervisorPid}). Use 'ethos team status ${name}' for details.`);
        process.exit(1);
    }
...
// stop
// ---------------------------------------------------------------------------
async function runTeamStop(name) {
    if (!name) {
    ...
    try {
        ...
        console.log(`Sent SIGTERM to supervisor (PID ${supervisorPid}). Team "${name}" is shutting down.`);
        console.log(`${c.dim}Use 'ethos team status ${name}' to confirm all members have stopped.${c.reset}`);
    }
    catch (err) {
        const code = err.code;
        if (code === 'ESRCH') {
...
function assertSafeTeamName(name) {
    if (name === '.' || name === '..' || !TEAM_NAME_PATTERN.test(name)) {
        console.error(`Invalid team name "${name}". Allowed: letters, digits, dashes, dots, underscores; no path separators; cannot be "." or "..".`);
        process.exit(1);
    }
}
async function runTeamDestroy(name, args) {
    if (!name) {
    ...
    if (runtimeHealth(rt) === 'running') {
        console.error(`Team "${name}" is still running. Stop it first: ${c.cyan}ethos team stop ${name}${c.reset}`);
        process.exit(1);
    }
    ...
    if (!yes) {
        ...
        if (teamDirExists)
            console.log(`  - ${teamDir} (logs, runtime, kanban board.db)`);
        console.log('');
...
async function runTeamStatus(name, args = []) {
    const jsonMode = args.includes('--json');
    ...
    console.log(`  ${'Personality'.padEnd(22)}${'Port'.padEnd(8)}${'Status'.padEnd(14)}${'PID'.padEnd(10)}Failures`);
    console.log(`  ${'-'.repeat(60)}`);
    for (const m of rt.members) {
        const sc = statusColor(m.status);
        console.log(`  ${m.personality.padEnd(22)}${String(m.port).padEnd(8)}${sc}${m.status.padEnd(14)}${c.reset}${String(m.pid ?? '—').padEnd(10)}${m.failureCount}`);
    }
    ...
    if (existsSync(boardPath)) {
        try {
            ...
            board.close();
            if (stats.size > 0) {
                console.log(`  ${'Member'.padEnd(22)}${'Completed'.padEnd(12)}${'Failed'.padEnd(10)}${'Tier'}`);
                console.log(`  ${'-'.repeat(52)}`);
                for (const [id, s] of stats) {
                    const tier = autonomyTier(s, manifest.trust_policy);
                    console.log(`  ${id.padEnd(22)}${String(s.ticketsCompleted).padEnd(12)}${String(s.ticketsFailed).padEnd(10)}${tier}`);
                }
            }
        }
        catch (err) {
            console.error(`  ${c.yellow}Warning: could not read board stats: ${err instanceof Error ? err.message : String(err)}${c.reset}`);
        }
    }
    console.log();
}
...
// create
// ---------------------------------------------------------------------------
async function runTeamCreate(name, extraArgs = []) {
...
async function runTeamCreateBlank(name) {
    if (!name) {
...
async function runTeamCreateAiAssisted(name) {
    ...
    console.log(`${c.dim}I'll help you design a team of specialist personalities. Let's start.${c.reset}\n`);
...
// <name> add / remove
// ---------------------------------------------------------------------------
async function runTeamMemberAdd(teamName, personality) {
    if (!personality) {
...
async function runTeamMemberRemove(teamName, personality) {
    if (!personality) {
    ...
    writeFileSync(manifestPath, serializeTeamManifest(updatedManifest), 'utf-8');
    console.log(`${c.green}✓${c.reset} Removed ${c.bold}${personality}${c.reset} from team ${teamName}`);
    if (updatedManifest.members.length === 0) {
        console.log(`${c.dim}  Team has no members — add one before starting: ethos team ${teamName} add <personality>${c.reset}`);
    }
}
// ---------------------------------------------------------------------------
// logs
// ---------------------------------------------------------------------------
async function runTeamLogs(name, args) {
    if (!name) {
    ...
    const rt = readRuntime(name);
    if (!rt) {
        console.error(`No log history found for team "${name}" — start it first with 'ethos team start ${name}'.`);
        process.exit(1);
    }
...
// Public entry point
// ---------------------------------------------------------------------------
export async function runTeamCommand(sub, args) {
    switch (sub) {
        case 'list':
        case '':
        ...
        default: {
            ...
            if (action === 'add') {
                ...
                console.log('Usage: ethos team [list | create <name> | start <name> | stop <name> | destroy <name> [--yes] | status <name> | logs <name> | <name> add <personality> | <name> remove <personality>]');
            }
        }
    }
}
...
// retro
// ---------------------------------------------------------------------------
async function runTeamRetro(name) {
    if (!name) {
...
export function buildSupervisorLaunchArgs(entryPoint, teamName, manifestPath) {
