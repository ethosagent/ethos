import { listProcesses, readProcessLogs, STOP_SUPPORTED_SIGNALS, stopProcess, } from '@ethosagent/tools-process';
...
// list
// ---------------------------------------------------------------------------
async function runProcessList(dataDir, args) {
    ...
    if (items.length === 0) {
        console.log(`${c.dim}No tracked processes. Start one with the 'process_start' tool inside 'ethos chat'.${c.reset}`);
        return;
    }
    console.log(`\n${c.bold}Processes:${c.reset}\n`);
    console.log(`  ${'ID'.padEnd(38)}${'Name'.padEnd(24)}${'PID'.padEnd(10)}${'Status'.padEnd(10)}Duration`);
    ...
    for (const p of items) {
        ...
        console.log(`  ${p.id.padEnd(38)}${p.name.slice(0, 22).padEnd(24)}${String(p.pid).padEnd(10)}${statusCell}${formatDuration(p.duration_ms)}`);
    }
    console.log();
}
// ---------------------------------------------------------------------------
...
async function runProcessLogs(dataDir, args) {
...
function parseStopFlags(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--signal') {
            const raw = args[i + 1];
            if (raw === undefined || !STOP_SUPPORTED_SIGNALS.includes(raw)) {
...
async function runProcessStop(dataDir, args) {
...
// Public entry point
// ---------------------------------------------------------------------------
export async function runProcessCommand(sub, args, dataDir = ethosDir()) {
    switch (sub) {
        case 'list':
        case '':
        ...
        case 'stop':
            ...
            break;
        default:
            console.log(`${c.cyan}Usage:${c.reset} ethos process [list [--json] | logs <id> [--lines N] [--stream stdout|stderr|both] | stop <id> [--signal SIGTERM|SIGKILL]]`);
    }
}
