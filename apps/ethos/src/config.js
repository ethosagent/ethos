...
async function resolveSecretValue(value, secrets) {
    ...
    for (const m of matches) {
        ...
        if (secret === null) {
            ...
            throw new Error(`Secret not found: ${ref}. Run 'ethos secrets set ${ref} <value>' to store it.`);
        }
...
export async function writeConfig(storage, config) {
    ...
    if (config.apiVersion)
        lines.push(`apiVersion: ${config.apiVersion}`);
    ...
    if (config.telegramToken)
        lines.push(`telegramToken: ${config.telegramToken}`);
    if (config.discordToken)
        lines.push(`discordToken: ${config.discordToken}`);
    if (config.slackBotToken)
        lines.push(`slackBotToken: ${config.slackBotToken}`);
    if (config.slackAppToken)
        lines.push(`slackAppToken: ${config.slackAppToken}`);
    if (config.slackSigningSecret)
        lines.push(`slackSigningSecret: ${config.slackSigningSecret}`);
    if (config.emailImapHost)
        lines.push(`emailImapHost: ${config.emailImapHost}`);
    if (config.emailImapPort)
        lines.push(`emailImapPort: ${config.emailImapPort}`);
    if (config.emailUser)
        lines.push(`emailUser: ${config.emailUser}`);
    if (config.emailPassword)
        lines.push(`emailPassword: ${config.emailPassword}`);
    if (config.emailSmtpHost)
        lines.push(`emailSmtpHost: ${config.emailSmtpHost}`);
    if (config.emailSmtpPort)
        lines.push(`emailSmtpPort: ${config.emailSmtpPort}`);
    if (config.verbose)
        lines.push('verbose: true');
    if (config.displayVerbosity)
        lines.push(`display.verbosity: ${config.displayVerbosity}`);
    ...
    if (config.displayResumeHint === false)
        lines.push('display.resume_hint: false');
    if (config.displayResumeRecapTurns !== undefined)
        lines.push(`display.resume_recap_turns: ${config.displayResumeRecapTurns}`);
    if (config.displayBellOnComplete)
        lines.push('display.bell_on_complete: true');
    if (config.skin)
        lines.push(`skin: ${config.skin}`);
    if (config.retention) {
    ...
    if (config.evolverCronEnabled)
        lines.push('evolver.cron_enabled: true');
    if (config.evolverSchedule)
        lines.push(`evolver.schedule: ${config.evolverSchedule}`);
    ...
    if (config.telegram?.bots.length) {
        for (const [i, bot] of config.telegram.bots.entries()) {
            if (bot.id)
                lines.push(`telegram.bots.${i}.id: ${bot.id}`);
    ...
    if (config.slack?.apps.length) {
        for (const [i, app] of config.slack.apps.entries()) {
            if (app.id)
                lines.push(`slack.apps.${i}.id: ${app.id}`);
    ...
    if (config.teams) {
        for (const [name, tcfg] of Object.entries(config.teams)) {
            if (tcfg.autoStop)
                lines.push(`teams.${name}.autoStop: true`);
        }
    }
    ...
    if (config.channelFilter) {
        for (const [platform, cfg] of Object.entries(config.channelFilter)) {
            if (cfg.enabled === false)
                lines.push(`channel_filter.${platform}.enable: false`);
            if (cfg.ownerUserId)
                lines.push(`channel_filter.${platform}.ownerUserId: ${cfg.ownerUserId}`);
            if (cfg.recipientAllowlist && cfg.recipientAllowlist.length > 0) {
                lines.push(`channel_filter.${platform}.recipientAllowlist: ${cfg.recipientAllowlist.join(',')}`);
            }
            if (cfg.dmPolicy)
                lines.push(`channel_filter.${platform}.dmPolicy: ${cfg.dmPolicy}`);
    ...
    if (config.providers && config.providers.length > 0) {
        for (const [i, p] of config.providers.entries()) {
            ...
            if (p.model)
                lines.push(`providers.${i}.model: ${p.model}`);
            if (p.baseUrl)
                lines.push(`providers.${i}.baseUrl: ${p.baseUrl}`);
            if (p.apiVersion)
                lines.push(`providers.${i}.apiVersion: ${p.apiVersion}`);
        }
    }
    if (config.auxiliary?.compression) {
        ...
        if (c.provider)
            lines.push(`auxiliary.compression.provider: ${c.provider}`);
        if (c.apiKey)
            lines.push(`auxiliary.compression.apiKey: ${c.apiKey}`);
        if (c.baseUrl)
            lines.push(`auxiliary.compression.baseUrl: ${c.baseUrl}`);
    }
    if (config.auxiliary?.vision) {
        const v = config.auxiliary.vision;
        lines.push(`auxiliary.vision.model: ${v.model}`);
        if (v.provider)
            lines.push(`auxiliary.vision.provider: ${v.provider}`);
        if (v.apiKey)
            lines.push(`auxiliary.vision.apiKey: ${v.apiKey}`);
        if (v.baseUrl)
            lines.push(`auxiliary.vision.baseUrl: ${v.baseUrl}`);
    }
    if (config.modelCatalog) {
        if (config.modelCatalog.enabled === false)
            lines.push('modelCatalog.enabled: false');
        if (config.modelCatalog.url)
            lines.push(`modelCatalog.url: ${config.modelCatalog.url}`);
    ...
    if (config.logs?.rotation) {
        ...
        if (r.maxBytes !== undefined)
            lines.push(`logs.rotation.maxBytes: ${r.maxBytes}`);
        if (r.maxFiles !== undefined)
            lines.push(`logs.rotation.maxFiles: ${r.maxFiles}`);
        if (r.enabled === false)
            lines.push('logs.rotation.enabled: false');
    }
    if (config.aws?.secrets) {
        const s = config.aws.secrets;
        if (s.enabled !== undefined)
            lines.push(`aws.secrets.enabled: ${s.enabled}`);
        if (s.region)
            lines.push(`aws.secrets.region: ${s.region}`);
        if (s.prefix)
            lines.push(`aws.secrets.prefix: ${s.prefix}`);
        if (s.endpoint)
            lines.push(`aws.secrets.endpoint: ${s.endpoint}`);
    }
    if (config.webBaseUrl)
        lines.push(`webBaseUrl: ${config.webBaseUrl}`);
...
export async function resolveConfigSecrets(config, secrets) {
    ...
    if (r.baseUrl)
        r.baseUrl = await resolveSecretValue(r.baseUrl, secrets);
    if (r.telegramToken)
        r.telegramToken = await resolveSecretValue(r.telegramToken, secrets);
    if (r.discordToken)
        r.discordToken = await resolveSecretValue(r.discordToken, secrets);
    if (r.slackBotToken)
        r.slackBotToken = await resolveSecretValue(r.slackBotToken, secrets);
    if (r.slackAppToken)
        r.slackAppToken = await resolveSecretValue(r.slackAppToken, secrets);
    if (r.slackSigningSecret)
        r.slackSigningSecret = await resolveSecretValue(r.slackSigningSecret, secrets);
    if (r.emailPassword)
        r.emailPassword = await resolveSecretValue(r.emailPassword, secrets);
    if (r.providers) {
        r.providers = await Promise.all(r.providers.map(async (p) => ({
            ...p,
    ...
    if (r.telegram?.bots) {
        r.telegram = {
            ...r.telegram,
            bots: await Promise.all(r.telegram.bots.map(async (bot) => ({
                ...bot,
    ...
    if (r.slack?.apps) {
        r.slack = {
            ...r.slack,
            apps: await Promise.all(r.slack.apps.map(async (app) => ({
                ...app,
...
function parseConfigYaml(src) {
    ...
    for (const line of src.split('\n')) {
        ...
        if (prov) {
            ...
            if (field)
                providersKv[idx][field] = prov[3].trim().replace(/^["']|["']$/g, '');
            continue;
        }
        ...
        if (cf) {
            ...
            channelFilterKv[platform][cf[2]] = cf[3]
                .trim()
                .replace(/^["']|["']$/g, '');
            continue;
        }
    ...
    const activeContext = (activeContextType === 'personality' || activeContextType === 'team') && activeContextName
    ...
    const auxiliaryVision = auxiliaryVisionKv.model
        ...
        : undefined;
    const modelCatalogProviders = Object.keys(modelCatalogProvidersKv).length > 0
        ? Object.fromEntries(Object.entries(modelCatalogProvidersKv)
            .filter(([, v]) => v.url)
            .map(([id, v]) => [id, { url: v.url }]))
        : undefined;
    const modelCatalogEnabled = modelCatalogKv.enabled === 'true'
        ? true
        : modelCatalogKv.enabled === 'false'
            ? false
            : undefined;
    const modelCatalog = Object.keys(modelCatalogKv).length > 0 || modelCatalogProviders
        ? {
    ...
    const logsRotation = Object.keys(logsRotationKv).length > 0
        ? {
        ...
        : undefined;
    const awsSecrets = Object.keys(awsSecretsKv).length > 0
        ? {
    ...
    const config = {
        ...
        displayResumeRecapTurns: (() => {
            if (displayKv.resume_recap_turns === undefined)
                return undefined;
        ...
        quick_commands,
        channelFilter,
        auxiliary: auxiliaryCompression || auxiliaryVision
            ? {
...
export async function loadConfigStrict(storage, secrets) {
    ...
    if (!parsed)
        return null;
    if (secrets)
        validateNoPlaintextSecrets(parsed);
...
export function validateNoPlaintextSecrets(config) {
    ...
    if (violations.length > 0) {
        const details = violations
            .map((v) => `  - field '${v.field}' appears to contain a plaintext ${v.label}. ` +
...
function buildRetentionConfig(kv) {
    ...
    const cfg = {};
    if (kv.messages)
        cfg.messages = kv.messages;
    if (kv.traces)
        cfg.traces = kv.traces;
    if (kv.spans)
        cfg.spans = kv.spans;
    if (kv.blobs)
        cfg.blobs = kv.blobs;
    if (kv.archive)
        cfg.archive = kv.archive;
    const ev = {};
    if (kv['events.error'])
        ev.error = kv['events.error'];
    if (kv['events.audit'])
        ev.audit = kv['events.audit'];
    if (kv['events.channel'])
        ev.channel = kv['events.channel'];
    if (kv['events.install'])
        ev.install = kv['events.install'];
...
function buildBotBinding(kv, label) {
    ...
    if (type !== 'personality' && type !== 'team') {
        errors.push(`${label}: missing or invalid 'bind.type' ` +
...
export function applyPlatformShim(config) {
    ...
    if (config.telegramToken && (config.telegram?.bots?.length ?? 0) === 0) {
        ...
        deprecations.push("Config field 'telegramToken' is deprecated. Use the list form: " +
    ...
    if (config.slackBotToken &&
        ...
        deprecations.push("Config fields 'slackBotToken'/'slackAppToken'/'slackSigningSecret' are deprecated. " +
...
function rejectUnsafeIdent(label, value, errors) {
    if (!SAFE_IDENT.test(value)) {
        errors.push(`${label}: '${value}' must match /^[A-Za-z0-9_-]+$/ — dots, whitespace, '#', and quotes are reserved by the config format.`);
    }
}
/**
...
export function validateBotBindings(config, deps) {
    ...
    const checkBind = (label, botId, bind, botKey) => {
        if (botId !== undefined)
            rejectUnsafeIdent(`${label}.id`, botId, errors);
        ...
        if (bind.type === 'personality' && !deps.personalityIds.has(bind.name)) {
            errors.push(`${label}: bind.name='${bind.name}' is not a known personality. ` +
                'Add the personality under ~/.ethos/personalities/, or fix the binding.');
        }
        if (bind.type === 'team' && !deps.teamNames.has(bind.name)) {
            errors.push(`${label}: bind.name='${bind.name}' is not a known team. ` +
...
function buildChannelFilter(kv) {
    ...
    for (const platform of platforms) {
        ...
        if (entry.enable === 'false')
            cfg.enabled = false;
        else if (entry.enable === 'true')
            cfg.enabled = true;
        if (entry.ownerUserId)
            cfg.ownerUserId = entry.ownerUserId;
        ...
        if (entry.contextVisibility === 'all' ||
...
function retentionToLines(cfg) {
    const lines = [];
    if (cfg.messages)
        lines.push(['messages', cfg.messages]);
    if (cfg.traces)
        lines.push(['traces', cfg.traces]);
    if (cfg.spans)
        lines.push(['spans', cfg.spans]);
    if (cfg.blobs)
        lines.push(['blobs', cfg.blobs]);
    if (cfg.archive)
        lines.push(['archive', cfg.archive]);
    if (cfg.events) {
        if (cfg.events.error)
            lines.push(['events.error', cfg.events.error]);
        if (cfg.events.audit)
            lines.push(['events.audit', cfg.events.audit]);
        if (cfg.events.channel)
            lines.push(['events.channel', cfg.events.channel]);
        if (cfg.events.install)
            lines.push(['events.install', cfg.events.install]);
    }
    return lines;
}
