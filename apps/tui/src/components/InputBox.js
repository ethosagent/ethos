...
function renderWithCursor(value, cursor) {
    ...
    return (_jsxs(Text, { children: [value.slice(0, cursor), _jsx(Text, { inverse: true, children: value[cursor] }), value.slice(cursor + 1)] }));
}
export function InputBox({ value, disabled, isActive = true, onChange, onSubmit, onTabComplete, onArrowUp, onArrowDown, onEscape, }) {
    ...
    useInput((input, key) => {
        ...
        if (input) {
            replace(value.slice(0, safeCursor) + input + value.slice(safeCursor), safeCursor + input.length);
        }
    ...
    return (_jsxs(Box, { borderStyle: "single", borderColor: tokens.surface.borderSubtle, paddingX: 1, flexDirection: "column", children: [_jsxs(Box, { children: [_jsx(Text, { color: tokens.semantic.info, bold: true, children: "You" }), _jsxs(Text, { dimColor: true, children: [" ", tokens.glyphs.prompt, " "] }), value === '' ? (_jsxs(Text, { children: [_jsx(Text, { inverse: true, children: " " }), _jsx(Text, { dimColor: true, children: disabled ? 'waiting…' : 'Type a message or /help…' })] })) : showCollapsed ? (_jsxs(Text, { dimColor: true, children: ["[pasted ", lineCount, " lines / ", value.length, " chars \u2014 Enter to send]"] })) : (renderWithCursor(value, safeCursor))] }), !showCollapsed && lineCount > 1 && (_jsx(Box, { children: _jsxs(Text, { dimColor: true, children: [lineCount, " lines \u00B7 Shift+Enter for newline \u00B7 Enter to send"] }) }))] }));
}
