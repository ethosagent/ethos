...
function InlineDiff({ diff, maxLines = 8 }) {
  ...
  return _jsxs(Box, {
    ...
    children: [
      ...
      truncated &&
        _jsxs(Text, { dimColor: true, children: ['(', lines.length - maxLines, ' more lines)'] }),
    ],
  });
}
    ...
    children: [
      ...
      pendingPatches > 0 &&
        _jsxs(Text, {
          ...
          dimColor: true,
          children: [pendingPatches, ' pending \u00B7 a=approve d=deny'],
        }),
      visible.length === 0
        ? _jsx(Text, { dimColor: true, children: 'no file activity yet' })
        : visible.map((entry) =>
            _jsxs(
              Box,
              {
                ...
                children: [
                  _jsxs(Box, {
                    children: [
                      _jsx(Text, { dimColor: true, children: entry.at }),
                      _jsx(Text, { children: ' ' }),
                      _jsxs(Text, {
                        color: colorFor(entry.status),
                        inverse: entry.id === selectedId,
                        children: [
                          actionGlyph(entry.action),
                          ' ',
                          entry.action,
                          ' ',
                          entry.path,
                  ...
                  entry.diff && entry.id === selectedId && _jsx(InlineDiff, { diff: entry.diff }),
                ],
              },
              entry.id,
            ),
