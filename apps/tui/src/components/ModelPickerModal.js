...
export function ModelPickerModal({ current, onSelect, onCancel }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "single", paddingX: 1, children: [_jsx(Text, { bold: true, children: "Pick a model" }), _jsx(Box, { marginTop: 1, flexDirection: "column", children: providers.map((provider) => (_jsxs(Box, { flexDirection: "column", marginBottom: 1, children: [_jsx(Text, { color: "yellow", bold: true, children: provider }), KNOWN_MODELS.filter((m) => m.provider === provider).map((m) => {
                            const idx = KNOWN_MODELS.indexOf(m);
                            const isSelected = idx === selected;
                            return (_jsxs(Box, { gap: 1, paddingLeft: 1, children: [_jsx(Text, { color: isSelected ? 'cyan' : undefined, children: isSelected ? '▶' : ' ' }), _jsx(Text, { bold: isSelected, children: m.id }), m.cost && _jsxs(Text, { dimColor: true, children: ["\u2014 ", m.cost] }), m.id === current && _jsx(Text, { color: "green", children: "(current)" })] }, m.id));
                        })] }, provider))) }), _jsx(Text, { dimColor: true, children: "\u2191/\u2193 navigate \u00B7 Enter select \u00B7 Esc cancel" })] }));
}
