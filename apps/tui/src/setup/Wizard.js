...
export function Wizard({ existing, startAtStep, singleStep, onComplete }) {
    ...
    const fullOnlyHint = isFullOnlyStep(state.step) ? (_jsx(Text, { dimColor: true, children: `  advanced step ${currentIdx} of ${totalSteps} · Esc skip` })) : null;
    return (_jsx(WizardContext.Provider, { value: { answers: state.answers, accent, dispatch }, children: _jsxs(Box, { flexDirection: "column", paddingTop: 1, paddingLeft: 2, children: [_jsx(StepRouter, { step: state.step, onComplete: onComplete, dispatch: dispatch }), fullOnlyHint, !state.singleStep && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: DESIGN.textTertiary, children: `  step ${currentIdx} of ${totalSteps}` }) }))] }) }));
}
