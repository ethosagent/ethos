    ...
    return _jsxs(Box, {
      ...
      children: [
        _jsx(Text, {
          ...
          bold: true,
          children: 'Connect a messaging platform:',
        }),
        _jsx(Box, {
          flexDirection: 'column',
          children: PLATFORMS.map((p, i) => {
              ...
              Box,
              {
                ...
                children: [
                  ...
                  _jsx(Text, {
                    ...
                    bold: isSelected,
                    children: p.label.padEnd(20),
                  }),
                  _jsx(Text, {
                    color: isSkip && isSelected ? accent : DESIGN.textTertiary,
                    children: p.hint,
                  }),
                ],
              },
    ...
    return _jsxs(Box, {
      ...
      children: [
        _jsx(Text, {
          color: DESIGN.textPrimary,
          bold: true,
          children: `Validating ${selectedPlatform?.label ?? ''}...`,
        }),
        _jsx(Text, { color: DESIGN.textTertiary, children: '  Checking credentials (3s timeout)' }),
      ],
    });
  }
    ...
    return _jsxs(Box, {
      ...
      children: [
        _jsx(Text, {
          color: DESIGN.textPrimary,
          bold: true,
          children: `${selectedPlatform?.label ?? ''} credentials:`,
        }),
        validation.ok
          ? _jsx(Text, {
              color: DESIGN.success,
              children: `  ${GLYPHS.toolOk} Connected${validation.label ? ` · ${validation.label}` : ''}`,
            })
          : _jsx(Text, {
              color: DESIGN.error,
              children: `  ${GLYPHS.toolFail} ${validation.error ?? 'Validation failed'}`,
            }),
        !validation.ok &&
          _jsx(Text, {
            color: DESIGN.textTertiary,
            children: '  Esc to re-enter credentials   Enter to save anyway',
          }),
        validation.ok &&
          _jsx(Text, { color: DESIGN.textTertiary, children: '  Enter to continue' }),
      ],
    });
  }
  // configure phase
  ...
  return _jsxs(Box, {
    ...
    children: [
      _jsx(Text, {
        color: DESIGN.textPrimary,
        bold: true,
        children: `Configure ${selectedPlatform?.label ?? ''}:`,
      }),
      _jsx(Box, {
        flexDirection: 'column',
        children: fields.map((f, i) => {
          ...
          return _jsxs(
            Box,
            {
              ...
              children: [
                _jsx(Text, {
                  color: isDone ? DESIGN.success : isActive ? accent : DESIGN.textTertiary,
                  children: isDone
                    ? `  ${GLYPHS.toolOk} `
                    : isActive
                      ? `  ${GLYPHS.prompt} `
                ...
                _jsx(Text, {
                  color: isDone ? DESIGN.textSecondary : DESIGN.textPrimary,
                  children: `${f.label}: `,
                }),
                isActive
                  ? _jsx(Text, {
                      color: maskedValue ? DESIGN.textPrimary : DESIGN.textTertiary,
                      children: maskedValue || '(type here)',
                    })
                  : isDone
                    ? _jsx(Text, { color: DESIGN.textTertiary, children: maskedValue || '—' })
                    : null,
              ],
            },
            f.key,
      ...
      _jsx(Text, {
        color: DESIGN.textTertiary,
        children: '  Enter next field   Esc back to platform select',
      }),
    ],
  });
}
