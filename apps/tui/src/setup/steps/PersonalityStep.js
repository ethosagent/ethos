...
function FullMarkPreview({ id }) {
  ...
  return _jsx(Box, {
    ...
    marginLeft: 4,
    children: grid.map((row, r) => _jsx(Text, { color: accent, children: row.join('') }, r)),
  });
}
export function PersonalityStep() {
  ...
  if (previewing && selectedEntry) {
    return _jsxs(Box, {
      ...
      gap: 1,
      children: [
        _jsx(Text, { color: DESIGN.textPrimary, bold: true, children: selectedEntry.id }),
        _jsx(FullMarkPreview, { id: selectedEntry.id }),
        _jsx(Text, { color: DESIGN.textTertiary, children: '  Esc or Space to close preview' }),
      ],
    });
  }
  return _jsxs(Box, {
    ...
    children: [
      _jsx(Text, {
        ...
        bold: true,
        children: 'Choose a default personality:',
      }),
      _jsx(Box, {
        flexDirection: 'column',
        children: PERSONALITIES.map((p, i) => {
          ...
          return _jsxs(
            Box,
            {
              ...
              children: [
                _jsx(Text, {
                  color: isSelected ? cursorAccent : DESIGN.textTertiary,
                  children: ` ${cursor} `,
                }),
                _jsx(Text, { color: pAccent, children: markSlice }),
                _jsx(Text, { color: pAccent, children: GLYPHS.accentStripe }),
                _jsx(Text, {
                  color: isSelected ? DESIGN.textPrimary : DESIGN.textSecondary,
                  bold: isSelected,
                  children: p.id.padEnd(12),
                }),
                _jsx(Text, { color: DESIGN.textTertiary, children: p.description }),
              ],
            },
            p.id,
          );
      ...
      _jsx(Text, {
        ...
        children: '  ↑↓ select   Enter confirm   Space ▤ preview   Esc back',
      }),
    ],
  });
}
